...
class graph
{
	...
	*/
	function graph()
	{
		...
		$this->data = array();
		$this->links = array();
		$this->tips  = array();	//ADDED BY MIBEL
		$this->width = 250;
		$this->height = 200;
		$this->js_path = 'js/';
		$this->swf_path = '';
	 ...
	 */
   	function set_unique_id2($new_id){
	      $this->unique_id = $new_id;
	}
	...
	*/
	...
	function render()
	{
		...
		if($this->output_type == 'js')
		{
		      if(!$this->get_unique_id()){
			$this->set_unique_id();
		      }
		
		        $tmp[] = '<div id="' . $this->unique_id . '"></div>';
		      //$tmp[] = '<object id="'.$this->unique_id. '"></object>';
			$tmp[] = '<script type="text/javascript" src="' . $this->js_path . 'swfobject.js"></script>';
			//$tmp[] = '<script type="text/javascript" src="' . $this->js_path . 'swfobject1_5.js"></script>';
		...
		//pr($this->tips);
		$num = 1;
		foreach($this->tips as $tips){		 
		  if($num == 1){
		    $tmp[] = $this->format_output('tool_tips_set',$tips);
		  }
		  else{
		    $tmp[] = $this->format_output('tool_tips_set_'.$num,$tips);	
		  }
		  $num++;	
		}

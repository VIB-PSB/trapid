...
class TrapidController extends AppController {
    var $name = 'Trapid';
    var $helpers = ['Html', 'Form']; // ,"Javascript","Ajax"); //NOTE: Javascript and Ajax helpers were removed in Cake 2.X
    ...
    var $components = ['Cookie', 'TrapidUtils', 'Sequence', 'Session'];
    var $paginate = [
        'Transcripts' => [
    ...
    function manage_jobs($exp_id = null) {
        ...
        if ($_POST) {
            ...
            $this->redirect(['controller' => 'trapid', 'action' => 'manage_jobs', $exp_id]);
        }
    ...
    function index() {
        ...
        $news_items = $this->News->find('all', [
            'conditions' => ['is_visible' => 'y'],
        ...
        $user_data = $this->Authentication->find('first', ['conditions' => ['user_id' => $user_id, 'email' => $email]]);
    ...
    function experiments() {
        ...
        $user_email = $this->Authentication->find('first', [
            'fields' => ['email'],
        ...
        //shared experiments
        $shared_exp_ids = $this->SharedExperiments->find('all', ['conditions' => ['user_id' => $user_id]]);
        ...
        if (count($shared_experiments) != 0) {
            $all_user_ids = $this->Authentication->find('all', ['fields' => ['user_id', 'email']]);
        ...
        if ($_POST) {
            if (
                array_key_exists('experiment_name', $_POST) &&
                array_key_exists('experiment_description', $_POST) &&
                array_key_exists('data_source', $_POST)
            ) {
                ...
                foreach ($available_sources as $as) {
                    if ($as['DataSources']['db_name'] == $data_source) {
                        $data_source_name = $as['DataSources']['name'];
                    }
                }
                ...
                $this->Experiments->save([
                    ...
                    'process_state' => 'empty',
                    'used_plaza_database' => $data_source
                ]);
                // get last experiment id
                $user_experiments = $this->Experiments->query(
                    "SELECT `experiment_id` FROM `experiments` WHERE `user_id`='" .
                        $this->Authentication->getDataSource()->value($user_id, 'integer') .
                ...
                $this->ExperimentLog->addAction(
                    $exp_id,
                    'create_experiment_options',
                    'reference_database=' . $data_source_name,
                    2
                );
                $this->redirect(['controller' => 'trapid', 'action' => 'experiments']);
            } else {
    ...
    function view_log($exp_id = null) {
        ...
        $this->TrapidUtils->checkPageAccess(
            ...
            $this->process_states['all']
        );
        $this->set('exp_info', $exp_info);
        $this->set('exp_id', $exp_id);

        $log_info = $this->ExperimentLog->find('all', [
            'conditions' => ['experiment_id' => $exp_id],
            'order' => ['ExperimentLog.date ASC']
        ]);
    ...
    function experiment_access($exp_id = null) {
        ...
        $this->TrapidUtils->checkPageAccess(
            $exp_info['title'],
            $exp_info['process_state'],
            $this->process_states['finished']
        );

        //get all the users.
        $all_users_ = $this->Authentication->find('all', ['fields' => ['user_id', 'email']]);
        ...
        if ($_POST && $is_owner && array_key_exists('exp_access_change', $_POST)) {
            ...
            if ($_POST['exp_access_change'] == 'share' && array_key_exists('new_share', $_POST)) {
                ...
                foreach ($selected as $k => $v) {
                    $this->SharedExperiments->save(['user_id' => $k, 'experiment_id' => $exp_id]);
                }
            }
            if ($_POST['exp_access_change'] == 'revoke' && array_key_exists('revoke_email', $_POST)) {
                ...
                if (array_key_exists($revoke_email, $all_users_inv)) {
                    ...
                    $this->SharedExperiments->deleteAll(
                        ['user_id' => $revoke_user_id, 'experiment_id' => $exp_id],
                        false
                    );
                }
            }
        ...
        $shared_user_ids = $this->SharedExperiments->find('all', ['conditions' => ['experiment_id' => $exp_id]]);
        $shared_user_ids = $this->TrapidUtils->reduceArray($shared_user_ids, 'SharedExperiments', 'user_id');
        $shared_users = ['owner' => [], 'shared' => []];
        $shared_users['owner'][$exp_info['user_id']] = $all_users[$exp_info['user_id']];
        foreach ($shared_user_ids as $sui) {
            $shared_users['shared'][$sui] = $all_users[$sui];
        }
    ...
    function experiment_settings($exp_id = null) {
        ...
        if ($_POST) {
            if (array_key_exists('experiment_name', $_POST)) {
                ...
                $this->Experiments->updateAll(
                    ['title' => $this->Experiments->getDataSource()->value($new_exp_name, 'string')],
                    ['experiment_id' => $exp_id]
                );
            }
            if (array_key_exists('experiment_description', $_POST)) {
                ...
                $this->Experiments->updateAll(
                    ['description' => $this->Experiments->getDataSource()->value($new_exp_desc, 'string')],
                    ['experiment_id' => $exp_id]
                );
            }
            $exp_info = $this->Experiments->getDefaultInformation($exp_id);
    ...
    function experiment($exp_id = null) {
        ...
        $standard_experiment_info = $this->Experiments->find('first', ['conditions' => ['experiment_id' => $exp_id]]);
        $this->TrapidUtils->checkPageAccess(
            $standard_experiment_info['Experiments']['title'],
            $standard_experiment_info['Experiments']['process_state'],
            $this->process_states['default']
        );
        ...
        $datasource_info = $this->DataSources->find('first', [
            'conditions' => ['db_name' => $standard_experiment_info['Experiments']['used_plaza_database']]
        ]);
        ...
        if ($standard_experiment_info['Experiments']['process_state'] != 'empty') {
            ...
            $transcripts_p = $this->paginate('Transcripts', ["Transcripts.experiment_id = '" . $exp_id . "'"]);
        ...
        $this->set(
            'title_for_layout',
            'Experiment overview &middot; ' . $standard_experiment_info['Experiments']['title']
        );
    }
    ...
    function similarity_hits($exp_id = null, $transcript_id = null) {
        ...
        $similarity_hits = $this->Similarities->find('first', [
            'fields' => ['UNCOMPRESS(similarity_data) as sim_data'],
            'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
        ]);
        ...
        if ($exp_info['genefamily_type'] == 'HOM') {
            $gf_prefix = $this->DataSources->find('first', ['conditions' => ['name' => $exp_info['datasource']]]);
            $gf_prefix = $gf_prefix['DataSources']['gf_prefix'];
            if ($gf_prefix) {
                $gf_ids = $this->GfData->find('all', [
                    'conditions' => ['gene_id' => $gene_ids, "`gf_id` LIKE '" . $gf_prefix . "%'"]
                ]);
            } else {
                if ($db_type == 'eggnog') {
                    // Set taxonomic scope
                    ...
                    if ($transcript_info['Transcripts']['gf_id']) {
                        $ref_gf = $this->GeneFamilies->find('first', [
                            'fields' => 'plaza_gf_id',
                            'conditions' => [
                                'experiment_id' => $exp_id,
                                'gf_id' => $transcript_info['Transcripts']['gf_id']
                            ]
                        ]);
                    ...
                    $gf_ids = $this->GfData->find('all', [
                        'conditions' => ['gene_id' => $gene_ids, 'scope' => $tax_scope]
                    ]);
                    $this->set('tax_scope_data', $tax_scope_data);
                } else {
                    $gf_ids = $this->GfData->find('all', ['conditions' => ['gene_id' => $gene_ids]]);
                }
            }
            ...
            $transcript_gfs = $this->GeneFamilies->find('all', [
                'conditions' => ['experiment_id' => $exp_id, 'plaza_gf_id' => $plaza_gf_ids]
            ]);
            $transcript_gfs1 = $this->TrapidUtils->indexArraySimple(
                $transcript_gfs,
                'GeneFamilies',
                'plaza_gf_id',
                'gf_id'
            );
            $transcript_gfs2 = $this->TrapidUtils->indexArraySimple(
                $transcript_gfs,
                'GeneFamilies',
                'gf_id',
                'num_transcripts'
        ...
        if ($_POST && array_key_exists('plaza_gf_id', $_POST)) {
            ...
            $num_plaza_genes = $this->GfData->find('count', ['conditions' => ['gf_id' => $new_plaza_gf_id]]);
            if ($num_plaza_genes == 0) {
                $this->set('error', 'Illegal external identifier for gene family');
            }
            ...
            if (array_key_exists('trapid_gf_id', $_POST)) {
                ...
                $new_trapid_gf_info = $this->GeneFamilies->find('first', [
                    'conditions' => ['gf_id' => $new_trapid_gf_id]
                ]);
                if (!$new_trapid_gf_info) {
                    $this->set('error', 'Illegal internal gene family identifier');
                    return;
                }
                $total_new_gf = false;
            }
            ...
            if ($total_new_gf) {
                ...
                $this->GeneFamilies->save([
                    'experiment_id' => $exp_id,
                    'gf_id' => $new_trapid_gf_id,
                    'plaza_gf_id' => $new_plaza_gf_id,
                    'num_transcripts' => '1'
                ]);
                $this->Transcripts->updateAll(
                    ['gf_id' => "'" . $new_trapid_gf_id . "'"],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                );
            }
            //
            else {
                ...
                $this->GeneFamilies->updateAll(
                    [
                        'num_transcripts' => "'" . ($new_trapid_gf_info['GeneFamilies']['num_transcripts'] + 1) . "'",
                        'used_species' => null,
                        'exclude_transcripts' => null,
                        'msa' => null,
                        'msa_stripped' => null,
                        'msa_stripped_params' => null,
                        'tree' => null
                    ],
                    ['experiment_id' => $exp_id, 'gf_id' => $new_trapid_gf_id]
                );
                $this->Transcripts->updateAll(
                    ['gf_id' => "'" . $new_trapid_gf_id . "'"],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
            ...
            $prev_transcript_count = $this->GeneFamilies->find('first', [
                'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $prev_gf_id],
                'fields' => ['num_transcripts']
            ]);
            ...
            $this->ExperimentLog->addAction(
                $exp_id,
                'change_genefamily',
                $transcript_id . '(' . $prev_gf_id . '->' . $new_trapid_gf_id . ')'
            );

            if ($prev_transcript_count == 0) {
                //delete this entry
                $this->GeneFamilies->deleteAll(['experiment_id' => $exp_id, 'gf_id' => $prev_gf_id]);
            } else {
                $this->GeneFamilies->updateAll(
                    [
                        'num_transcripts' => "'" . $prev_transcript_count . "'",
                        'used_species' => null,
                        'exclude_transcripts' => null,
                        'msa' => null,
                        'msa_stripped' => null,
                    ...
                    ['experiment_id' => $exp_id, 'gf_id' => $prev_gf_id]
                );
            }
            ...
            $shell_file = $this->TrapidUtils->create_shell_script_data_update_gf(
                $exp_id,
            ...
            if ($shell_file == null || $qsub_file == null) {
                $this->Session->write(
                    'error',
                    'Problem creating program files. Functional annotation synchronization not performed.'
                );
            ...
            $this->redirect(['controller' => 'trapid', 'action' => 'similarity_hits', $exp_id, $transcript_id]);
        }
    ...
    function rna_similarity_hits($exp_id = null, $transcript_id = null) {
        ...
        $rna_similarity_hits = $this->RnaSimilarities->find('first', [
            'fields' => ['UNCOMPRESS(similarity_data) as sim_data'],
            'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
        ]);
    ...
    function transcript($exp_id = null, $transcript_id = null) {
        ...
        if ($_POST) {
            ...
            if (array_key_exists('orf_sequence', $_POST)) {
                $this->Transcripts->updateAll(
                    [
                        'orf_sequence' =>
                            'COMPRESS(' .
                            $this->Transcripts->getDataSource()->value($_POST['orf_sequence'], 'string') .
                            ')'
                    ],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                );
            ...
            if (array_key_exists('transcript_sequence', $_POST)) {
                $this->Transcripts->updateAll(
                    [
                        'transcript_sequence' =>
                            'COMPRESS(' .
                            $this->Transcripts->getDataSource()->value($_POST['transcript_sequence'], 'string') .
                            ')'
                    ],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                );
                ...
                $this->Transcripts->updateAll(
                    [
                        'transcript_sequence_corrected' =>
                            'COMPRESS(' .
                            $this->Transcripts->getDataSource()->value($_POST['corrected_sequence'], 'string') .
                            ')'
                    ],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                );
            ...
            if (array_key_exists('meta_annotation', $_POST)) {
                $this->Transcripts->updateAll(
                    [
                        'meta_annotation' => $this->Transcripts
                            ->getDataSource()
                            ->value($_POST['meta_annotation'], 'string')
                    ],
                    ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                );
            ...
            if (array_key_exists('subsets', $_POST) && $_POST['subsets'] == 'subsets') {
                ...
                $transcript_subsets = $this->TranscriptsLabels->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id]
                ]);
                $transcript_subsets = $this->TrapidUtils->reduceArray(
                    $transcript_subsets,
                    'TranscriptsLabels',
                    'label'
                );
                ...
                if (
                    array_key_exists('new_subset', $_POST) &&
                    $_POST['new_subset'] == 'on' &&
                    array_key_exists('new_subset_name', $_POST)
                ) {
                    ...
                    $save_data = [
                        ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id, 'label' => $new_subset]
                    ];
                ...
                foreach ($_POST as $k => $v) {
                    if ($v == 'on' && array_key_exists($k, $available_subsets) && !in_array($k, $transcript_subsets)) {
                        $save_data[] = ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id, 'label' => $k];
                    }
                }
                ...
                foreach ($transcript_subsets as $subset) {
                    if (!array_key_exists($subset, $_POST)) {
                        // Deletion of subset
                        $this->TranscriptsLabels->query(
                            "DELETE FROM `transcripts_labels` WHERE `experiment_id`='" .
                                $exp_id .
                                "' AND `transcript_id`='" .
                                $transcript_id .
                                "' AND `label`='" .
        ...
        if ($transcript_info['Transcripts']['orf_sequence'] != '') {
            $transcript_info['Transcripts']['aa_sequence'] = $this->Sequence->translate_cds_php(
                $transcript_info['Transcripts']['orf_sequence'],
                $transcript_info['Transcripts']['transl_table']
            );
        }
        $this->set('transcript_info', $transcript_info['Transcripts']);
        ...
        // 1. GO
        $associated_go = $this->TranscriptsGo->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id, 'type' => 'go']
        ]);
        ...
        // 2. InterPro
        $associated_interpro = $this->TranscriptsInterpro->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id, 'type' => 'ipr']
        ]);
        ...
        // 3. KO
        $associated_ko = $this->TranscriptsKo->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id, 'type' => 'ko']
        ]);
        ...
        if ($exp_info['perform_tax_binning'] == 1) {
            ...
            $transcript_txdata = $this->TranscriptsTax->find('first', [
                'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $transcript_id],
                'fields' => ['txid', 'tax_results']
            ]);
            ...
            // Otherwise retrieve taxonomy data from `full_taxonomy` and parse `tax_results` string to extract Kaiju score.
                ...
                $txdata = $this->FullTaxonomy->find('first', [
                    'fields' => ['scname', 'tax'],
                    'conditions' => ['txid' => $transcript_txid]
                ]);
        ...
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'transcript%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
    ...
    function transcript_selection() {
        ...
        if ($num_parameters < 3 || $num_parameters % 2 == 0) {
            $this->redirect('/');
        }
        ...
        $download_type = null;
        $available_download_types = ['table', 'fasta_transcript', 'fasta_orf', 'fasta_protein_ref'];
        if ($_POST) {
            if (
                array_key_exists('download_type', $_POST) &&
                in_array($_POST['download_type'], $available_download_types)
            ) {
        ...
        if ($download_type == 'fasta_transcript') {
            $this->set('file_name', 'transcripts_' . implode('_', $parameters) . '.fasta');
            return;
        }
        if ($download_type == 'fasta_orf') {
            $this->set('file_name', 'orf_' . implode('_', $parameters) . '.fasta');
            return;
        }
        if ($download_type == 'fasta_protein_ref') {
            //should only be accesible from a GF page, or any other with only a single restriction
        ...
        if ($download_type == 'table') {
            $this->set('file_name', 'table_' . implode('_', $parameters) . '.tsv');
            return;
        }
    }
  ...
  */
    function create_collection_subset($exp_id = null, $collection_type = null) {
        ...
        if ($this->request->is('post')) {
            ...
            $exp_subsets = $this->TranscriptsLabels->find('all', [
                'fields' => ['label'],
                'conditions' => ['experiment_id' => $exp_id]
            ]);
            $subset_exists = false;
            ...
            switch ($collection_type) {
                ...
                case 'go':
                    ...
                    $transcript_data = $this->TranscriptsGo->find('all', [
                        'fields' => 'transcript_id',
                        'conditions' => ['experiment_id' => $exp_id, 'type' => 'go', 'name' => $go_term]
                    ]);
                    $transcript_ids = array_map(function ($x) {
                        return $x['TranscriptsGo']['transcript_id'];
                    }, $transcript_data);
                    break;
                case 'ipr':
                    $transcript_data = $this->TranscriptsInterpro->find('all', [
                        'fields' => 'transcript_id',
                        'conditions' => [
                            'experiment_id' => $exp_id,
                            'is_hidden' => '0',
                    ...
                    $transcript_ids = array_map(function ($x) {
                        return $x['TranscriptsInterpro']['transcript_id'];
                    }, $transcript_data);
                    break;
                case 'ko':
                    $transcript_data = $this->TranscriptsKo->find('all', [
                        'fields' => 'transcript_id',
                        'conditions' => [
                            'experiment_id' => $exp_id,
                            'is_hidden' => '0',
                    ...
                    $transcript_ids = array_map(function ($x) {
                        return $x['TranscriptsKo']['transcript_id'];
                    }, $transcript_data);
                    break;
                case 'gf':
                    $transcript_data = $this->Transcripts->find('all', [
                        'fields' => ['transcript_id'],
                        'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $parameters[0]]
                    ]);
                    $transcript_ids = array_map(function ($x) {
                        return $x['Transcripts']['transcript_id'];
                    }, $transcript_data);
                    break;
                case 'rf':
                    $transcript_data = $this->Transcripts->find('all', [
                        'fields' => ['transcript_id'],
                        'conditions' => ['experiment_id' => $exp_id, 'rf_ids' => $parameters[0]]
                    ]);
                    $transcript_ids = array_map(function ($x) {
                        return $x['Transcripts']['transcript_id'];
                    }, $transcript_data);
                    break;
                case 'subset':
                    $transcript_data = $this->TranscriptsLabels->find('all', [
                        'fields' => ['transcript_id'],
                        'conditions' => ['experiment_id' => $exp_id, 'label' => $parameters[0]]
                    ]);
                    $transcript_ids = array_map(function ($x) {
                        return $x['TranscriptsLabels']['transcript_id'];
                    }, $transcript_data);
                    break;
                default:
                    $transcript_ids = [];
            ...
            if (empty($transcript_ids)) {
                return str_replace(
                    ['{label_class}', '{label_content}'],
                    ['label-warning', 'Error: no transcripts retrieved'],
                    $label_base
                );
            }
            ...
            return str_replace(
                ['{label_class}', '{label_content}'],
                [$creation_class, $creation_msg . ' (' . $counter . ' transcripts)'],
                $label_base
            );
        }
    }
    ...
    function getTrapidSequences($exp_id, $transcript_data) {
        parent::check_user_exp($exp_id);
        $exp_info = $this->Experiments->getDefaultInformation($exp_id);
        $seq_type = $this->DataSources->find('first', [
            'conditions' => ['name' => $exp_info['datasource']],
            'fields' => 'seq_type'
        ]);
    ...
    function getReferenceSequences($exp_id, $param_type, $param_value) {
        ...
        $result = []; //identifier to sequence
        if ($param_type == 'gf_id') {
            if ($gf_type == 'HOM') {
                //get HOM id
                $plaza_gf_id = $this->GeneFamilies->find('first', [
                    'conditions' => ['gf_id' => $param_value],
                    'fields' => 'plaza_gf_id'
                ]);
                $result = $this->Annotation->getSequencesGf($plaza_gf_id['GeneFamilies']['plaza_gf_id']);
            } else {
                //iOrtho group content
                $iortho_content = $this->GeneFamilies->find('first', [
                    'conditions' => ['gf_id' => $param_value],
                    'fields' => 'gf_content'
                ]);
    ...
    function search($exp_id = null) {
        ...
        $sv = filter_var(trim($_POST['search_value']), FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
        ...
        switch ($st) {
            // Transcript sequence
            case 'transcript':
                $transcripts_info = $this->Transcripts->find('count', [
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $sv]
                ]);
                if (!$transcripts_info) {
                    $this->set('search_result', 'bad_search');
                    return;
                } else {
                    $this->redirect(['controller' => 'trapid', 'action' => 'transcript', $exp_id, urlencode($sv)]);
                }
                break;
            ...
            case 'gene':
                ...
                if ($db_type == 'eggnog') {
                    ...
                    $genes = $this->Annotation->find('first', ['conditions' => ['gene_id' => $sv]]);
                    if (!$genes) {
                        $this->set('search_result', 'bad_search');
                        return;
                    }
                    ...
                    $gene_family_data = $this->GfData->find('all', [
                        'fields' => 'gf_id',
                        'conditions' => ['gene_id' => $gene_id]
                    ]);
                    ...
                    $gf_info = $this->GeneFamilies->find('all', [
                        'conditions' => ['experiment_id' => $exp_id, 'plaza_gf_id' => $gf_ids],
                ...
                // PLAZA reference database (default)
                    ...
                    if ($exp_info['genefamily_type'] == 'IORTHO') {
                        ...
                        $this->set('gf_info', $gf_info);
                        return;
                    } elseif ($exp_info['genefamily_type'] == 'HOM') {
                        // Find gene. If there is no corresponding gene in `annotation`, return an error
                        $genes = $this->Annotation->find('first', ['conditions' => ['gene_id' => $sv]]);
                        if (!$genes) {
                            $this->set('search_result', 'bad_search');
                        ...
                        $gene_id = $genes['Annotation']['gene_id'];
                        // Get associated GF information from reference DB
                        $gene_family_data = $this->GfData->find('first', [
                            'fields' => 'gf_id',
                            'conditions' => ['gene_id' => $gene_id, 'gf_id LIKE' => $exp_info['gf_prefix'] . '%']
                        ]);
                        // If no GF is associated to the provided gene identifier, return an error
                        if (!$gene_family_data) {
                            $this->set('search_result', 'bad_search');
                        ...
                        $gf_info = $this->GeneFamilies->find('first', [
                            'conditions' => ['experiment_id' => $exp_id, 'plaza_gf_id' => $gf_id],
                            'fields' => ['gf_id', 'plaza_gf_id', 'num_transcripts']
                        ]);
                        // If nothing corresponds, return an error
                        if (!$gf_info) {
            ...
            // Gene family
            case 'gf':
                $transcripts_info = $this->GeneFamilies->find('count', [
                    'conditions' => ['experiment_id' => $exp_id, 'gf_id LIKE' => "%$sv%"]
                ]);
                if (!$transcripts_info) {
                    $this->set('search_result', 'bad_search');
                    return;
                    ...
                    $gf = $this->GeneFamilies->find('first', [
                        'conditions' => ['experiment_id' => $exp_id, 'gf_id LIKE' => "%$sv%"]
                    ]);
            ...
            case 'go':
                ...
                if ($this->ExtendedGo->isValidGoIdPattern($sv)) {
                    $go_terms = $this->ExtendedGo->find('all', ['conditions' => ['name' => $sv, 'type' => 'go']]);
                }
                ...
                // or find all GO term IDs matching the search string.
                    ...
                    $go_terms = $this->ExtendedGo->find('all', [
                        'conditions' => ['desc LIKE' => "%$sv%", 'type' => 'go']
                    ]);
                }
                if (!$go_terms) {
                    $this->set('search_result', 'bad_search');
                ...
                $go_terms = $this->TrapidUtils->indexArrayMulti($go_terms, 'ExtendedGo', 'name', ['desc', 'info']);
            ...
            case 'interpro':
                ...
                if ($this->ProteinMotifs->isValidIprIdPattern($sv)) {
                    ...
                    $ipr_terms = $this->ProteinMotifs->find('all', [
                        'conditions' => ['name' => $sv, 'type' => 'interpro']
                    ]);
                }
                // Otherwise, it is a description: find all InterPro IDs matching it
                else {
                    $ipr_terms = $this->ProteinMotifs->find('all', [
                        'conditions' => ['desc LIKE' => "%$sv%", 'type' => 'interpro']
                    ]);
                }
            ...
            case 'ko':
                ...
                if ($this->KoTerms->isValidKoIdPattern($sv)) {
                    $ko_terms = $this->KoTerms->find('all', ['conditions' => ['name' => $sv, 'type' => 'ko']]);
                }
                // Otherwise, it is a description: find all KO term IDs matching it
                else {
                    $ko_terms = $this->KoTerms->find('all', ['conditions' => ['desc LIKE' => "%$sv%", 'type' => 'ko']]);
                }
            ...
            // RNA family
            case 'rf':
                $transcripts_info = $this->RnaFamilies->find('count', [
                    'conditions' => ['experiment_id' => $exp_id, 'rf_id LIKE' => "%$sv%"]
                ]);
                if (!$transcripts_info) {
                    $this->set('search_result', 'bad_search');
                    return;
                } else {
                    $rf = $this->RnaFamilies->find('first', [
                        'conditions' => ['experiment_id' => $exp_id, 'rf_id LIKE' => "%$sv%"]
                    ]);
    ...
    function suggest_search($exp_id = null, $search_type = null, $search_str = null) {
        ...
        parent::check_user_exp($exp_id);
        if (!$exp_id || !$search_type || !$search_str) {
            return;
        }
        if (!in_array($search_type, $allowed_types)) {
            return;
        }
        if (strlen($search_str) < $min_length) {
            return;
        }
        ...
        switch ($search_type) {
            case 'transcript':
                $search_data = $this->Transcripts->find('all', [
                    'fields' => ['transcript_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id LIKE' => "%$search_str%"],
                    'limit' => $limit_results
                ]);
                $suggestions = array_map(function ($x) {
                    return $x['Transcripts']['transcript_id'];
                }, $search_data);
                $exact_data = $this->Transcripts->find('first', [
                    'fields' => ['transcript_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $search_str]
                ]);
                if ($exact_data) {
            ...
            case 'gf':
                $search_data = $this->GeneFamilies->find('all', [
                    'fields' => ['gf_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'gf_id LIKE' => "%$search_str%"],
                    'limit' => $limit_results
                ]);
                $suggestions = array_map(function ($x) {
                    return $x['GeneFamilies']['gf_id'];
                }, $search_data);
                $exact_data = $this->GeneFamilies->find('first', [
                    'fields' => ['gf_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $search_str]
                ]);
                if ($exact_data) {
            ...
            case 'rf':
                $search_data = $this->RnaFamilies->find('all', [
                    'fields' => ['rf_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'rf_id LIKE' => "%$search_str%"],
                    'limit' => $limit_results
                ]);
                $suggestions = array_map(function ($x) {
                    return $x['RnaFamilies']['rf_id'];
                }, $search_data);
                $exact_data = $this->RnaFamilies->find('first', [
                    'fields' => ['rf_id'],
                    'conditions' => ['experiment_id' => $exp_id, 'rf_id' => $search_str]
                ]);
                if ($exact_data) {
            ...
            case 'go':
                ...
                if (preg_match("/^GO:[0-9]{1,7}$/i", $search_str)) {
                    $search_data = $this->ExtendedGo->find('all', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'go', 'LOWER(name) LIKE' => "$search_str%"], // LOWER() since `name` uses a case-sensitive collation in EggNOG ref. DB
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = [
                            'name' => $sd['ExtendedGo']['name'],
                            'info' => $sd['ExtendedGo']['info'],
                            'desc' => $sd['ExtendedGo']['desc']
                        ];
                    }
                } else {
                    $search_data = $this->ExtendedGo->find('all', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'go', 'desc LIKE' => "%$search_str%"],
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = [
                    ...
                    $exact_data = $this->ExtendedGo->find('first', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'go', 'desc' => $search_str]
                    ]);
                    if ($exact_data) {
                        $suggestions[] = [
                            'name' => $exact_data['ExtendedGo']['name'],
                            'info' => $exact_data['ExtendedGo']['info'],
                            'desc' => $exact_data['ExtendedGo']['desc']
                        ];
                    }
                }
                break;
            ...
            case 'interpro':
                ...
                if (preg_match("/^IPR[0-9]{1,6}$/i", $search_str)) {
                    $search_data = $this->ProteinMotifs->find('all', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'interpro', 'name LIKE' => "$search_str%"],
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = [
                            'name' => $sd['ProteinMotifs']['name'],
                            'info' => $sd['ProteinMotifs']['info'],
                            'desc' => $sd['ProteinMotifs']['desc']
                        ];
                    }
                } else {
                    $search_data = $this->ProteinMotifs->find('all', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'interpro', 'desc LIKE' => "%$search_str%"],
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = [
                    ...
                    $exact_data = $this->ProteinMotifs->find('first', [
                        'fields' => ['name', 'desc', 'info'],
                        'conditions' => ['type' => 'interpro', 'desc' => $search_str]
                    ]);
                    if ($exact_data) {
                        $suggestions[] = [
                            'name' => $exact_data['ProteinMotifs']['name'],
                            'info' => $exact_data['ProteinMotifs']['info'],
                            'desc' => $exact_data['ProteinMotifs']['desc']
                        ];
                    }
                }
                break;
            ...
            case 'ko':
                ...
                if (preg_match("/^K[0-9]{2,5}$/i", $search_str)) {
                    $search_data = $this->KoTerms->find('all', [
                        'fields' => ['name', 'desc'],
                        'conditions' => ['type' => 'ko', 'LOWER(name) LIKE' => "$search_str%"], // LOWER() since `name` uses a case-sensitive collation in EggNOG ref. DB
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = ['name' => $sd['KoTerms']['name'], 'desc' => $sd['KoTerms']['desc']];
                    }
                } else {
                    $search_data = $this->KoTerms->find('all', [
                        'fields' => ['name', 'desc'],
                        'conditions' => ['type' => 'ko', 'desc LIKE' => "%$search_str%"],
                        'limit' => $limit_results
                    ]);
                    foreach ($search_data as $sd) {
                        $suggestions[] = ['name' => $sd['KoTerms']['name'], 'desc' => $sd['KoTerms']['desc']];
                    }
                    $exact_data = $this->KoTerms->find('first', [
                        'fields' => ['name', 'desc'],
                        'conditions' => ['type' => 'ko', 'desc' => $search_str]
                    ]);
                    if ($exact_data) {
                        $suggestions[] = [
                            'name' => $exact_data['KoTerms']['name'],
                            'desc' => $exact_data['KoTerms']['desc']
                        ];
                    }
                }
                break;
        ...
        if ($_POST) {
            ...
            foreach (array_keys($possible_types) as $type) {
                if (!array_key_exists($type, $possible_types)) {
                    $this->redirect(['controller' => 'trapid', 'action' => 'enrichment_preprocessing', $exp_id]);
                }
                ...
                $shell_file = $this->TrapidUtils->create_shell_file_enrichment_preprocessing(
                    $exp_id,
                    $ini_file,
                    $type,
                    $possible_pvalues,
                ...
                $this->Experiments->updateAll(['enrichment_state' => "'processing'"], ['experiment_id' => $exp_id]);
    ...
    function initial_processing($exp_id = null) {
        ...
        $exp_info = $this->Experiments->getDefaultInformation($exp_id);
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'initial_processing%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
        ...
        $possible_db_types = [
            ...
            'GF_REP' => 'Gene Family Representatives'
        ];
        $possible_gf_types = ['HOM' => 'Gene Families', 'IORTHO' => 'Integrative Orthology'];
        ...
        $data_sources = $this->DataSources->find('first', [
            'conditions' => ['db_name' => $exp_info['used_plaza_database']]
        ]);
        ...
        $gf_representatives = $this->TrapidUtils->checkAvailableDiamondDB($exp_info['used_plaza_database'], [
            'gf_representatives' => 'Genefamily representatives'
        ]);
        ...
        if (count($species_info) == 0) {
            $this->set(
                'error',
                'No valid species similarity search databases found. Species similarity search databases are available for PLAZA reference databases only.'
            );
            $this->set('no_species_available', true);
        }
        if (count($clades) == 0 && count($species_info) > 0) {
            $this->set('error', 'No valid clades similarity search databases found. ');
        }
        if (count($species_info) == 0) {
            unset($possible_db_types['SINGLE_SPECIES']);
        }
        if (count($clades) == 0) {
            unset($possible_db_types['CLADE']);
        }
        if (count($gf_representatives) == 0) {
            unset($possible_db_types['GF_REP']);
        }

        //retrieve information for the gene family type
        if ($data_sources['DataSources']['gf'] == 0) {
            unset($possible_gf_types['HOM']);
        }
        if ($data_sources['DataSources']['iortho'] == 0) {
            unset($possible_gf_types['IORTHO']);
        }
        ...
        $possible_evalues = ['2', '3', '4', '5', '6', '7', '8', '9', '10'];
        ...
        $default_sim_search_clade = $this->Configuration->find('first', [
            'fields' => 'value',
            'conditions' => [
                'method' => 'initial_processing_settings',
                'key' => $data_sources['DataSources']['db_name'],
                'attr' => 'default_sim_search_clade'
            ]
        ]);
        ...
        if ($_POST) {
            ...
            if (
                !(
                    array_key_exists('blast_db_type', $_POST) &&
                    array_key_exists('blast_db', $_POST) &&
                    array_key_exists('blast_evalue', $_POST) &&
                    array_key_exists('gf_type', $_POST) &&
                    array_key_exists('functional_annotation', $_POST)
                )
            ) {
                $this->set('error', 'Incorrect parameters : missing parameters');
                return;
            }
            $num_blast_hits = 1;
            ...
            if (
                !(
                    array_key_exists($blast_db_type, $possible_db_types) &&
                ...
                $this->set('error', 'Incorrect parameters: faulty parameters');
                return;
            }
            if ($blast_db_type == 'SINGLE_SPECIES') {
                if (!array_key_exists($blast_db, $species_info)) {
                    $this->set('error', 'Incorrect parameters: unknown species - ' . $blast_db);
                    return;
                } else {
                    $used_blast_desc = $species_info[$blast_db];
                }
            }
            if ($blast_db_type == 'CLADE') {
                if (!array_key_exists($blast_db, $clades)) {
                    $this->set('error', 'Incorrect parameters : unknown clade - ' . $blast_db);
                    return;
                } else {
                    $used_blast_desc = $clades[$blast_db];
                }
            }
            if ($blast_db_type == 'GF_REP') {
                if ($blast_db != 'gf_representatives') {
                    $this->set('error', 'Incorrect parameters : wrong GF_REP DB - ' . $blast_db);
                    return;
                }
            }
            if ($gf_type == 'IORTHO' && $blast_db_type != 'SINGLE_SPECIES') {
                $this->set('error', 'Incorrect parameters : IORTHO conflict');
                return;
            }

            if ($blast_db_type == 'SINGLE_SPECIES') {
                $num_blast_hits = 1;
            } elseif ($blast_db_type == 'CLADE') {
                $num_blast_hits = 1;
            } elseif ($blast_db_type == 'GF_REP') {
                $num_blast_hits = 5;
            }
            ...
            if (!isset($_POST['transl_table'])) {
                $this->set('error', 'Incorrect genetic code/translation table, please try again. ');
                return;
            }
            ...
            $ini_file = $this->TrapidUtils->create_ini_file_initial(
                $exp_id,
                $exp_info['used_plaza_database'],
                $blast_db,
                $gf_type,
            ...
            if ($shell_file == null || $qsub_file == null || $ini_file == null) {
                $this->set('error', 'Problem creating program files');
                return;
            }
            ...
            $this->Experiments->updateAll(
                [
                    ...
                    'genefamily_type' => "'" . $gf_type . "'",
                    'last_edit_date' => "'" . date('Y-m-d H:i:s') . "'",
                    'used_blast_database' => "'" . $possible_db_types[$blast_db_type] . '/' . $used_blast_desc . "'",
                    'perform_tax_binning' => (int) $perform_tax_binning
                ],
                ['experiment_id' => $exp_id]
            );
            if ($gf_type == 'IORTHO') {
                $this->Experiments->updateAll(
                    ['target_species' => "'" . $blast_db . "'"],
                    ['experiment_id' => $exp_id]
                );
            }
            ...
            $this->ExperimentLog->addAction(
                $exp_id,
                'initial_processing_options',
                'n_rfam_clans=' . sizeof($rfam_clans),
                2
            );
    ...
    function import_data($exp_id = null) {
        ...
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'data_upload%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
        ...
        $uploaded_files = $this->DataUploads->find('all', [
            'conditions' => ['user_id' => $exp_info['user_id'], 'experiment_id' => $exp_id]
        ]);
        ...
        if ($_POST && array_key_exists('type', $_POST)) {
            ...
            if ($_POST['type'] == 'upload_file' && array_key_exists('uploadtype', $_POST)) {
                ...
                if (array_key_exists('label_name', $_POST)) {
                    $label_name = Sanitize::paranoid($_POST['label_name'], ['.', '_', '-']); //also remove other characters b/o problems
                }
                //#####   URL ###########################
                if ($_POST['uploadtype'] == 'url' && array_key_exists('uploadedurl', $_POST)) {
                    ...
                    $this->DataUploads->saveAll([
                        [
                            'user_id' => $exp_info['user_id'],
                            'experiment_id' => $exp_id,
                            'type' => 'url',
                            'name' => $uploadurl,
                            'label' => $label_name,
                    ...
                    $this->redirect(['controller' => 'trapid', 'action' => 'import_data', $exp_id]);
                }
                //#####   FILE  ###########################  --> IMMEDIATELY UNZIP DATA!!!
                elseif ($_POST['uploadtype'] == 'file' && array_key_exists('uploadedfile', $_FILES)) {
	   ...
	   $MAX_FILE_SIZE_ZIP			= 32000000;*/
                    ...
                    if ($_FILES['uploadedfile']['name'] == '') {
                        $this->set('error', 'Illegal input file (no name)');
                        return;
                    }
                    if ($_FILES['uploadedfile']['size'] == 0) {
                        $this->set('error', 'Illegal input file (size is 0)');
                        return;
                    }
                    if ($_FILES['uploadedfile']['tmp_name'] == '') {
                        $this->set('error', 'Illegal input file (no tmp_name)');
                        return;
                    }
                    ...
                    if (count($filename_d) == 1) {
                        $this->set('error', 'No file extension detected. Please rename file.');
                        return;
                    }
                    $file_extension = $filename_d[count($filename_d) - 1];
                    if (count($filename_d) >= 3 && $filename_d[count($filename_d) - 2] == 'tar') {
                        $file_extension = 'tar.gz';
                    }
                    $normal_fasta_file = true;
                    if ($file_extension == 'fasta' || $file_extension == 'tfa' || $file_extension == 'fa') {
                        $normal_fasta_file = true;
                    } elseif ($file_extension == 'zip' || $file_extension == 'gz' || $file_extension == 'tar.gz') {
                        $normal_fasta_file = false;
                    } else {
                        $this->set(
                            'error',
                            "Illegal file-name. Only allowed file-extensions: 'fasta','fa','tfa' (normal) ,'zip','gz' (compressed)"
                        );
                        return;
                    }
                    if ($normal_fasta_file) {
                        ...
                        if ($_FILES['uploadedfile']['size'] > $MAX_FILE_SIZE_NORMAL) {
                            $this->set('error', 'File is too large');
                            return;
                        }
                    } else {
                        if ($_FILES['uploadedfile']['size'] > $MAX_FILE_SIZE_ZIP) {
                            $this->set('error', 'File is too large');
                            return;
                        }
                    }
                    ...
                    $this->set('error', 'Invalid upload type');
                    $this->set('uploaded_files', $uploaded_files);
                    return;
                }
            }
            ...
            //#########################################
            elseif ($_POST['type'] == 'delete_file') {
                foreach ($_POST as $k => $v) {
                    if (substr($k, 0, 3) == 'id_' && $v == 'on') {
                        $to_del = substr($k, 3);
                        if (array_key_exists($to_del, $uploaded_files_index)) {
                            if (
                                $uploaded_files_index[$to_del]['user_id'] == $exp_info['user_id'] &&
                                $uploaded_files_index[$to_del]['experiment_id'] == $exp_id
                            ) {
                                $type = $uploaded_files_index[$to_del]['type'];
                                //delete from database
                                $this->DataUploads->query(
                                    "DELETE FROM `data_uploads` WHERE `user_id`='" .
                                        $exp_info['user_id'] .
                                        "' AND
			`experiment_id`='" .
                                        $exp_id .
            ...
            //     DATABASE UPLOAD
            //#########################################
            elseif ($_POST['type'] == 'database_file') {
                ...
                $this->Experiments->updateAll(
                    ['process_state' => "'loading_db'", 'last_edit_date' => "'" . date('Y-m-d H:i:s') . "'"],
                    ['experiment_id' => $exp_id]
                );
                ...
                $this->set('error', 'No valid type defined');
                $this->set('uploaded_files', $uploaded_files);
                return;
            }
        }
    ...
    function import_data_old($exp_id = null) {
        ...
        if ($_POST) {
            ...
            if ($normal_fasta_file) {
                ...
                if ($file_extension == 'gz' || $file_extension == 'tar.gz') {
                    ...
                    if (count($file_info) < 2) {
                        $this->set('error', 'Corrupt archive');
                        return;
                    }
                    ...
                    if ($f[1] > $MAX_FILE_SIZE_ZIP_EXPANDED) {
                        $this->set('error', 'Uncompressed file is too large');
                        return;
                    }
                    //extract file
                    if ($file_extension == 'gz') {
                        shell_exec('gunzip -c ' . $myFile . ' > ' . $tmp_dir . 'unzipped.fasta');
                    } elseif ($file_extension == 'tar.gz') {
                    ...
                    if ($file_size > $MAX_FILE_SIZE_ZIP_EXPANDED) {
                        $this->set('error', 'Uncompressed file is too large');
                        return;
                    }
                    //extract file
            ...
            $fh = fopen($myFile, 'r');
            if (!$fh) {
                $this->set('error', 'Cannot open file');
                return;
            }
            $transcripts = [];
            ...
            while (($buffer = fgets($fh)) !== false) {
                //read line per line instead of entire file at once, to prevent memory issues
                if (substr($buffer, 0, 1) == '>') {
                    if ($current_transcript != null) {
                        if (count($transcripts) > 1000) {
                            //reduce out of memory problems
            ...
            $this->Experiments->updateAll(
                ['last_edit_date' => "'" . date('Y-m-d H:i:s') . "'", 'process_state' => "'upload'"],
                ['experiment_id' => $exp_id]
            );
    ...
    function import_labels($exp_id = null) {
        ...
        if ($this->request->is('post')) {
            ...
            if (!isset($_FILES['uploadedfile'])) {
                ...
                $this->redirect(['controller' => 'trapid', 'action' => 'experiment']);
            }
            if (
            ...
            // Create the subset
            $label = Sanitize::paranoid($_POST['label'], ['.', '_', '-']);
            $myFile = $_FILES['uploadedfile']['tmp_name'];
            $fh = fopen($myFile, 'r');
    ...
    function export_data($exp_id = null) {
        ...
        $structural_export = [
            ...
            'frameshift_info' => 'Frameshift information',
            'orf' => 'ORF information',
            'meta_annotation' => 'Meta annotation'
        ];
        $structural_export_cols = [
            'transcript_id' => ['transcript_id'],
            'frame_info' => ['detected_frame', 'detected_strand', 'full_frame_info'],
            'frameshift_info' => ['putative_frameshift'],
            ...
            if ($export_type == 'structural') {
                ...
                foreach ($_POST as $k => $v) {
                    if (array_key_exists($k, $structural_export_cols)) {
                        foreach ($structural_export_cols[$k] as $col) {
                            $columns[] = $col;
                        }
                    }
                }
                ...
                if ($sequence_type == 'original') {
                    $export_data = $this->TrapidUtils->initiateExport(
                        ...
                        $exp_id,
                        'SEQ_TRANSCRIPT',
                        'transcripts' . $outfile_suffix . '.fasta',
                        $subset_label
                    );
     ...
     */
    function handle_export_data($exp_id = null, $cluster_job_id = null, $timestamp = null, $has_timed_out = null) {
        $this->autoRender = false;
        $this->response->type('json');
        parent::check_user_exp($exp_id);
    ...
    function empty_experiment($exp_id = null) {
        ...
        $this->Experiments->updateAll(
            ['last_edit_date' => "'" . date('Y-m-d H:i:s') . "'", 'process_state' => "'empty'"],
            ['experiment_id' => $exp_id]
        );
    ...
    function delete_experiment($exp_id = null) {
        ...
        if ($exp_info['process_state'] == 'empty') {
            $this->redirect(['controller' => 'trapid', 'action' => 'delete_experiment_sync', $exp_id]);
        }
        ...
        if ($shell_file == null || $qsub_file == null) {
            ...
            $this->Experiments->updateAll(['process_state' => "'deleting'"], ['experiment_id' => $exp_id]);
            $command = "sh $qsub_file -q medium -o $qsub_out -e $qsub_err " . ' -cwd ' . $shell_file;
            exec($command, $output);
            $this->redirect(['controller' => 'trapid', 'action' => 'experiments']);
        }
    ...
    function delete_experiment_sync($exp_id = null) {
        parent::check_user_exp($exp_id);
        $exp_info = $this->Experiments->getDefaultInformation($exp_id);
        $this->TrapidUtils->checkPageAccess(
            $exp_info['title'],
        ...
        $this->TranscriptsLabels->query("DELETE FROM `transcripts_labels` WHERE `experiment_id`='" . $exp_id . "'");
        $this->FunctionalEnrichments->query(
        ...
        // Remove experiment
        $this->Experiments->deleteAll(['Experiments.experiment_id' => $exp_id]);
        ...
        $this->DeletedExperiments->save([
            ...
            'used_plaza_database' => $exp_info['used_plaza_database'],
            'num_transcripts' => $exp_info['transcript_count'],
            'title' => $exp_info['title'],
            'creation_date' => $exp_info['creation_date'],
            'last_edit_date' => $exp_info['last_edit_date'],
    ...
    function change_password() {
        ...
        if ($_POST) {
            if (array_key_exists('new_password1', $_POST) && array_key_exists('new_password2', $_POST)) {
                ...
                if ($pass1 != $pass2) {
                    $this->set('error', 'Passwords are not the same');
                    return;
                }
                if (strlen($pass1) < 8) {
                    $this->set('error', 'Passwords need to consist of 8 or more characters');
                    return;
                }

                //checks done, create hash-version of the new password
                $hashed_pass = hash('sha256', $pass1);
                $this->Authentication->updateAll(['password' => "'" . $hashed_pass . "'"], ['user_id' => $user_id]);
        ...
        if ($_POST) {
            ...
            if (array_key_exists('registration', $_POST)) {
                ...
                if (
                    ...
                    $this->set('error', 'Invalid form parameters');
                    return;
                }
                // No need to escape?
                $email = $_POST['login'];
                ...
                if (!($email != '' && $organization != '' && $country != '')) {
                    $this->set('error', 'Not all fields are filled in');
                    return;
                }
                // Check whether valid email address, using the models validation
                $this->Authentication->set(['email' => $email]);
                ...
                $user_data = $this->Authentication->find('first', ['conditions' => ['email' => $email]]);
                if ($user_data) {
                    $this->set('error', 'Email-address already in use');
                    return;
                }
            ...
            if (array_key_exists('pass_recovery', $_POST)) {
                ...
                if (!$user_data) {
                    $this->set('error', 'Email-address does not correspond to a TRAPID login. ');
                    return;
                }
            ...
            // Logging in
                ...
                if (array_key_exists('login', $_POST) && array_key_exists('password', $_POST)) {
                    ...
                    if (!$user_data) {
                        $this->set('error', 'Wrong email/password');
                        return;
                    }
    ...
    function cleanup_experiments() {
        ...
        if ($cleanup_id == -1) {
            ...
            $qsub_file = $this->TrapidUtils->create_qsub_script_general();
            $shell_script = $this->TrapidUtils->create_monthly_cleanup_script(
                $year,
                $month,
                $cleanup_warning,
                $cleanup_delete

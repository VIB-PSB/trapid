...
class FunctionalAnnotationController extends AppController {
    ...
    var $paginate = [
        'TranscriptsGo' => ['limit' => 10, 'order' => ['TranscriptsGo.transcript_id' => 'ASC']],
        'TranscriptsInterpro' => ['limit' => 10, 'order' => ['TranscriptsInterpro.transcript_id' => 'ASC']],
        'TranscriptsKo' => ['limit' => 10, 'order' => ['TranscriptsKo.transcript_id' => 'ASC']]
    ];
    var $uses = [
        'AnnotSources',
        'Annotation',
    ...
    function child_go($exp_id = null, $go_web = null) {
        ...
        $go_information = $this->ExtendedGo->find('first', ['conditions' => ['type' => 'go', 'name' => $go]]);
        $num_transcripts = $this->TranscriptsGo->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'type' => 'go', 'name' => $go]
        ]);
        ...
        $child_gos = $this->GoParents->find('all', [
            'conditions' => ['parent' => $go, 'type' => 'go'],
            'fields' => ['child']
        ]);
        ...
        $go_descriptions = $this->ExtendedGo->find('all', ['conditions' => ['type' => 'go', 'name' => $child_gos]]);
        $go_descriptions = $this->TrapidUtils->indexArrayMulti($go_descriptions, 'ExtendedGo', 'name', [
            'desc',
            'info'
        ]);
    ...
    function parent_go($exp_id = null, $go_web = null) {
        ...
        $go_information = $this->ExtendedGo->find('first', ['conditions' => ['name' => $go, 'type' => 'go']]);
        $num_transcripts = $this->TranscriptsGo->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'type' => 'go', 'name' => $go]
        ]);
        if (!$go_information || $num_transcripts == 0) {
        ...
        $parental_gos = $this->GoParents->find('all', [
            'conditions' => ['child' => $go, 'type' => 'go'],
            'fields' => ['parent']
        ]);
        ...
        $go_descriptions = $this->ExtendedGo->find('all', ['conditions' => ['name' => $parental_gos, 'type' => 'go']]);
        $go_descriptions = $this->TrapidUtils->indexArrayMulti($go_descriptions, 'ExtendedGo', 'name', [
            'desc',
            'info'
        ]);
    ...
    function go($exp_id = null, $go_web = null) {
        ...
        $go_information = $this->ExtendedGo->find('first', ['conditions' => ['name' => $go]]);
        $num_transcripts = $this->TranscriptsGo->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'type' => 'go', 'name' => $go]
        ]);
        if (!$go_information || $num_transcripts == 0) {
        ...
        $transcripts_p = $this->paginate('TranscriptsGo', ['experiment_id' => $exp_id, 'name' => $go, 'type' => 'go']);
    ...
    function interpro($exp_id = null, $interpro = null) {
        ...
        $interpro_info = $this->ProteinMotifs->find('first', [
            'conditions' => ['name' => $interpro, 'type' => 'interpro']
        ]);
        $num_transcripts = $this->TranscriptsInterpro->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'type' => 'ipr', 'name' => $interpro]
        ]);
        ...
        $transcripts_p = $this->paginate('TranscriptsInterpro', [
            'experiment_id' => $exp_id,
            'type' => 'ipr',
            'name' => $interpro
        ]);
    ...
    function assoc_gf($exp_id = null, $type = null, $identifier = null) {
        ...
        if ($type == 'go') {
            ...
            if ($num_transcripts > $max_transcripts) {
                $this->set(
                    'error',
                    'Unable to find associated gene families for GO terms with more than ' .
                        $max_transcripts .
                        ' associated transcripts'
                );
                return;
            }
            $transcripts_p = $this->TranscriptsGo->find('all', [
            ...
            $interpro_information = $this->ProteinMotifs->find('first', [
                'conditions' => ['name' => $interpro, 'type' => 'interpro']
            ]);
            ...
            if ($num_transcripts > $max_transcripts) {
                $this->set(
                    'error',
                    'Unable to find associated gene families for InterPro domains with more than ' .
                        $max_transcripts .
                        ' associated transcripts'
                );
                return;
            }
            $transcripts_p = $this->TranscriptsInterpro->find('all', [
                'fields' => ['transcript_id'],
                'conditions' => ['experiment_id' => $exp_id, 'type' => 'ipr', 'name' => $interpro]
            ]);
            $transcript_ids = $this->TrapidUtils->reduceArray($transcripts_p, 'TranscriptsInterpro', 'transcript_id');
            ...
            $num_transcripts = $this->TranscriptsKo->find('count', [
                'conditions' => ['experiment_id' => $exp_id, 'type' => 'ko', 'name' => $ko]
            ]);
            if ($num_transcripts == 0) {
                $this->redirect(['controller' => 'trapid', 'action' => 'experiment', $exp_id]);
            }
            $ko_information = $this->KoTerms->find('first', ['conditions' => ['name' => $ko, 'type' => 'ko']]);
            ...
            if ($num_transcripts > $max_transcripts) {
                $this->set(
                    'error',
                    'Unable to find associated gene families for KO terms with more than ' .
                        $max_transcripts .
                        ' associated transcripts'
                );
                return;
            }
            $transcripts_p = $this->TranscriptsKo->find('all', [
                'fields' => ['transcript_id'],
                'conditions' => ['experiment_id' => $exp_id, 'type' => 'ko', 'name' => $ko]
            ]);
        ...
        $ipr_descriptions = [];
        $ko_descriptions = [];
        $assoc_transcripts = $this->Transcripts->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => array_keys($gene_families)],
            'fields' => ['transcript_id', 'gf_id']
        ]);
        foreach ($assoc_transcripts as $t) {
            ...
            if (!array_key_exists($gf, $first_transcripts)) {
                ...
                $trid_go = $this->TranscriptsGo->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $trid, 'type' => 'go']
                ]);
                $trid_ipr = $this->TranscriptsInterpro->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $trid, 'type' => 'ipr']
                ]);
                $trid_ko = $this->TranscriptsKo->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'transcript_id' => $trid, 'type' => 'ko']
                ]);
                $extra_annot_go[$gf] = [];
                $extra_annot_ipr[$gf] = [];
                $extra_annot_ko[$gf] = [];
        ...
        $go_descriptions = $this->ExtendedGo->find('all', [
            'conditions' => ['name' => array_unique($go_descriptions), 'type' => 'go']
        ]);
        $ipr_descriptions = $this->ProteinMotifs->find('all', [
            'conditions' => ['name' => array_unique($ipr_descriptions), 'type' => 'interpro']
        ]);
        $ko_descriptions = $this->KoTerms->find('all', [
            'conditions' => ['name' => array_unique($ko_descriptions), 'type' => 'ko']
        ]);
    ...
    function ko($exp_id = null, $ko = null) {
        ...
        $ko_info = $this->KoTerms->find('first', ['conditions' => ['name' => $ko, 'type' => 'ko']]);
        $num_transcripts = $this->TranscriptsKo->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'type' => 'ko', 'name' => $ko]
        ]);
        ...
        $transcripts_p = $this->paginate('TranscriptsKo', ['experiment_id' => $exp_id, 'type' => 'ko', 'name' => $ko]);

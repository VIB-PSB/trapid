...
class ToolsController extends AppController {
    ...
    var $name = "Tools";
    var $paginate = array("Transcripts" => array("limit" => 10, "order" => array("Transcripts.transcript_id" => "ASC")));
    var $uses = array("AnnotSources", "Annotation", "Authentication", "CompletenessResults", "Configuration", "DataSources", "ExperimentJobs", "ExperimentLog", "ExperimentStats", "Experiments", "ExtendedGo", "FullTaxonomy", "FunctionalEnrichments", "GeneFamilies", "GfData", "GoParents", "HelpTooltips", "KoTerms", "ProteinMotifs", "RnaFamilies", "Transcripts", "TranscriptsGo", "TranscriptsInterpro", "TranscriptsKo", "TranscriptsLabels", "TranscriptsTax");
...
//    if(!array_key_exists($type,$possible_types)){$this->redirect("/");}
    ...
    function go_enrichment_graph($exp_id = null, $selected_subset = null, $go_type = null, $pvalue = null) {
        ...
        if (empty($enrichment_results)) {
            $this->redirect(array("controller" => "tools", "action" => "enrichment", $exp_id));
        }
...
//     ini_set('memory_limit', '512M');  // Hack-ish?
        ...
        $this->set("col_names", array('Subset', 'Go', 'Gene family'));
...
//    $sankey_link_data = $this->FunctionalEnrichments->getSankeyLinkData($exp_id, 'go');
    ...
    function label_enrichedinterpro_gf2($exp_id = null) {
        ...
        $this->set("col_names", array('Subset', 'InterPro', 'Gene family'));
        $this->set('dropdown_names', array('Domains', 'gene families'));
        $place_holder = '###';
        $this->set("place_holder", $place_holder);
    ...
    function label_enrichedko_gf2($exp_id = null) {
        $this->general_set_up($exp_id);
        $this->set('title_for_layout', 'Sankey diagram');
        $this->set("col_names", array('Subset', 'KO', 'Gene family'));
    ...
    function label_gf_intersection($exp_id = null, $label = null) {
        ...
        $this->set("col_names", array('Subset', 'Gene family', 'Subset'));
    ...
    function label_interpro_intersection($exp_id = null, $label = null) {
        ...
        $this->set("col_names", array('Subset', 'InterPro', 'Subset'));
    ...
    function label_ko_intersection($exp_id = null, $label = null) {
        ...
        $this->set("col_names", array('Subset', 'KO', 'Subset'));
    ...
    function label_go_intersection($exp_id = null, $label = null) {
        ...
        $this->set("col_names", array('Subset', 'Go', 'Subset'));
    ...
    function tax_binning($exp_id = null) {
        ...
        $top_tax_domain = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = "domain");
        $domain_sum_transcripts = number_format(array_sum(array_map(function ($x) {
            return intval($x[1]);
        }, $top_tax_domain)));
        // Top tax phylum level
//        $top_tax_phylum = $this->read_top_tax_data($exp_id=$exp_id, $tax_rank="phylum");
        $top_tax_phylum = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = "phylum");
        $phylum_sum_transcripts = number_format(array_sum(array_map(function ($x) {
            return intval($x[1]);
        }, $top_tax_phylum)));
        // Top tax order level
        $top_tax_order = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = "order");
        $order_sum_transcripts = number_format(array_sum(array_map(function ($x) {
            return intval($x[1]);
        }, $top_tax_order)));
        // Top tax genus level
        $top_tax_genus = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = "genus");
        $genus_sum_transcripts = number_format(array_sum(array_map(function ($x) {
            return intval($x[1]);
        }, $top_tax_genus)));

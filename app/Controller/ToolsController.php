...
class ToolsController extends AppController {
    var $components = ['Cookie', 'TrapidUtils', 'Statistics'];
    var $name = 'Tools';
    var $paginate = ['Transcripts' => ['limit' => 10, 'order' => ['Transcripts.transcript_id' => 'ASC']]];
    var $uses = [
        'AnnotSources',
        'Annotation',
        'Authentication',
    ...
    function view_msa($user_identifier = null, $exp_id = null, $gf_id = null, $type = 'normal', $viewtype = 'display') {
        ...
        $this->layout = '';
        if (!$user_identifier || !$exp_id || !$gf_id) {
            return;
        }
        $user_identifer = filter_var($user_identifier, FILTER_SANITIZE_NUMBER_INT); // Unnecessary?
        parent::check_user_exp($exp_id);
        if (!parent::check_user_exp_no_cookie($user_identifier, $exp_id)) {
            return;
        }
        ...
        if (!($type == 'normal' || $type == 'stripped')) {
            $type = 'normal';
        }
        $this->set('gf_id', $gf_id);
        if ($type == 'normal') {
            $this->set('msa', $gf_info['GeneFamilies']['msa']);
        } elseif ($type == 'stripped') {
            $this->set('msa', $gf_info['GeneFamilies']['msa_stripped']);
        }
    ...
    function get_msa($exp_id = null, $gf_id = null, $type = 'normal') {
        ...
        $alignment = $this->GeneFamilies->find('first', [
            'fields' => [$alignment_field],
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $gf_id]
        ]);
    ...
    function has_double_entries($exp_id, $gf_info, $selected_species, $gf_type) {
        ...
        $transcripts = $this->Transcripts->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $gf_info['gf_id']],
        ...
        if ($gf_type == 'HOM') {
            ...
            foreach ($gene_id_data as $spec => $spec_genes) {
                if (array_key_exists($spec, $species)) {
                    $gene_ids = array_merge($gene_ids, $spec_genes);
                }
            }
        } else {
    ...
    function get_tree($exp_id = null, $gf_id = null, $format = 'newick') {
        ...
        // Retrieve data from the db and create a string. If not empty, return it.
        $tree = $this->GeneFamilies->find('first', [
            'fields' => [$tree_field],
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $gf_id]
        ]);
    ...
    function create_tree($exp_id = null, $gf_id = null) {
        ...
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'msatree_%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
    ...
    */
        ...
        if ($current_job_number >= MAX_CLUSTER_JOBS) {
            $this->redirect(['controller' => 'gene_family', 'action' => 'gene_family', $exp_id, $gf_id]);
        }

        // Get phylogenetic profile, depending on type of GF assignment (HOM/IORTHO)
        $phylo_profile = [];
        if ($exp_info['genefamily_type'] == 'HOM') {
            $gf_content = $this->GfData->find('all', [
                'conditions' => ['gf_id' => $gf_info['GeneFamilies']['plaza_gf_id']],
                'fields' => 'gene_id'
            ]);
            $phylo_profile = $this->Annotation->getSpeciesProfile(
                $this->TrapidUtils->reduceArray($gf_content, 'GfData', 'gene_id')
            );
            ...
            $phylo_profile = $this->Annotation->getSpeciesProfile(
                explode(' ', $iortho_content['GeneFamilies']['gf_content'])
            );
        }
        $this->set('phylo_profile', $phylo_profile);

        // Get number of transcripts which are partial
        $num_partial_transcripts = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $gf_id, 'meta_annotation' => 'Partial']
        ]);
        $this->set('num_partial_transcripts', $num_partial_transcripts);

        // Get all transcripts, together with their meta annotation
        $gf_transcripts = $this->Transcripts->find('all', [
            'conditions' => ['experiment_id' => $exp_id, 'gf_id' => $gf_id],
            'fields' => ['transcript_id', 'meta_annotation']
        ]);
        $gf_transcripts = $this->TrapidUtils->indexArraySimple(
            $gf_transcripts,
            'Transcripts',
            'transcript_id',
            'meta_annotation'
        ...
        $available_species_tax = $this->TrapidUtils->indexArrayMulti($available_species, 'AnnotSources', 'tax_id', [
            'species',
            'common_name'
        ]);
        $available_species_common = $this->TrapidUtils->indexArrayMulti(
            $available_species,
            'AnnotSources',
            'common_name',
            ['species', 'tax_id']
        );
        $available_species_species = $this->TrapidUtils->indexArrayMulti(
            $available_species,
            'AnnotSources',
            'species',
            ['common_name', 'tax_id']
        ...
        // List valid options
        $tree_programs = ['fasttree' => 'FastTree', 'iqtree' => 'IQ-TREE', 'phyml' => 'PhyML', 'raxml' => 'RaxML'];
        $this->set('tree_programs', $tree_programs);
        $tree_program = 'fasttree';
        $this->set('tree_program', $tree_program);

        $msa_programs = ['muscle' => 'MUSCLE', 'mafft' => 'MAFFT'];
        ...
        $previous_results = ['msa' => false, 'msa_stripped' => false, 'tree' => false];
        ...
        if ($gf_info['GeneFamilies']['msa_stripped']) {
            ...
            $this->set(
                'stripped_msa_length',
                $this->TrapidUtils->getMsaLength($gf_info['GeneFamilies']['msa_stripped'])
            );
        }
        if ($gf_info['GeneFamilies']['tree']) {
            $previous_results['tree'] = true;
        }
        ...
        if ($_POST) {
            ...
            $this->set('previous_results', ['msa' => false, 'msa_stripped' => false, 'tree' => false]);
            $selected_species = [];
            $selected_clades = [];
            $exclude_transcripts = [];
            foreach ($_POST as $k => $v) {
                ...
                } elseif (array_key_exists($t, $clades_parental)) {
                    $selected_clades[$t] = $t;
                } elseif (strlen($t) > 8 && substr($t, 0, 8) == 'exclude_') {
            ...
            // Check MSA program
            if (!array_key_exists('msa_program', $_POST)) {
                $this->set('error', 'No MSA algorithm defined');
                return;
            }
            if (array_key_exists($_POST['msa_program'], $msa_programs)) {
                $msa_program = $_POST['msa_program'];
            }
            $this->set('msa_program', $msa_program);
            ...
            if (!$msa_only) {
                // Check tree program
                if (!array_key_exists('tree_program', $_POST)) {
                    $this->set('error', 'No tree algorithm defined');
                    return;
                }
                if (array_key_exists($_POST['tree_program'], $tree_programs)) {
                    $tree_program = $_POST['tree_program'];
                }
                $this->set('tree_program', $tree_program);

                // Select editing mode for MSA
                if (!array_key_exists('editing_mode', $_POST)) {
                    $this->set('error', 'No editing mode defined');
                    return;
                }
                if (in_array($_POST['editing_mode'], $editing_modes)) {
            ...
            $contains_double_entries = $this->has_double_entries(
                $exp_id,
                $gf_info['GeneFamilies'],
                $selected_species,
            ...
            $this->GeneFamilies->updateAll(
                [
                    'used_species' => "'" . implode(',', $selected_species) . "'",
                    'exclude_transcripts' => "'" . implode(',', $exclude_transcripts) . "'"
                ],
            ...
            if (!$msa_only) {
                $shell_file = $this->TrapidUtils->create_shell_script_tree(
                    $exp_id,
                    $gf_id,
                    $msa_program,
                    $editing_mode,
            ...
            foreach ($selected_species as $ss) {
                $this->ExperimentLog->addAction($exp_id, 'create_' . $job_type, $ss, 3);
            }
    ...
    function compare_ratios_chart($exp_id = null, $type = null) {
        ...
        if ($type == 'go') {
            ...
            if ($_POST) {
                if (
                    !(
                        array_key_exists('subset1', $_POST) &&
                        array_key_exists('subset2', $_POST) &&
                        array_key_exists('go_category', $_POST) &&
                        array_key_exists('go_depth', $_POST)
                    )
                ) {
                    $this->redirect(['controller' => 'trapid', 'action' => 'experiment', $exp_id]);
                }
                ...
                if ($subset1 == $subset2) {
                    $this->set('error', 'Subset 1 should not be equal to Subset 2');
                    return;
                }
                ...
                $min_coverage = null;
                // ???
                if (array_key_exists('min_coverage', $_POST)) {
                    $min_coverage = $this->ExtendedGo->getDataSource()->value($_POST['min_coverage'], 'float');
                }
                if ($min_coverage) {
                    $this->set('min_coverage', $min_coverage);
                }
                ...
                if (array_key_exists('both_present', $_POST)) {
                    $both_subsets_present = true;
                    $this->set('both_present', true);
                }

                //select the transcripts
                $subset1_transcripts = $this->TranscriptsLabels->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'label' => $subset1],
                    'fields' => ['transcript_id']
                ]);
                $subset2_transcripts = $this->TranscriptsLabels->find('all', [
                    'conditions' => ['experiment_id' => $exp_id, 'label' => $subset2],
                    'fields' => ['transcript_id']
                ]);
                $subset1_transcripts = $this->TrapidUtils->reduceArray(
                    $subset1_transcripts,
                    'TranscriptsLabels',
                    'transcript_id'
                );
                $subset2_transcripts = $this->TrapidUtils->reduceArray(
                    $subset2_transcripts,
                    'TranscriptsLabels',
                    'transcript_id'
                );
                ...
                $go_ids = $this->ExtendedGo->find('all', [
                    'conditions' => [
                        'type' => 'go',
                        'num_sptr_steps' => $go_depth,
                        'is_obsolete' => '0',
                        'info' => $go_category
                    ],
                    'fields' => ['name', 'desc']
                ...
                if (count($go_ids) == 0) {
                    $this->set('error', 'No GO terms match with the given parameters');
                    return;
                }
                ...
                $data_sub1 = $this->TranscriptsGo->findTranscriptCounts(
                    $exp_id,
                    array_keys($go_ids),
                    $subset1_transcripts
                );
                $data_sub2 = $this->TranscriptsGo->findTranscriptCounts(
                    $exp_id,
                    array_keys($go_ids),
                    $subset2_transcripts
                );
                ...
                $selected_gos = [];
                $result = ['vars' => [], 'smps' => ['All', $subset1, $subset2], 'desc' => ['Ratios'], 'data' => []];
                $counter = 0;
                foreach ($data_all as $k => $v) {
                    if (array_key_exists($k, $data_sub1) || array_key_exists($k, $data_sub2)) {
                        ...
                        if ($both_subsets_present && ($sub1_count == 0 || $sub2_count == 0)) {
                        }
                        //do nothing, not present in both
                        else {
                            if ($min_coverage && ($sub1_ratio < $min_coverage || $sub2_ratio < $min_coverage)) {
                            }
                            //do nothing, min ratio not reached
                            else {
                                $result['vars'][] = $k . ' ' . $go_ids[$k];
                                $result['data'][] = [$all_ratio, $sub1_ratio, $sub2_ratio];
                                $counter++;
                            }
                        }
                    }
    ...
    function compare_ratios($exp_id = null, $type = null) {
        ...
        if ($_POST) {
            ...
            $subset2 = filter_var($_POST['subset2'], FILTER_SANITIZE_STRING);
            if ($subset1 == $subset2) {
                $this->set('error', 'subset 1 must be different from subset 2');
                return;
            }
            if (!(array_key_exists($subset1, $subsets) && array_key_exists($subset2, $subsets))) {
                $this->redirect(['controller' => 'tools', 'action' => 'go_ratios', $exp_id]);
            }
            $this->set('subset1', $subset1);
            $this->set('subset2', $subset2);
            $subset1_transcripts = $this->TranscriptsLabels->find('all', [
            ...
            if ($type == 'go') {
                ...
                $go_descriptions = $this->ExtendedGo->find('all', [
                    'conditions' => ['name' => $go_ids, 'type' => 'go']
                ]);
                ...
                $subset1_ipr_counts = $this->TranscriptsInterpro->findInterproCountsFromTranscripts(
                    $exp_id,
                    $subset1_transcripts
                );
                $subset2_ipr_counts = $this->TranscriptsInterpro->findInterproCountsFromTranscripts(
                    $exp_id,
                    $subset2_transcripts
                );
                ...
                $ipr_descriptions = $this->ProteinMotifs->find('all', [
                    'conditions' => ['name' => $ipr_ids, 'type' => 'interpro']
                ]);
                //	$ipr_descriptions	= $this->TrapidUtils->indexArraySimple($ipr_descriptions,"ProteinMotifs","motif_id","desc");
                $ipr_descriptions = $this->TrapidUtils->indexArraySimple(
                    $ipr_descriptions,
                    'ProteinMotifs',
                    'name',
                    'desc'
    ...
    function download_compare_ratios(
        ...
        $comparison_types = ['1' => 'both', '2' => 'first', '3' => 'last'];
        ...
        if ($type == 'go') {
            ...
            $go_ids = array_unique(array_merge(array_keys($subset1_go_counts), array_keys($subset2_go_counts)));
            //	$go_descriptions	= $this->ExtendedGo->find("all",array("conditions"=>array("go"=>$go_ids)));
            $go_descriptions = $this->ExtendedGo->find('all', ['conditions' => ['name' => $go_ids, 'type' => 'go']]);
            //	$go_types		= $this->TrapidUtils->indexArraySimple($go_descriptions,"ExtendedGo","go","type");
            $go_types = $this->TrapidUtils->indexArraySimple($go_descriptions, 'ExtendedGo', 'name', 'info');
            //	$go_descriptions	= $this->TrapidUtils->indexArraySimple($go_descriptions,"ExtendedGo","go","desc");
            $go_descriptions = $this->TrapidUtils->indexArraySimple($go_descriptions, 'ExtendedGo', 'name', 'desc');
    ...
    function enrichment($exp_id = null) {
        ...
        // Get tooltip content
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'enrichment_%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
        ...
        $subsets = $this->TranscriptsLabels->getLabels($exp_id);
        if (count($subsets) == 0) {
            $this->set('error', 'No subsets defined');
            return;
        }
        $this->set('subsets', $subsets);

        //possible p-values
        $possible_pvalues = ['0.05', '0.01', '0.005', '0.001', '0.0001', '0.00001'];
        ...
        if ($_POST) {
            ...
            if (!array_key_exists('annotation_type', $_POST)) {
                $this->set('error', 'no functional annotation type indicated in form');
                return;
            }
            if (!in_array($_POST['annotation_type'], array_keys($possible_types))) {
                $this->set('error', 'invalid functional annotation type');
                return;
            } else {
            ...
            if (array_key_exists('pvalue', $_POST)) {
                ...
                if (in_array($pvalue, $possible_pvalues)) {
                    $selected_pvalue = $pvalue;
                }
                $this->set('selected_pvalue', $selected_pvalue);
            }
            // Check if previous results exist in the DB
            $enrichment_results = $this->FunctionalEnrichments->find('all', [
                'conditions' => [
            ...
            if (empty($enrichment_results) || !array_key_exists('use_cache', $_POST)) {
                ...
                $shell_file = $this->TrapidUtils->create_shell_file_enrichment(
                    $exp_id,
                    $ini_file,
                    $type,
                    $subset,
                ...
                $cluster_job = $this->TrapidUtils->getClusterJobId($output);
                if ($cluster_job == null) {
                    $this->set('error', 'problem with retrieving job identifier from web cluster');
                    return;
                }
                $this->set('job_id', $cluster_job);
    ...
    // Note 2022-11-21: the graph is not used anymore ... Should this be deleted?
    function go_enrichment_graph($exp_id = null, $selected_subset = null, $go_type = null, $pvalue = null) {
        ...
        if (!$selected_subset || !array_key_exists($selected_subset, $subsets)) {
            $this->redirect(['controller' => 'tools', 'action' => 'go_enrichment', $exp_id]);
        }
        ...
        $enrichment_results = $this->FunctionalEnrichments->find('all', [
            'conditions' => [
                'experiment_id' => $exp_id,
                'label' => $selected_subset,
                'max_p_value' => $pvalue,
                'data_type' => 'go'
            ]
        ]);
        if (empty($enrichment_results)) {
            $this->redirect(['controller' => 'tools', 'action' => 'enrichment', $exp_id]);
        }
        ...
        foreach ($enrichment_results as $r) {
            ...
            if ($res['log_enrichment'] > 0) {
                $result[$res['identifier']] = [
                    ...
                    'p_value' => $res['p_value'],
                    'ratio' => $res['log_enrichment'],
                    'perc' => $res['subset_ratio']
                ];
            }
        }
        ...
        $go_data = $this->ExtendedGo->find('all', ['conditions' => ['name' => array_keys($result), 'type' => 'go']]);
        ...
        foreach ($result as $go_id => $g) {
            if ($go_types[$go_id] == $go_type) {
                ...
                if (!array_key_exists($sptr, $sptr_array)) {
                    $sptr_array[$sptr] = [];
                }
    ...
    function download_enrichment($exp_id = null, $type = null, $selected_subset = null, $pvalue = null) {
        ...
        $db_results = $this->FunctionalEnrichments->find('all', [
            'fields' => ['identifier', 'label', 'p_value', 'log_enrichment', 'subset_ratio', 'is_hidden'],
            'conditions' => [
                'experiment_id' => $exp_id,
                'label' => $selected_subset,
                'max_p_value' => $pvalue,
            ...
            if ($res['log_enrichment'] > 0) {
                if ($type == 'go') {
                    $result[$res['identifier']] = [
                        ...
                        'p-value' => $res['p_value'],
                        'enrichment' => $res['log_enrichment'],
                        'subset_ratio' => $res['subset_ratio']
                    ];
                } elseif ($type == 'ipr') {
                    $result[$res['identifier']] = [
                        'ipr' => $res['identifier'],
        ...
        if ($type == 'go') {
            $go_data = $this->ExtendedGo->find('all', [
                'conditions' => ['name' => array_keys($result), 'type' => 'go']
            ]);
            ...
            $ipr_data = $this->ProteinMotifs->find('all', [
                'conditions' => ['name' => array_keys($result), 'type' => 'interpro']
            ]);
            ...
            $ko_data = $this->KoTerms->find('all', ['conditions' => ['name' => array_keys($result), 'type' => 'ko']]);
    ...
    function load_enrichment($exp_id = null, $type = null, $subset_title = null, $pvalue = null, $job_id = null) {
        ...
        $possible_types = ['go' => 'GO', 'ipr' => 'Protein domain', 'ko' => 'KO'];
        //check type
        if (!array_key_exists($type, $possible_types)) {
            $this->redirect('/');
        }
        ...
        if ($type == 'go') {
            ...
            $go_graph_data_all = ['headers' => ['ontology' => 'go'], 'nodes' => []];
            //        $go_graph_data = array("headers"=>array("ontology"=>"go"), "nodes"=>array());
            $n_trs_subset = $this->TranscriptsLabels->find('count', [
                'conditions' => ['experiment_id' => $exp_id, 'label' => $subset_title]
            ]);
...
//            $go_graph_data["nodes"][$go_id] = $node_data;
            ...
            // Get tooltip content
            $tooltips = $this->TrapidUtils->indexArraySimple(
                $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'go_enrichment_graph%'"]]),
                'HelpTooltips',
                'tooltip_id',
                'tooltip_text'
            );
            ...
            $ipr_data = $this->ProteinMotifs->find('all', ['conditions' => ['name' => array_keys($result)]]);
            $ipr_descriptions = $this->TrapidUtils->indexArray($ipr_data, 'ProteinMotifs', 'name', 'desc');
            $ipr_types = $this->TrapidUtils->indexArray($ipr_data, 'ProteinMotifs', 'name', 'info');
            $this->set('ipr_descriptions', $ipr_descriptions);
            $this->set('ipr_types', $ipr_types);
        } elseif ($type == 'ko') {
            $ko_data = $this->KoTerms->find('all', ['conditions' => ['name' => array_keys($result)]]);
            $ko_descriptions = $this->TrapidUtils->indexArray($ko_data, 'KoTerms', 'name', 'desc');
            $this->set('ko_descriptions', $ko_descriptions);
        }
    }
    ...
    function length_distribution($exp_id = null) {
        ...
        // Allowed chart types
        $possible_graphtypes = ['grouped' => '', 'stacked' => 'normal'];
        ...
        if ($this->request->is('post')) {
            ...
            if (
                array_key_exists('num_bins', $_POST) &&
                $_POST['num_bins'] >= $range_bins[0] &&
                $_POST['num_bins'] <= $range_bins[1]
            ) {
            ...
            $graph_type = 'grouped';
            if (
                array_key_exists('graph_type', $_POST) &&
                array_key_exists($_POST['graph_type'], $possible_graphtypes)
            ) {
            ...
            if (array_key_exists('sequence_type', $_POST) && $_POST['sequence_type'] == 'transcript') {
                ...
                if ($show_partials) {
                    ...
                    $json_transcript = $this->Statistics->update_json_data(
                        'Transcripts (Partial)',
                        $partial_lengths,
                        $json_transcript,
                        $bins_transcript
            ...
            elseif (array_key_exists('sequence_type', $_POST) && $_POST['sequence_type'] == 'orf') {
                ...
                if (array_key_exists('reference_species', $_POST)) {
                    ...
                    if ($this->AnnotSources->find('first', ['conditions' => ['species' => $rs]])) {
                ...
                if ($selected_ref_species != '') {
                    ...
                    $ref_species_data = $this->AnnotSources->find('first', ['conditions' => ['species' => $rs]]);
                    $ref_species_name = $ref_species_data['AnnotSources']['common_name'];
                    $json_orf = $this->Statistics->update_json_data(
                        $ref_species_name,
                        $ref_species_lengths,
                        $json_orf,
                        $bins_orf,
        ...
        $reference_species = $this->AnnotSources->find('all', ['order' => '`species` ASC']);
        $reference_species = $this->TrapidUtils->indexArraySimple(
            $reference_species,
            'AnnotSources',
            'species',
            'common_name'
        ...
        // Get tooltip content
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'seq_len_%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
    ...
    function statistics($exp_id = null) {
        ...
        $num_transcripts = $this->Transcripts->find('count', ['conditions' => ['experiment_id' => $exp_id]]);
        $num_orfs = $this->Transcripts->getOrfCount($exp_id);
        $num_start_codons = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'orf_contains_start_codon' => 1]
        ]);
        $num_stop_codons = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'orf_contains_stop_codon' => 1]
        ]);
        $num_putative_fs = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'putative_frameshift' => 1]
        ]);
        $num_correct_fs = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'is_frame_corrected' => 1]
        ]);
        $meta_annot_fulllength = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'meta_annotation' => 'Full Length']
        ]);
        $meta_annot_quasi = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'meta_annotation' => 'Quasi Full Length']
        ]);
        $meta_annot_partial = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'meta_annotation' => 'Partial']
        ]);
        $meta_annot_noinfo = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'meta_annotation' => 'No Information']
        ]);
        ...
        $num_gf = $this->GeneFamilies->find('count', ['conditions' => ['experiment_id' => $exp_id]]);
        $num_transcript_gf = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'not' => ['gf_id' => null]]
        ]);
        $biggest_gf = $this->GeneFamilies->find('first', [
            'conditions' => ['experiment_id' => $exp_id],
            'order' => ['num_transcripts DESC']
        ]);
        $biggest_gf_res = [
            'gf_id' => $biggest_gf['GeneFamilies']['gf_id'],
            'num_transcripts' => $biggest_gf['GeneFamilies']['num_transcripts']
        ];
        $single_copy = $this->GeneFamilies->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'num_transcripts' => 1]
        ]);
        ...
        $num_rf = $this->RnaFamilies->find('count', ['conditions' => ['experiment_id' => $exp_id]]);
        $num_transcript_rf = $this->Transcripts->find('count', [
            'conditions' => ['experiment_id' => $exp_id, 'not' => ['rf_ids' => null]]
        ]);
        $biggest_rf = $this->RnaFamilies->find('first', [
            'conditions' => ['experiment_id' => $exp_id],
            'order' => ['num_transcripts DESC']
        ]);
        if (!$biggest_rf) {
            $biggest_rf_res = ['rf_id' => 'N/A', 'num_transcripts' => 0];
        } else {
            $biggest_rf_res = [
                'rf_id' => $biggest_rf['RnaFamilies']['rf_id'],
                'num_transcripts' => $biggest_rf['RnaFamilies']['num_transcripts']
            ];
        }
        ...
        if ($exp_info['perform_tax_binning'] == 1) {
            ...
            $num_unclassified_trs = $this->TranscriptsTax->find('count', [
                'conditions' => ['experiment_id' => $exp_id, 'txid' => '0']
            ]);
            // Number of classified transcripts
            $num_classified_trs = $this->TranscriptsTax->find('count', [
                'conditions' => ['experiment_id' => $exp_id, 'not' => ['txid' => '0']]
            ]);
            ...
            $tax_classification_data[] = [
                '#Classified',
                $num_classified_trs . percent_str($num_classified_trs, $num_transcripts)
            ];
            $tax_classification_data[] = [
                '#Unclassified',
                $num_unclassified_trs . percent_str($num_unclassified_trs, $num_transcripts)
            ];
            foreach ($top_tax_domain as $top_tax) {
                if ($top_tax[0] != 'Unclassified') {
                    ...
                    $tax_classification_data[] = [
                        '#' . $top_tax[0],
                        $top_tax_trs . percent_str($top_tax_trs, $num_transcripts)
                    ];
                }
            }
        }
        ...
        if (in_array('go', $exp_info['function_types'])) {
            $funct_annot_data[] = [
                '#Transcripts with GO',
                $go_stats['num_transcript_go'] . percent_str($go_stats['num_transcript_go'], $num_transcripts)
            ];
        }
        if (in_array('interpro', $exp_info['function_types'])) {
            $funct_annot_data[] = [
                '#Transcripts with Protein Domain',
                $interpro_stats['num_transcript_interpro'] .
                percent_str($interpro_stats['num_transcript_interpro'], $num_transcripts)
            ];
        }
        if (in_array('ko', $exp_info['function_types'])) {
            $funct_annot_data[] = [
                '#Transcripts with KO',
                $ko_stats['num_transcript_ko'] . percent_str($ko_stats['num_transcript_ko'], $num_transcripts)
            ];
        }
            ...
            [
                ...
                'data' => [
                    ...
                    [
                        ...
                        $num_putative_fs . percent_str($num_putative_fs, $num_transcripts)
                    ]
                ]
            ],
            [
    ...
    function GOSankey($exp_id = null, $go_web) {
        ...
        $urls = [
            urldecode(Router::url(['controller' => 'functional_annotation', 'action' => 'go', $exp_id, $place_holder])),
            urldecode(Router::url(['controller' => 'gene_family', 'action' => 'gene_family', $exp_id, $place_holder]))
        ];
        $rows = $this->Transcripts->getOneGOToGFMapping($exp_id, $go);

        $this->set('mapping', json_encode($rows));
        $this->set('descriptions', json_encode($this->ExtendedGo->retrieveGoInformation([$go])));
    ...
    function interproSankey($exp_id = null, $interpro = null) {
        ...
        $urls = [
            urldecode(
                Router::url(['controller' => 'functional_annotation', 'action' => 'interpro', $exp_id, $place_holder])
            ),
            urldecode(Router::url(['controller' => 'gene_family', 'action' => 'gene_family', $exp_id, $place_holder]))
        ];
        ...
        $this->set('mapping', json_encode($rows));
        $this->set('descriptions', json_encode($this->ProteinMotifs->retrieveInterproInformation([$interpro])));
        $this->set('urls', json_encode($urls));
    ...
    function KOSankey($exp_id = null, $ko = null) {
        ...
        // Need to use `urldecode()` otherwise `#` gets encoded.
        $urls = [
            urldecode(Router::url(['controller' => 'functional_annotation', 'action' => 'ko', $exp_id, $place_holder])),
            urldecode(Router::url(['controller' => 'gene_family', 'action' => 'gene_family', $exp_id, $place_holder]))
        ];
        $rows = $this->Transcripts->getOneKOToGFMapping($exp_id, $ko);

        $this->set('mapping', json_encode($rows));
        $this->set('descriptions', json_encode($this->KoTerms->retrieveKoInformation([$ko]))); // Why not `find()` if only one term?
        $this->set('urls', json_encode($urls));
    ...
    function label_enrichedgo_gf2($exp_id = null) {
        ...
        $this->set('col_names', ['Subset', 'Go', 'Gene family']);
        $this->set('dropdown_names', ['Domains', 'Gene families']);
        $place_holder = '###';
        $this->set('place_holder', $place_holder);
        ...
        $gf_prefix = $exp_id . '_';
        $urls = [
            urldecode(Router::url(['controller' => 'labels', 'action' => 'view', $exp_id, $place_holder])),
            urldecode(Router::url(['controller' => 'functional_annotation', 'action' => 'go', $exp_id, $place_holder])),
            urldecode(
                Router::url([
    ...
    function label_enrichedinterpro_gf2($exp_id = null) {
        ...
        $this->set('col_names', ['Subset', 'InterPro', 'Gene family']);
        $this->set('dropdown_names', ['Domains', 'gene families']);
        $place_holder = '###';
        $this->set('place_holder', $place_holder);
    ...
    function label_enrichedko_gf2($exp_id = null) {
        $this->general_set_up($exp_id);
        $this->set('title_for_layout', 'Sankey diagram');
        $this->set('col_names', ['Subset', 'KO', 'Gene family']);
        $this->set('dropdown_names', ['KOs', 'gene families']);
        $place_holder = '###';
        $this->set('place_holder', $place_holder);
    ...
    function label_gf_intersection($exp_id = null, $label = null) {
        ...
        $this->set('col_names', ['Subset', 'Gene family', 'Subset']);
    ...
    function label_interpro_intersection($exp_id = null, $label = null) {
        ...
        $this->set('col_names', ['Subset', 'InterPro', 'Subset']);
    ...
    function label_ko_intersection($exp_id = null, $label = null) {
        ...
        $this->set('col_names', ['Subset', 'KO', 'Subset']);
    ...
    function label_go_intersection($exp_id = null, $label = null) {
        ...
        $this->set('col_names', ['Subset', 'Go', 'Subset']);
    ...
    function tax_binning($exp_id = null) {
        ...
        $top_tax_domain = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = 'domain');
        $domain_sum_transcripts = number_format(
            array_sum(
        ...
        $top_tax_phylum = $this->read_top_tax_data($exp_id = $exp_id, $tax_rank = 'phylum');
        $phylum_sum_transcripts = number_format(
            array_sum(
                array_map(function ($x) {
                    return intval($x[1]);
                }, $top_tax_phylum)
        ...
        $order_sum_transcripts = number_format(
            array_sum(
                array_map(function ($x) {
                    return intval($x[1]);
                }, $top_tax_order)
        ...
        $genus_sum_transcripts = number_format(
            array_sum(
                array_map(function ($x) {
                    return intval($x[1]);
                }, $top_tax_genus)
        ...
        $this->set('display_krona_url', Router::url(['controller' => 'tools', 'action' => 'display_krona', $exp_id]));
        $this->set(
            'treeview_json_url',
            Router::url(['controller' => 'tools', 'action' => 'get_treeview_json', $exp_id])
        );
        $this->set('top_tax_domain', $top_tax_domain);
        ...
        if ($this->request->is('post')) {
            ...
            foreach ($tax_names as $tn) {
                if ($tn == $unclassified_str) {
                    ...
                    $lineage = $this->FullTaxonomy->find('first', [
                        'fields' => ['tax'],
                        'conditions' => ['scname' => $tn]
                    ]);
            ...
            if (sizeof($transcripts) > 0) {
                ...
                $counter = $this->TranscriptsLabels->enterTranscriptsByChunks(
                    $exp_id,
                    $transcripts,
                    $subset_name,
                    20000,
    ...
    function core_gf_completeness($exp_id = null) {
        ...
        // Get tooltip content
        $tooltips = $this->TrapidUtils->indexArraySimple(
            $this->HelpTooltips->find('all', ['conditions' => ["tooltip_id LIKE 'core_gf_%'"]]),
            'HelpTooltips',
            'tooltip_id',
            'tooltip_text'
        );
        ...
        $previous_completeness_jobs = $this->CompletenessResults->find('all', [
            'conditions' => ['experiment_id' => $exp_id],
            'fields' => ['id', 'clade_txid', 'used_method', 'label', 'completeness_score']
        ]);
        ...
        foreach ($previous_completeness_jobs as &$completeness_job) {
            ...
            $tax_name = $this->FullTaxonomy->find('first', [
                'fields' => ['scname'],
                'conditions' => ['txid' => $tax_id]
            ]);
        ...
        $core_gf_clades = [];
        $core_gf_clade_str = $this->Configuration->find('first', [
            'conditions' => [
                'method' => 'completeness_parameters',
                'key' => $exp_info['used_plaza_database'],
                'attr' => 'clade_list'
            ],
            'fields' => ['value']
        ]);
        ...
        if ($this->request->is('post')) {
            ...
            $clade_db = $this->FullTaxonomy->find('first', ['conditions' => ['txid' => $clade_tax_id]]);
            ...
            if (empty($clade_db)) {
                ...
                $this->autoRender = false;
                return "<p class='text-danger'><strong>Error:</strong> Invalid clade selected (" .
                    $clade_tax_id .
                    ').</p>';
            }
            if ($current_job_number >= MAX_CLUSTER_JOBS) {
            ...
            // More checks to come ...
            $previous_completeness_job = $this->CompletenessResults->find('first', [
                'conditions' => [
                    'experiment_id' => $exp_id,
                    'clade_txid' => $clade_tax_id,
            ...
            $this->ExperimentLog->addAction(
                $exp_id,
                'core_gf_completeness_options',
                'conservation_threshold=' . $species_perc,
                2
            );
            ...
            $this->redirect([
                'controller' => 'tools',
                'action' => 'handle_core_gf_completeness',
                $exp_id,
                $cluster_job,
                $clade_tax_id,
                $transcript_label,
    ...
    function handle_core_gf_completeness(
        $exp_id,
        $cluster_job_id,
        $clade_tax_id,
        $label,
    ...
    function load_core_gf_completeness($exp_id, $clade_tax_id, $label, $tax_source, $species_perc, $top_hits) {
        ...
        $tax_name = $this->FullTaxonomy->find('first', [
            'fields' => ['scname'],
            'conditions' => ['txid' => $clade_tax_id]
        ]);
        $tax_name = $tax_name['FullTaxonomy']['scname'];
        // Retrieve data from database
        $completeness_job = $this->CompletenessResults->find('first', [
            'conditions' => [
                'experiment_id' => $exp_id,
                'clade_txid' => $clade_tax_id,
                'label' => $label,
        ...
        $n_missing = strlen($completeness_job['CompletenessResults']['missing_gfs'])
            ? sizeof(explode(';', $completeness_job['CompletenessResults']['missing_gfs']))
            : 0;
        $n_represented = strlen($completeness_job['CompletenessResults']['represented_gfs'])
            ? sizeof(explode(';', $completeness_job['CompletenessResults']['represented_gfs']))
            : 0;
        $n_total = $n_missing + $n_represented;
        $missing_gfs_array = [];
        $represented_gfs_array = [];
        if ($n_missing > 0) {
            foreach (explode(';', $completeness_job['CompletenessResults']['missing_gfs']) as $missing_gf_str) {
                $record = explode(':', $missing_gf_str);
                array_push($missing_gfs_array, [
                    'gf_id' => $record[0],
                    'n_genes' => $record[1],
                    'n_species' => $record[2],
                    'gf_weight' => $record[3]
        ...
        if ($n_represented > 0) {
            foreach (explode(';', $completeness_job['CompletenessResults']['represented_gfs']) as $represented_gf_str) {
                ...
                array_push($represented_gfs_array, [
                    'gf_id' => $record[0],
                    'n_genes' => $record[1],
                    'n_species' => $record[2],
                    'gf_weight' => $record[3],
    ...
    // Delete core GF completeness results
    function delete_core_gf_results(
        $exp_id = null,
        $clade_tax_id = null,
        $label = null,
        $tax_source = null,
        ...
        $this->redirect(['controller' => 'tools', 'action' => 'core_gf_completeness', $exp_id]);
    }
    ...
    function search_tax($clade_prefix) {
        ...
        $clades = $this->FullTaxonomy->find('all', [
            'fields' => ['scname', 'txid'],
            'conditions' => ['scname LIKE' => $clade_prefix . '%'],
            'limit' => $limit_results
        ]);
        $clades_json = [];

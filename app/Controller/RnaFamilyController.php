...
class RnaFamilyController extends AppController {
    ...
    function index($exp_id = null) {
        ...
        $rf_names = $this->Configuration->find("all", array(
            "conditions" => array("method" => "rfam_families", "key" => $rna_families_ids_original, "attr" => "rfam_id"),
            "fields" => array("key", "value")
        ));
        $clan_names = $this->Configuration->find("all", array(
            "conditions" => array("method" => "rfam_clans", "key" => $rna_clans_ids_original, "attr" => "clan_id"),
            "fields" => array("key", "value")
        ));
        ...
        $user_group = $this->Authentication->find("first", array(
            "fields" => array("group"),
            "conditions" => array("user_id" => parent::check_user())
        ));
    ...
    function rna_family($exp_id = null, $rf_id = null) {
        ...
        // Get Rfam name and description from values stored in `configuration`
        $rf_name = $this->Configuration->find("first", array(
            "conditions" => array("method" => "rfam_families", "key" => $rf_data['RnaFamilies']['rfam_rf_id'], "attr" => "rfam_id"),
            "fields" => array("value")
        ));
        $rf_data['RnaFamilies']['name'] = $rf_name['Configuration']['value'];
        $rf_desc = $this->Configuration->find("first", array(
            "conditions" => array("method" => "rfam_families", "key" => $rf_data['RnaFamilies']['rfam_rf_id'], "attr" => "description"),
            "fields" => array("value")
        ));
        ...
        if ($rf_data['RnaFamilies']['rfam_clan_id']) {
            $clan_name = $this->Configuration->find("first", array(
                "conditions" => array("method" => "rfam_clans", "key" => $rf_data['RnaFamilies']['rfam_clan_id'], "attr" => "clan_id"),
                "fields" => array("value")
            ));
            $rf_data['RnaFamilies']['clan_name'] = $clan_name['Configuration']['value'];
            $clan_desc = $this->Configuration->find("first", array(
                "conditions" => array("method" => "rfam_clans", "key" => $rf_data['RnaFamilies']['rfam_clan_id'], "attr" => "description"),
                "fields" => array("value")
            ));
        ...
        // GO
        $transcripts_go = $this->TrapidUtils->indexArray($this->TranscriptsGo->find("all", array(
        ...
        $transcripts_ipr = $this->TrapidUtils->indexArray($this->TranscriptsInterpro->find("all", array(
        ...
        // KO
        $transcripts_ko = $this->TrapidUtils->indexArray($this->TranscriptsKo->find("all", array(
        ...
        $transcripts_labels = $this->TrapidUtils->indexArray($this->TranscriptsLabels->find("all", array(
            "conditions" => array("experiment_id" => $exp_id, "transcript_id" => $transcript_ids))),

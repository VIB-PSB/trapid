...
class DocumentationController extends AppController {
    ...
    var $uses = ['Configuration', 'DataSources'];
    ...
    function general() {
        ...
        $export_data = $this->Configuration->find('all', [
            'conditions' => ['method' => 'doc_export_files', 'attr' => 'file_content'],
            'fields' => ['key', 'value']
        ]);
        $export_examples = [];
    ...
    function tools_parameters() {
        ...
        $ref_db_data_raw = $this->DataSources->find('all', ['conditions' => ['restrict_to' => null]]);
        $ref_db_data = [];
        foreach ($ref_db_data_raw as $ref_db) {
            $ref_db_data[$ref_db['DataSources']['db_name']] = [
                'url' => $ref_db['DataSources']['URL'],
                'name' => $ref_db['DataSources']['name']
            ];
        }
        // Get tools' data

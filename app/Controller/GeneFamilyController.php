...
class GeneFamilyController extends AppController {
    ...
    var $name = "GeneFamily";
    var $paginate = array(
        "Transcripts" => array("limit" => 10, "order" => array("Transcripts.transcript_id" => "ASC")),
        // Sorting by `experiment_id` is needed to force use of `experiment_id` index
        "GeneFamilies" => array("maxLimit" => 20,
            "order" => array("GeneFamilies.experiment_id" => "ASC", "GeneFamilies.gf_id" => "ASC")
        )
    );
    var $uses = array("AnnotSources", "Annotation", "Authentication", "Configuration", "DataSources", "ExperimentJobs",
        "Experiments", "ExtendedGo", "GeneFamilies", "GfData", "GoParents", "HelpTooltips", "KoTerms", "ProteinMotifs",
        "Transcripts", "TranscriptsGo", "TranscriptsInterpro", "TranscriptsKo", "TranscriptsLabels");
    ...
    function expansion($exp_id = null) {
        ...
        if ($_POST) {
            ...
            if ($exp_info["genefamily_type"] == "HOM") {
                ...
                $reference_genes = $this->Annotation->find("all", array(
                    "conditions" => array("species" => $selected_species), "fields" => array("gene_id")
                ));
    ...
    function index($exp_id = null) {
        ...
        $user_group = $this->Authentication->find("first", array(
            "fields" => array("group"), "conditions" => array("user_id" => parent::check_user())
        ));
    ...
    function functional_annotation($exp_id = null, $gf_id = null) {
        ...
        $transcripts = $this->Transcripts->find("all", array(
            "conditions" => array("experiment_id" => $exp_id, "gf_id" => $gf_id),
        ...
        $go_data = $this->TranscriptsGo->find("all", array(
            "conditions" => array("experiment_id" => $exp_id, "transcript_id" => $transcripts, "type" => "go")
        ));
        $interpro_data = $this->TranscriptsInterpro->find("all", array(
            "conditions" => array("experiment_id" => $exp_id, "transcript_id" => $transcripts, "type" => "ipr")
        ));
        $ko_data = $this->TranscriptsKo->find("all", array(
            "conditions" => array("experiment_id" => $exp_id, "transcript_id" => $transcripts, "type" => "ko")
        ));
    ...
    function getGfContent($exp_id, $gf_id, $plaza_gf_id, $gf_type) {
        ...
        if ($gf_type == "HOM") {
            ...
            $t = $this->GeneFamilies->find("first", array(
                "fields" => array("gf_content"),
                "conditions" => array("experiment_id" => $exp_id, "gf_id" => $gf_id)
            ));
    ...
    function multifasta($exp_id = null, $gf_id = null) {
        ...
        $gf_content_seqs = $this->Annotation->find("all", array(
            "conditions" => array("gene_id" => $gf_content), "fields" => array("gene_id", "seq"))
        );
    ...
    function gene_family($exp_id = null, $gf_id = null) {
        ...
        $transcript_count = $this->Transcripts->find("count", array(
            "conditions" => array("experiment_id" => $exp_id, "gf_id" => $gf_id)
        ));
        ...
        $transcripts_go = $this->TrapidUtils->indexArray(
            $this->TranscriptsGo->find("all", array(
                "conditions" => array("experiment_id" => $exp_id, "transcript_id" => $transcript_ids, "is_hidden" => "0", "type" => "go")
    ...
    function normalizeGfInfo($exp_id, $gf_id, $new_count = null) {
        ...
        if ($new_count == 0) {
            ...
            $this->GeneFamilies->updateAll(
                array("num_transcripts" => "'" . $new_count . "'"),
                array("experiment_id" => $exp_id, "gf_id" => $gf_id)
            );
            return;
        }
    }

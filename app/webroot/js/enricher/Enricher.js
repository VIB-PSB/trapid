...
function EnricherGo(attachPoint, json, options) {
    ...
    // options.animationEnabled = options.animationEnabled || true;
    options.animationEnabled	= ('animationEnabled' in options)? options.animationEnabled : true;
	...
	//console.log("level-separation : "+options.levelSeparation+", edge-separation : "+options.edgeSeparation+", node-separation : "+options.nodeSeparation);
	...
	options.donutGraphOuterWidth = options.donutGraphOuterWidth || function(n){
		return 20;
	};
    //options.donutGraphInnerWidth = options.donutGraphInnerWidth || 10;
	options.donutGraphInnerWidth = options.donutGraphInnerWidth || function(n){
		return 10;
	}
    ...
    // options.sidebarMiniMapShow = options.sidebarMiniMapShow || true;	 // Always true , even if sidebarMiniMapShow set to false
    options.sidebarMiniMapShow	= ('sidebarMiniMapShow' in options)? options.sidebarMiniMapShow : true;
	...
	options.ontologyDescriptionsTextSize = ((typeof options.ontologyDescriptionsTextSize)=="undefined")?1.5:options.ontologyDescriptionsTextSize;	//because size 0 is allowed, and would not work with the '||' operator
	options.ontologyTermsTextSize = ((typeof options.ontologyTermsTextSize)=="undefined")?1.5:options.ontologyTermsTextSize;	//because size 0 is allowed, and would not work with the '||' operator

    options.subsetName = options.subsetName || "unknown_subset";
    ...
    function copyStylesInline(destinationNode, callback) {
        ...
        d3.xhr("../../css/enricher.css", function (data) {
    ...
    var downloadBaseName = 'go_graph_' + options.subsetName.toLowerCase();
    if(options.namespace) {
        downloadBaseName += "_" + options.namespace;
    }
    ...
    if (options.export.png) {
        document.getElementById(options.export.png).addEventListener('click', function() {
            downloadPNG(downloadBaseName + '.png');
        });
    }
    if (options.export.svg) {
        document.getElementById(options.export.svg).addEventListener('click', function handleDownloadSVG() {
            downloadSVG(downloadBaseName + '.svg');
        });
    }
    if (options.export.json) {
        document.getElementById(options.export.json).addEventListener('click', function handleDownloadJSON() {
            downloadJSON(downloadBaseName + '.json');
        });
    }
}

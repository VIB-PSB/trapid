function DirectedAcyclicGraph(attachPoint, options) {
    ...
    var nodeid = function (d) { return d.id.replace(":", "-"); }
    ...
    var drawnode = function (d) {
        ...
        if (d.details.is_root == 1) {
            ...
            var fill = d3.svg.arc()
                .innerRadius(0)
                .outerRadius(Math.sqrt(2) * size / 2 - options.donutGraphOuterWidth / 2)
                .startAngle(0 * (Math.PI / 180))
                .endAngle(360 * (Math.PI / 180));
            var donutValue = d3.svg.arc()
                .innerRadius(Math.sqrt(2) * size / 2 - options.donutGraphOuterWidth)
                .outerRadius(Math.sqrt(2) * size / 2)
                .startAngle(0 * (Math.PI / 180))
                .endAngle(360 * (Math.PI / 180) * options.donutGraphValue(d.details));
            var donutFill = d3.svg.arc()
                .innerRadius(Math.sqrt(2) * size / 2 - (options.donutGraphOuterWidth + options.donutGraphInnerWidth) / 2)
                .outerRadius(Math.sqrt(2) * size / 2 - (options.donutGraphOuterWidth - options.donutGraphInnerWidth) / 2)
        ...
        if (options.linkUrl) {
            text.append("a")
                .attr("href", options.linkUrl + nodeid(d))
                .attr("target", options.linkTarget)
                .append("tspan")
                .attr("x", 0).attr("dy", "1em").text(nodeid);
        } else {
        ...
        for (token in nameTokens) {
            text.append("tspan").attr("x", 0).attr("dy", "1.1em").text(nameTokens[token]);
        }

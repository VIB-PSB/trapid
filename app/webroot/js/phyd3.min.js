...
$jscomp.iteratorFromArray=function(d,b){$jscomp.initSymbolIterator();d instanceof String&&(d+="");var a=0,f={next:function(){if(a<d.length){var h=a++;return{value:b(h,d[h]),done:!1}}f.next=function(){return{done:!0,value:void 0}};return f.next()}};f[Symbol.iterator]=function(){return f};return f};$jscomp.polyfill("Array.prototype.values",function(d){return d?d:function(){return $jscomp.iteratorFromArray(this,function(b,a){return a})}},"es6","es3");
(function(){"undefined"==typeof phyd3&&(phyd3={});phyd3.newick={};phyd3.newick.parse=function(d){var b=[],a={};d=d.split(/\s*(;|\(|\)|,|:)\s*/);for(var f=0,h=0;h<d.length;h++){var v=d[h];switch(v){case "(":v={};a.id=f;f++;a.branchset=[v];b.push(a);a=v;break;case ",":v={};b[b.length-1].branchset.push(v);a=v;break;case ")":a=b.pop();break;case ":":break;default:var t=d[h-1];")"==t||"("==t||","==t?a.name=v:":"==t&&(a.branchLength=parseFloat(v));a.id=f;f++}}return a}})();
...
end:b[2]});!d.children&&b[2][0]>phyd3.phylogram.dx&&(phyd3.phylogram.dx=b[2][0]);return"M"+b[0]+" "+b[1]+" "+b[2]};phyd3.phylogram.scaleBranchLengths=function(d,b){function a(b,d){d(b);if(b.children)for(var f=b.children.length-1;0<=f;f--)a(b.children[f],d)}a(d[0],function(a){0>a.branchLength&&(a.branchLength*=-1);a.rootDist=(a.parent?a.parent.rootDist:0)+(a.branchLength||0)});var f=d.map(function(a){return a.rootDist}),h=d3.scale.linear().domain([0,d3.max(f)]).range([0,b]);a(d[0],function(a){a.y=
parseInt(h(a.rootDist))});return h};phyd3.phylogram.randomColor=function(){function d(a,b,d){0>d&&(d+=1);1<d&&--d;return d<1/6?a+6*(b-a)*d:.5>d?b:d<2/3?a+(b-a)*(2/3-d)*6:a}var b=Math.random();return function(a,b,h){if(0==b)h=b=a=h;else{var f=.5>h?h*(1+b):h+b-h*b,t=2*h-f;h=d(t,f,a+1/3);b=d(t,f,a);a=d(t,f,a-1/3)}return"#"+Math.round(255*h).toString(16)+Math.round(255*b).toString(16)+Math.round(255*a).toString(16)}((b+.618033988749895)%1,.5,.6)};phyd3.phylogram.scaledomainWidths=function(d,b){d=d.map(function(a){return a.sequences&&
a.sequences[0]&&a.sequences[0].domainArchitecture&&a.sequences[0].domainArchitecture.sequenceLength?a.sequences[0].domainArchitecture.sequenceLength:0});return d3.scale.linear().domain([0,d3.max(d)]).range([0,b])};phyd3.phylogram.build=function(d,b,a){function f(){n.selectAll("text.groupLabel").remove();n.selectAll("rect.groupLabel").remove();var e=0;for(gid in b.groups){var m=b.groups[gid];m.x1=Number.MAX_SAFE_INTEGER;m.x2=0;m.y1=Number.MAX_SAFE_INTEGER;m.y2=0;e<m.depth&&(e=m.depth);v(b,m)}for(;0<=
e;e--)for(gid in b.groups)m=b.groups[gid],m.depth==e&&(n.insert("text",":first-child").attr("class","groupLabel").attr("y",m.x1).attr("x",0).attr("dy",2*a.nodeHeight).attr("text-anchor","start").attr("stroke",m.foregroundColor).attr("stroke-width",a.outline+"px").attr("font-size",1.5*a.nodeHeight+"px").text(m.label),n.insert("rect",":first-child").attr("class","groupLabel").attr("stroke",m.backgroundColor).attr("fill",m.backgroundColor).attr("y",m.x1).attr("height",m.x2-m.x1).attr("x",0).attr("width",
phyd3.phylogram.dx).append("title").text(m.label))}function h(a,m,g){if(g||a.id==m){g=!0;a.groupIDs||(a.groupIDs=[]);a.groupIDs.push(m);var e=b.groups[m];a.x<e.x1&&(e.x1=a.x);a.x>e.x2&&(e.x2=a.x);a.y<e.y1&&(e.y1=a.y);a.y>e.y2&&(e.y2=a.y)}if(a.children)for(e=0;e<a.children.length;e++)h(a.children[e],m,g)}function v(a,b){if(a.groupIDs)for(var e=0;e<a.groupIDs.length;e++)a.groupIDs[e]==b.id&&(a.id==b.id&&(b.depth=a.depth),a.x<b.x1&&(b.x1=a.x),a.x>b.x2&&(b.x2=a.x),a.y<b.y1&&(b.y1=a.y),a.y>b.y2&&(b.y2=
a.y));if(a.children)for(e=0;e<a.children.length;e++)v(a.children[e],b)}function t(a,b){if(a.groupIDs){var e=a.groupIDs.indexOf(b);-1!=e&&a.groupIDs.splice(e,1)}if(a.children)for(e=0;e<a.children.length;e++)t(a.children[e],b)}function z(){a.domainWidth=a.domainWidthStep;a.graphWidth=a.initialGraphWidth;a.scaleX=1;a.scaleY=1;a.translateX=0;a.translateY=0;ea.translate([0,0])}function B(){d3.select(d).select("svg").remove();svg=d3.select(d).insert("svg");svg.attr("width",aa+"px").attr("height",a.height+
"px").attr("overflow","hidden").attr("version","1.1").attr("font-family","Open Sans").attr("xmlns","http://www.w3.org/2000/svg").call(ea);svg.append("svg:rect").attr("class","canvas").attr("width","100%").attr("height","100%").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);svg.append("defs").append("marker").attr("id","markerBoxplot").attr("markerWidth","1").attr("markerHeight","10").attr("refX","1").attr("refY","5").append("line").attr("x1","0").attr("x2","0").attr("y1","0").attr("y2",
"10").attr("stroke",A()).attr("stroke-width","2px");n=svg.append("svg:g",":first-child").attr("id","main").attr("transform","translate("+a.marginX+", "+a.marginY+")");svg.append("svg:text").attr("dy","20px").attr("class","title").attr("stroke",A()).attr("stroke-width","1px").attr("font-size","15px").text(b.name?b.name:"");svg.append("svg:text").attr("class","title").attr("dy","40px").attr("stroke",A()).attr("stroke-width","0.5px").attr("font-size","10px").text(b.description?b.description:"");T=n.selectAll("g.node");
O();0==phyd3.phylogram.dx&&(d3.select("#phylogram").attr("checked","").attr("disabled","disabled"),a.showPhylogram=!0,O());C=n.selectAll("g.leaf.node");I=C.selectAll("g.domain");pa();fa();qa();ha();ia();ra();sa();T.append("svg:text").attr("class","name").attr("dx",a.nodeHeight+1).attr("dy",3).attr("text-anchor","start");Y();U();L();ba();F()}function ta(){n.selectAll("rect.searchResults").remove();var a=jQuery("#searchQuery").val(),d=new RegExp(a,"i");a&&a.length&&ua(b,d)}function A(){return a.invertColors?
a.backgroundColor:a.foregroundColor}function ja(){d3.selectAll(".canvas").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);var e=A();n.selectAll("path.link").attr("stroke",e);d3.selectAll("text").attr("fill",e);d3.selectAll("marker").selectAll("line").attr("stroke",e);d3.selectAll("text.title").attr("stroke",e);n.selectAll("text.nodelabel").attr("stroke",e).attr("fill",e);C.selectAll("path").attr("stroke",e);C.selectAll("line").attr("stroke",e);C.selectAll("rect").attr("stroke",e);
Y();ka()}function va(){var e=d.replace("#","");e=document.getElementById(e).querySelector("svg");var b=e.getElementById("main"),g=b.getBBox();b=b.getAttributeNode("transform");var f=e.getAttributeNode("width"),p=e.getAttributeNode("height"),l=b.value,k=f.value,x=p.value;b.value="translate("+a.marginX+", "+a.marginY+")";f.value=(Math.max(+f.value.replace("px",""),g.width)+a.marginX).toString();p.value=(Math.max(+p.value.replace("px",""),g.height)+a.marginY).toString();if("undefined"!=typeof window.XMLSerializer)var q=
(new XMLSerializer).serializeToString(e);else"undefined"!=typeof e.xml&&(q=e.xml);b.value=l;f.value=k;p.value=x;return q}function pa(){a.showDomains?(P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth),C.append("svg:path").attr("class","domain").attr("stroke",A()).attr("visibility",function(a,b){return a.show?"visibile":"hidden"}),I=I.data(function(a,b,g){a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.domains:[];for(g=0;g<a.length;g++)a[g].i=b;
a.sort(function(a,e){return Math.abs(e.to-e.from)-Math.abs(a.to-a.from)});return a}).enter().append("svg:g").attr("class","domain"),I.append("svg:rect").attr("class","domain hover-visible").attr("stroke",A()).attr("stroke-width",a.outline+"px").append("title").text(function(a){return a.name}),ca(),M(),ka()):(I.remove(),C.selectAll("path.domain").remove(),I=C.selectAll("g.domain"))}function fa(){a.showDomainNames?(I.append("svg:text").attr("class","domain").attr("dy",-3).attr("fill",A()).attr("font-size",
1.5*a.nodeHeight+"px").text(function(a){return a.name}),M()):I.selectAll("text.domain").remove()}function ca(){var e=C.data();C.selectAll("g.domain").attr("visibility",function(b,g,d){return e[d].show&&b.confidence<=a.domainLevel?"visible":"hidden"});C.selectAll("path.domain").attr("visibility",function(a){return a.show?"visible":"hidden"});d3.select("#domainLevel").attr("value",a.domainLevel.toPrecision(1))}function ka(){I.selectAll("rect.domain").attr("fill",function(e){return a.showDomainColors&&
b.domcolors[e.name]?b.domcolors[e.name].color.replace(/0x/,"#"):A()})}function M(){var e=C.data(),b=Q+(a.showLabels?R:0)+(a.showGraphs?w:0);P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);C.selectAll("path.domain").attr("d",function(a,d,m){d=phyd3.phylogram.dx-e[m].y;a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.sequenceLength:0;return"M"+parseInt(b+d)+",0L"+parseInt(b+d+P(a))+",0"});I.selectAll("rect.domain").attr("width",function(a){return parseInt(P(a.to-
a.from))}).attr("transform",function(g,d,m){return"translate("+parseInt(b+(phyd3.phylogram.dx-e[g.i].y)+P(g.from))+","+parseInt(-1*a.nodeHeight)+")"}).attr("height",2*a.nodeHeight-2);I.selectAll("text.domain").attr("transform",function(g){return"translate("+parseInt(b+(phyd3.phylogram.dx-e[g.i].y)+P(g.from))+","+parseInt(-1*a.nodeHeight)+")"});d3.select("#domainWidth").attr("value",a.domainWidth)}function ha(){n.selectAll("text.supportValue").remove();a.showSupportValues&&(n.selectAll("g.inner.node").append("svg:text").attr("class",
"supportValue").attr("dx",-2).attr("text-anchor","end").attr("fill",a.supportValuesColor).text(function(e){var b="";if(e.confidences)for(var g=0;g<e.confidences.length;g++)b+=parseFloat(e.confidences[g].value).toFixed(a.maxDecimalsSupportValues)+" ";return b}),d3.select("#maxDecimalsSupportValues").attr("value",a.maxDecimalsSupportValues),F(),L())}function ia(){n.selectAll("text.branchLength").remove();a.showLengthValues&&(n.selectAll("g.node").append("svg:text").attr("class","branchLength").attr("dx",
-2).attr("text-anchor","end").attr("fill",a.branchLengthColor).text(function(e){return e.branchLength?parseFloat(e.branchLength).toFixed(a.maxDecimalsLengthValues):""}),d3.select("#maxDecimalsLengthValues").attr("value",a.maxDecimalsLengthValues),F(),L())}function la(a,b,g){var e=a.attr("id")+"_"+a.selectAll("div.panel-default").size(),d=a.append("div").attr("class","panel panel-default");d.append("div").attr("class","panel-heading").attr("role","tab").attr("id","heading"+e).append("h4").attr("class",
"panel-title").append("a").attr("class","collapsed").attr("aria-expanded",g?"true":"false").attr("role","button").attr("data-toggle","collapse").attr("data-parent","#"+a.attr("id")).attr("href","#collapse"+e).attr("aria-controls","collapse"+e).html(b);return d.append("div").attr("id","collapse"+e).attr("class","panel-collapse collapse "+(g?"in":"")).attr("role","tabpanel").attr("aria-labelledby","heading"+e).append("div").attr("class","panel-body table-responsive")}function Ba(e){var m=d3.event,g=
m.layerX;y=m.layerY;var f="popup"+parseInt(Date.now()*Math.random()*1E3),p=function(){d3.select("#"+f).remove();n.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","0")};g=d3.select(d).append("div").attr("class","popup").attr("id",f).style("position","absolute").style("top",parseInt(y)+"px").style("left",parseInt(g)+"px").style("width",parseInt(a.popupWidth)+"px").style("color","black").style("background-color","white");var l=g.append("button").attr("class","btn btn-link").style("position",
"absolute").style("right","8px");l.append("span").attr("class","glyphicon glyphicon-remove").attr("aria-hidden","true").style("top","0px");l.on("click",p);g.append("span").append("a").text(function(){return-1==a.pinnedNodes.indexOf(e.id)?"Pin this node":"Unpin this node"}).on("click",function(){var b=a.pinnedNodes.indexOf(e.id);-1==b?a.pinnedNodes.push(e.id):a.pinnedNodes.splice(b,1);p();Y()});g=g.append("div").attr("class","panel-group").attr("id","accordion_"+parseInt(Date.now()*Math.random()*
1E3)).attr("role","tablist").attr("aria-multiselectable","true");l="text"==m.target.nodeName||m.target.classList&&m.target.classList.contains("pointer")?!0:!1;l=la(g,"Node",l).append("table").attr("class","table table-condensed table-bordered").append("tbody");if(e.name){var k=l.append("tr");k.append("td").text("Name");k.append("td").text(e.name)}if(e.taxonomies)for(var x=0;x<e.taxonomies.length;x++){var q=e.taxonomies[x];k=l.append("tr");k.append("td").text("Taxononomy");k=k.append("td");b.taxcolors[q.code]&&
b.taxcolors[q.code].url&&(k=k.append("a").attr("href",b.taxcolors[q.code].url).attr("target","_blank"));var h="";h+=b.taxcolors[q.code]&&b.taxcolors[q.code].name?b.taxcolors[q.code].name+" ":"";h+=" ";h+=q.scientificName?"\x3ci\x3e"+q.scientificName+"\x3c/i\x3e":"";h+=" ";h+=q.commonName?q.commonName:"";h+=" ";h+=q.code?"("+q.code+")":"";k.html(h)}if(e.confidences)for(x=0;x<e.confidences.length;x++)k=l.append("tr"),k.append("td").text("Confidence"),k.append("td").html(e.confidences[x].value);k=l.append("tr");
k.append("td").text("Depth");k.append("td").text(e.depth);k=l.append("tr");k.append("td").text("Branch length");k.append("td").text(e.branchLength?e.branchLength:0);k=l.append("tr");k.append("td").text("Distance from root");k.append("td").text(e.rootDist);for(x=0;x<e.properties.length;x++)k=e.properties[x],u=k.ref,q=k.value,q=q.trim(),/^http:\/\//.test(q)&&(q="\x3ca href\x3d'"+q+"' target\x3d'_blank'\x3e"+q+"\x3c/a\x3e\x3cbr /\x3e"),k=l.append("tr"),k.append("td").text(u),k.append("td").html(q);for(var u in e.events)q=
e.events[u],k=l.append("tr"),k.append("td").text(u),k.append("td").html(q);if(e.sequences)for(x=0;x<e.sequences.length;x++){u=e.sequences[x];q="";q+=u.symbol?u.symbol+" ":"";q+=u.name?u.name+" ":"";q+=u.location?u.location+" ":"";q+=u.geneName?u.geneName:"";q.length&&(k=l.append("tr"),k.append("td").text("Sequence"),k.append("td").html(q));u.accession&&(q="",q+=u.accession.value?u.accession.value+" ":"",q+=u.accession.source?"["+u.accession.source+"] ":"",q+=u.accession.comment?"("+u.accession.comment+
")":"",q.length&&(k=l.append("tr"),k.append("td").text("Accession"),k.append("td").html(q)));u.molSeq&&u.molSeq.value&&(k=l.append("tr"),k.append("td").text("Molecular Sequence"),k.append("td").html(u.molSeq.value));if(u.uris){q="";for(k=0;k<u.uris.length;k++)q=u.uris[k],q="\x3ca href\x3d'"+q.value+"'\x3e"+(q.desc?q.desc:q.value)+"\x3c/a\x3e";q.length&&(k=l.append("tr"),k.append("td").text("See also"),k.append("td").html(q))}if(e.sequences[x].domainArchitecture&&e.sequences[x].domainArchitecture.domains){l=
m.target.parentNode.classList&&m.target.parentNode.classList.contains("domain")?!0:!1;l=la(g,"Domains",l).append("table").attr("id","dt"+f).attr("class","table table-condensed table-bordered");u=l.append("thead").append("tr");u.append("th").text("#");u.append("th").text("Name");u.append("th").text("Description");u.append("th").text("From");u.append("th").text("To");u.append("th").text("P value");h=l.append("tbody");for(q=0;q<e.sequences[x].domainArchitecture.domains.length;q++){var G=e.sequences[x].domainArchitecture.domains[q];
k=h.append("tr");k.append("td").style("background",b.domcolors[G.name]&&b.domcolors[G.name].color?b.domcolors[G.name].color.replace("0x","#"):"").text(" ");u=k.append("td");b.domcolors[G.name]&&b.domcolors[G.name].url&&(u=u.append("a").attr("href",b.domcolors[G.name].url).attr("target","_blank"));u.text(G.name);k.append("td").text(b.domcolors[G.name]?b.domcolors[G.name].description:"");k.append("td").text(G.from);k.append("td").text(G.to);k.append("td").text(G.confidence)}jQuery("#dt"+f).dataTable({searching:!1,
paging:!1,language:{info:""},order:[[3,"asc"]],columns:[{orderable:!1},null,null,null,null,null]})}}if(b.graphs)for(x=0;x<b.graphs.length;x++)if(h=b.graphs[x],h.data&&h.data[e.id]&&h.data[e.id].length)for(l=m.target.parentNode.classList&&m.target.parentNode.classList.contains(h.type)?!0:!1,l=la(g,h.name,l).append("table").attr("class","table table-condensed table-bordered").append("tbody"),q=0;q<h.legend.fields.length;q++)k=l.append("tr"),u=k.append("td"),h.legend.fields[q].url&&(u=u.append("a").attr("href",
h.legend.fields[q].url).attr("target","_blank")),u.text(h.legend.fields[q].name),k.append("td").text(h.data[e.id][q]);n.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","1");m.stopPropagation()}function ra(){a.lineupNodes?n.selectAll("g.leaf").append("path").attr("class","support").attr("stroke",A()).attr("stroke-dasharray","2,3").attr("stroke-width","0.5px"):n.selectAll("path.support").remove()}function Z(e,d){var g="";if(a.showTaxonomy&&e.taxonomies)for(var m=0;m<e.taxonomies.length;m++){var f=
e.taxonomies[m];a.showFullTaxonomy&&(g+=b.taxcolors[f.code]&&b.taxcolors[f.code].name?b.taxcolors[f.code].name:"",g+=" ",g+=f.scientificName?f.scientificName:"",g+=" ",g+=f.commonName?f.commonName:"",g+=" ");g+=f.code?f.code:"";g+=" "}m="";if(a.showSequences&&e.sequences)for(f=0;f<e.sequences.length;f++){var l=e.sequences[f];m+=l.symbol?l.symbol:"";m+=" ";m+=l.accession?l.accession.value:"";m+=" ";m+=l.name?l.name:"";m+=" ";m+=l.geneName?l.geneName:"";m+=" ";if(l.annotations)for(var k=0;k<l.annotations.length;k++)text+=
l.annotations[k].desc?l.annotations[k].desc:"",text+=" "}f=a.showNodeNames&&e.name?e.name+" ":" ";d||(e.children&&"only leaf"==a.showNodesType&&(f=" "),e.children||"only inner"!=a.showNodesType||(f=" "));return g+f+m}function U(){var e=0;ma=0;C.selectAll("text.name").text(function(a){var b=Z(a);b.length>e&&(e=b.length,ma=a.id);return b});n.selectAll("g.node.inner").selectAll("text.name").text(Z);a.showGraphs&&V();a.showDomains&&M()}function L(){n.selectAll("text.branchLength").attr("visibility",function(a){return a.show?
"visible":"hidden"});n.selectAll("text.supportValues").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("text.name").attr("visibility",function(a){return a.show?"visible":"hidden"});a.lineupNodes&&C.selectAll("path.support").attr("visibility",function(a){return a.show?"visible":"hidden"});a.showLabels&&wa();a.showGraphs&&xa();a.showDomains&&ca()}function Y(){n.selectAll("text.name").attr("stroke",A()).attr("stroke-width",function(b){return-1!=a.pinnedNodes.indexOf(b.id)?
2*a.outline+"px":a.outline+"px"}).attr("fill",function(e){var d=null;if(a.showTaxonomyColors&&b.taxcolors&&e.taxonomies)for(var g=0;g<e.taxonomies.length;g++){var f=e.taxonomies[g];b.taxcolors[f.code]&&b.taxcolors[f.code].color&&(d=b.taxcolors[f.code].color.replace(/0x/,"#"))}return d?d:A()})}function ya(){C.selectAll("text.name").attr("dx",function(b){return parseInt(a.lineupNodes?phyd3.phylogram.dx-b.y+5:5)});C.selectAll("path.support").attr("d",function(a){return"M0,0 L"+parseInt(phyd3.phylogram.dx-
a.y)+",0"});a.showLabels&&na();a.showGraphs&&V();a.showDomains&&M()}function F(){var b=0;n.selectAll("text.name").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");n.selectAll("text.branchLength").attr("dy",-a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");n.selectAll("text.supportValue").attr("dy",a.nodeHeight/2+5).attr("font-size",1.5*a.nodeHeight+"px");n.selectAll("g.leaf.cid_"+ma).selectAll("text.name").each(function(){var a=this.getBBox();a.width>b&&(b=a.width)});b+=10;
b<a.textLength&&(b=a.textLength);Q=b;d3.select("#nodeHeight").attr("value",2*a.nodeHeight+"px");ya();a.showLabels&&na();a.showGraphs&&V();a.showDomains&&M()}function sa(){if(a.showLabels&&b.labels){for(var e=0;e<b.labels.length;e++){var d=b.labels[e];d.id||(d.id=parseInt(Date.now()*Math.random()*1E3));d.data.tag&&n.selectAll("g.node").each(function(a){var b=a[d.data.tag],e=d.data.ref;e=e?e:"value";b="object"==typeof b?b[e]:b;if("property"==d.data.tag)for(pid=0;pid<a.properties.length;pid++){var g=
a.properties[pid];if(g.ref==e){b=g.value;break}}b&&(d.data[a.id]=b)});switch(d.type){case "text":for(cid in d.data)d.data[cid]&&n.selectAll(".cid_"+cid).append("text").attr("class","nodelabel text lid"+d.id).attr("dy",3).attr("text-anchor","start").attr("stroke-width",a.outline+"px").attr("stroke",A()).attr("fill",A()).attr("font-size",1.5*a.nodeHeight+"px").text(d.data[cid]).append("title").text(d.name+": "+d.data[cid]);break;case "color":for(cid in d.data)if(d.data[cid]){var g=a.nodeHeight,f=d.data[cid].replace(/0x/,
"#");n.selectAll(".cid_"+cid).append("rect").attr("height",2*g).attr("width",2*g).attr("class","nodelabel color lid"+d.id).attr("style","fill: "+f+"; stroke: "+f).append("title").text(d.name)}}n.append("text").attr("class","labellegend lid"+d.id).attr("fill",A()).text(0!=d.showLegend?d.name:"")}wa();na()}else n.selectAll(".nodelabel").remove(),n.selectAll(".labellegend").remove(),R=0;V();M()}function na(){var e=parseInt(a.nodeHeight);R=0;if(b.labels){for(var d=0;d<b.labels.length;d++){var g=b.labels[d],
f=0;n.selectAll(".nodelabel.lid"+g.id).attr("transform",function(a){var b=0,d=0;"color"==g.type&&(b=d=-e);var m=0;"text"==this.nodeName&&this.firstChild?m=this.firstChild.length*e/1.5:"rect"==this.nodeName&&(m=2*e);m>f&&(f=m);a.branchset.length?(b+=a.y,b-=e):b+=phyd3.phylogram.dx+Q;return"translate("+parseInt(b+e+R-a.y)+","+d+")"});n.selectAll("text.labellegend.lid"+g.id).attr("transform","translate("+parseInt(phyd3.phylogram.dx+R+Q+2*e)+",-10) rotate(-90)").attr("font-size",2*e+"px");R+=f+2*e}C.selectAll("text.nodelabel").attr("dy",
e/2).attr("font-size",1.5*e+"px");C.selectAll("rect.nodelabel").attr("width",2*e+"px").attr("height",2*e+"px")}}function wa(){n.selectAll(".nodelabel").attr("visibility",function(a){return a.show?"visible":"hidden"})}function qa(){if(a.showGraphs&&b.graphs&&b.graphs.length){d3.svg.arc().innerRadius(0).outerRadius(a.nodeHeight);for(var e=d3.layout.pie().value(function(a){return a.value}).sort(null),d=w=0;d<b.graphs.length;d++){var g=b.graphs[d];g.id||(g.id=parseInt(Date.now()*Math.random()*1E3));if(g.data.tag)var f=
n.selectAll("g.node").each(function(a){var b=a[g.data.tag],e=g.data.ref;e=e?e:"value";b="object"==typeof b?b[e]:b;if("property"==g.data.tag)for(var d=0;d<a.properties.length;d++){var f=a.properties[d];if(f.ref==e){b=f.value;break}}b&&(g.data[a.id]=[],g.data[a.id][0]=b,"pie"==g.type&&g.data.max&&(g.data[a.id][1]=g.data.max-b,g.legend.fields[1]={name:g.legend.fields[0].name,color:g.legend.fields[0].background?g.legend.fields[0].background:phyd3.phylogram.randomColor(),invert:!0}))});switch(g.type){case "pie":for(cid in g.data)if(f=
cid.replace(/\./g,"\\."),g.data[cid]&&Array.isArray(g.data[cid])){var p=!1;n.select(".cid_"+f).each(function(a){0==a.branchset.length&&(p=!0)});f=n.selectAll(".cid_"+f).append("svg:g").attr("class","graph pie gid"+g.id);for(var l=[],k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k],leaf:p});l=f.selectAll("path.pie.gid"+g.id).data(e(l)).enter().append("path").attr("class","pie hover-visible gid"+g.id).attr("stroke",A()).attr("stroke-width",a.outline+"px").attr("fill",
function(a,b){return g.legend.fields?(g.legend.fields[b]||(g.legend.fields[b]={},g.legend.fields[b].name="Series "+(b+1),g.legend.fields[b].color=phyd3.phylogram.randomColor()),g.legend.fields[b].color.replace(/0x/,"#")):""}).append("title").text(function(a,b){return g.legend.fields?(g.legend.fields[b]&&g.legend.fields[b].name?g.legend.fields[b].name+": ":"")+(g.legend.fields[b].invert?g.data.max-a.data.value:a.data.value):""})}break;case "binary":for(cid in g.data)if(g.data[cid]&&Array.isArray(g.data[cid])){f=
cid.replace(/\./g,"\\.");l=[];p=!1;n.select(".cid_"+f).each(function(a){0==a.branchset.length&&(p=!0)});f=n.selectAll(".cid_"+cid).append("svg:g").attr("class","graph binary gid"+g.id);for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k],shape:g.legend.fields[k]?g.legend.fields[k].shape:"",leaf:p});l=f.selectAll("path.binary.gid"+g.id).data(l).enter().append("path").attr("class","binary hover-visible gid"+g.id).attr("style",function(a,b){b=(g.legend.fields[b]?
g.legend.fields[b].color:"").replace(/0x/,"#");return"fill:"+(1<=a.value?b:"none")+";stroke:"+(0<=a.value?b:"none")}).append("title").text(function(a,b){return(g.legend.fields[b]?g.legend.fields[b].name+": ":"")+a.value})}break;case "multibar":D[g.id]=[];for(k=0;k<g.legend.fields.length;k++)l=d3.max(d3.values(g.data),function(a){if(a)return a[k]}),f=d3.min(d3.values(g.data),function(a){if(a)return a[k]}),0<f&&(f=0),D[g.id][k]=d3.scale.linear().domain([f,l]);for(cid in g.data)if(g.data[cid]){f=cid.replace(/\./g,
"\\.");f=n.select(".cid_"+f).append("svg:g").attr("class","graph multibar gid"+g.id);l=[];for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k]});l=f.selectAll("rect.multibar.gid"+g.id).data(l).enter().append("rect").attr("class","multibar hover-visible gid"+g.id).attr("fill",function(a,b){return(g.legend.fields[b]?g.legend.fields[b].color:"").replace(/0x/,"#")}).attr("stroke",A()).attr("stroke-width",a.outline).append("title").text(function(a,b){return(g.legend.fields[b]?
g.legend.fields[b].name+": ":"")+a.value})}break;case "boxplot":E[g.id]=null;var h=0,q=Number.MAX_SAFE_INTEGER;for(k=0;5>k;k++)l=d3.max(d3.values(g.data),function(a){if(a)return a[k]}),f=d3.min(d3.values(g.data),function(a){if(a)return a[k]}),l>h&&(h=l),f<q&&(q=f);console.log(q,h);E[g.id]=d3.scale.linear().domain([q,h]);for(cid in g.data)g.data[cid]&&(f=cid.replace(/\./g,"\\."),f=n.select(".cid_"+f).append("svg:g").attr("class","graph boxplot gid"+g.id),l=[],l.push({min:g.data[cid][0],q1:g.data[cid][1],
median:g.data[cid][2],q3:g.data[cid][3],max:g.data[cid][4]}),l=f.selectAll("rect.boxplot.gid"+g.id).data(l).enter(),l.append("line").attr("class","boxplot baseline hover-visible gid"+g.id).attr("style","marker-start: url(#markerBoxplot); marker-end: url(#markerBoxplot);").attr("stroke",A()),l.append("rect").attr("class","boxplot q1 hover-visible gid"+g.id).attr("fill",(g.legend.fields[0]?g.legend.fields[0].color:"").replace(/0x/,"#")).attr("stroke",A()).attr("stroke-width",a.outline).append("title").text(function(a){return"Min: "+
a.min+", Q1: "+a.q1+", Median: "+a.median}),l.append("rect").attr("class","boxplot q3 hover-visible gid"+g.id).attr("fill",(g.legend.fields[1]?g.legend.fields[1].color:"").replace(/0x/,"#")).attr("stroke",A()).attr("stroke-width",a.outline).append("title").text(function(a){return"Median: "+a.median+", Q3: "+a.q3+", Max: "+a.max}),l.append("line").attr("class","boxplot median hover-visible gid"+g.id).attr("stroke",A()).attr("stroke-width","2px"));break;case "heatmap":l=d3.max(d3.values(g.data),function(a){if(a)return d3.max(a)});
f=d3.min(d3.values(g.data),function(a){if(a)return d3.min(a)});var t=d3.scale.quantize().domain([f,l]).range(colorbrewer[g.legend.gradient.name][g.legend.gradient.classes]);for(cid in g.data)if(g.data[cid]){f=cid.replace(/\./g,"\\.");f=n.select(".cid_"+f).append("svg:g").attr("class","graph heatmap gid"+g.id);l=[];if(g.data[cid])for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k]});l=f.selectAll("rect.heatmap.gid"+g.id).data(l).enter().append("rect").attr("class",
"heatmap hover-visible gid"+g.id).attr("fill",function(b,e){return isNaN(b.value)?a.nanColor:t(b.value)}).attr("stroke",function(b,e){return isNaN(b.value)?a.nanColor:A()}).attr("stroke-width",a.outline+"px").append("title").text(function(a,b){return(g.legend.fields[b]?g.legend.fields[b].name+": ":"")+a.value})}break;default:console.log("Graph not supported "+g.type)}}xa();V()}else w=0,n.selectAll("g.graph").remove(),n.selectAll("text.legend").remove();M()}function V(){var e=a.nodeHeight,d=Q+R;C.selectAll("g.graph").attr("transform",
function(a){return"translate("+(phyd3.phylogram.dx-a.y+d)+",0)"});var g=n.selectAll("g.graph");d3.select("#graphWidth").attr("value",a.graphWidth);n.selectAll("text.legend").remove();w=0;if(b.graphs){for(var f=0;f<b.graphs.length;f++){var p=b.graphs[f];switch(p.type){case "pie":g.selectAll("path.pie.gid"+p.id).attr("d",function(a){if(0<a.value)return d3.svg.arc().innerRadius(0).outerRadius(e)(a)}).attr("transform",function(a){return" translate("+parseInt(a.data.leaf?e+w:0)+",0)"});a.showGraphs&&a.showGraphLegend&&
n.append("text").attr("class","legend").text(0!=p.legend.show?p.name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+d+w+2*e)+",-10) rotate(-90)");w+=2*e+5;break;case "binary":g.selectAll("path.binary.gid"+p.id).attr("d",function(a){var b=d3.svg.symbol().size(4*(e-2)*(e-2));b.type(p.legend.fields[a.i].shape);return b(a)}).attr("transform",function(a){return" translate("+(a.leaf?w+10+a.i*(2*e+5):0)+" ,0)"});if(a.showGraphs&&a.showGraphLegend)for(var l=0;l<p.legend.fields.length;l++)n.append("text").attr("class",
"legend").text(0!=p.legend.show?p.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(2*e+5)+2*e)+", -10) rotate(-90)");w+=10+p.legend.fields.length*(2*e+5);break;case "multibar":for(l=0;l<p.legend.fields.length;l++)D[p.id][l]=D[p.id][l].range([0,a.graphWidth]);g.selectAll("rect.multibar.gid"+p.id).attr("height",p.legend.stacked?parseInt(2*e/p.legend.fields.length):parseInt(2*e)).attr("width",function(a,b){b=parseInt(D[p.id]&&D[p.id][a.i]?D[p.id][a.i](a.value):0);a=
parseInt(D[p.id]&&D[p.id][a.i]?D[p.id][a.i](0):0);return b>a?b-a:a-b}).attr("transform",function(b,d){d=w;var g=-1*e;p.legend.stacked?g+=2*e/p.legend.fields.length*b.i:d+=b.i*(a.graphWidth+5);var f=parseInt(D[p.id]&&D[p.id][b.i]?D[p.id][b.i](b.value):0);b=parseInt(D[p.id]&&D[p.id][b.i]?D[p.id][b.i](0):0);return" translate("+parseInt(d+(b<f?b:f))+", "+parseInt(g)+")"});if(a.showGraphs&&a.showGraphLegend)if(p.legend.stacked)n.append("text").attr("class","legend").text(0!=p.legend.show?p.name:"").attr("transform",
"translate("+(phyd3.phylogram.dx+d+w+a.graphWidth/2)+",-10) rotate(-90)");else for(l=0;l<p.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=p.legend.show?p.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(a.graphWidth+5)+a.graphWidth/2)+",-10) rotate(-90)");w+=(p.legend.stacked?1:p.legend.fields.length)*(a.graphWidth+5);break;case "boxplot":E[p.id]=E[p.id].range([0,a.graphWidth]);g.selectAll("rect.q1.boxplot.gid"+p.id).attr("height",parseInt(2*
e)).attr("width",function(a){return parseInt(E[p.id](a.median)-E[p.id](a.q1))}).attr("transform",function(a,b){a=w+E[p.id](a.q1);return" translate("+parseInt(a)+",-"+parseInt(e)+")"});g.selectAll("rect.q3.boxplot.gid"+p.id).attr("height",parseInt(2*e)).attr("width",function(a){return parseInt(E[p.id](a.q3)-E[p.id](a.median))}).attr("transform",function(a,b){a=w+E[p.id](a.median);return" translate("+parseInt(a)+",-"+parseInt(e)+")"});g.selectAll("line.baseline.boxplot.gid"+p.id).attr("y1",parseInt(e)).attr("y2",
parseInt(e)).attr("x1",function(a){var b=E[p.id](a.min);0>b&&console.log(a.min,b);return E[p.id](a.min)}).attr("x2",function(a){return E[p.id](a.max)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(e)+")"});g.selectAll("line.median.boxplot.gid"+p.id).attr("y1",0).attr("y2",parseInt(2*e)).attr("x1",function(a){return E[p.id](a.median)}).attr("x2",function(a){return E[p.id](a.median)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(e)+")"});
a.showGraphs&&a.showGraphLegend&&n.append("text").attr("class","legend").text(0!=p.legend.show?p.name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+2*e)+", -10) rotate(-90)");w+=a.graphWidth+5;break;case "heatmap":g.selectAll("rect.heatmap.gid"+p.id).attr("width",2*e).attr("height",2*e).attr("transform",function(a,b){return" translate("+(w+2*a.i*e)+",-"+e+")"});if(a.showGraphs&&a.showGraphLegend)for(l=0;l<p.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=p.legend.show?
p.legend.fields[l].name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+d+w+(l+1)*e*2)+",-10) rotate(-90)");w+=2*p.legend.fields.length*e+5}}n.selectAll("text.legend").attr("fill",A()).attr("font-size",2*a.nodeHeight+"px")}}function Ca(){var e=a.nodeHeight;w=0;if(b.graphs)for(var d=0;d<b.graphs.length;d++){var g=b.graphs[d];switch(g.type){case "pie":w+=2*e+5;break;case "binary":w+=(g.legend.fields.length+1)*(2*e+5)+5;break;case "multibar":w+=g.legend.fields.length*(a.graphWidth+5);break;
case "heatmap":w+=2*g.legend.fields.length*e+5}}return 2*a.margin+(a.showNodeNames?Q:0)+(a.showGraphs?w:0)+(a.showDomains?a.domainWidth:0)}function xa(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var e=!0,g=a.nodeHeight,f=d.x,m=d.y,h=0;h<b.length;h++){var q=b[h].x,n=b[h].y;if((q-f)*(q-f)+(n-m)*(n-m)<=4*g*g){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};C.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",
d);n.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",d)}function O(d,m){phyd3.phylogram.lineAreas=[];phyd3.phylogram.dx=0;m=d3.layout.cluster().separation(function(a,b){return 1}).children(a.children||function(a){return a.branchset}).size([W*a.scaleY,N*a.scaleX]);K=m(b);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(K,N*a.scaleX);var e=n.selectAll("path.link");d||(e=e.data(m.links(K)),e.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",function(a){if(a.source.groupIDs&&
a.source.groupIDs.length){for(var d={depth:0},e=0;e<a.source.groupIDs.length;e++){var f=b.groups[a.source.groupIDs[e]];f&&f.depth>=d.depth&&(d=f)}return d.foregroundColor}return A()}));e.attr("d",phyd3.phylogram.rightAngleDiagonal);H=void 0;c=0;var h=K.filter(function(a){return!a.children});for(m=0;m<h.length;m++)(e=-1!=a.pinnedNodes.indexOf(h[m].id))?(h[m].show=!0,h[m].pinned=!0,da(h[m],!a.lineupNodes)||(H.show=H.pinned?!0:!1),H=h[m]):h[m].show=a.dynamicHide?da(h[m],!a.lineupNodes):!0;H=void 0;h=
K.filter(function(a){return a.children});for(m=0;m<h.length;m++)(e=-1!=a.pinnedNodes.indexOf(h[m].id))?(h[m].show=!0,h[m].pinned=!0,da(h[m],!0)||(H.show=H.pinned?!0:!1),H=h[m]):h[m].show=a.dynamicHide?da(h[m],!0):!0;d||(T=T.data(K),T.enter().append("svg:g").attr("class",function(a){return a.children?0==a.depth?"root node cid_"+a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",function(d){var e=d3.event;!e.ctrlKey||e.altKey||e.shiftKey?!e.altKey||e.ctrlKey||e.shiftKey?
!e.shiftKey||e.altKey||e.ctrlKey?e.shiftKey||e.altKey||e.ctrlKey||(void 0!==a.popupAction?a.popupAction(d):Ba(d)):(jQuery("#groupID").val(d.id),jQuery("#groupDepth").val(d.depth),b.groups[d.id]?(jQuery("#groupLabel").val(b.groups[d.id].label),jQuery("#groupLabelForegroundCP").colorpicker("setValue",b.groups[d.id].foregroundColor),jQuery("#groupLabelBackgroundCP").colorpicker("setValue",b.groups[d.id].backgroundColor)):(jQuery("#groupLabel").val(""),jQuery("#groupLabelForegroundCP").colorpicker("setValue",
A()),jQuery("#groupLabelBackgroundCP").colorpicker("setValue",a.invertColors?a.foregroundColor:a.backgroundColor)),jQuery("#groupLabelModal").modal("show")):(za(b,d.id),z(),B()):(Aa(b,d.id),f(),B())}).style("z-index",100).append("rect").attr("class","pointer").attr("fill","red").attr("width",2*a.nodeHeight+2+"px").attr("height",2*a.nodeHeight+2+"px").style("opacity","0").attr("x","-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));T.attr("transform",function(a){return"translate("+parseInt(a.y)+
","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function ua(b,d){if(Z(b,!0).match(d)){var e=!b.children&&a.lineupNodes?phyd3.phylogram.dx-b.y:0;n.selectAll("g.node.cid_"+b.id).append("rect").attr("class","searchResults").attr("width",Q+"px").attr("height",2*a.nodeHeight+2+"px").attr("fill","yellow").style("opacity","0.3").attr("x",e+"px").attr("y","-"+(a.nodeHeight+1)+"px")}if(b.children)for(e=0;e<b.children.length;e++)r=ua(b.children[e],d);return!1}function Aa(a,b,d){if(d||a.id==b)d=!0,a.branchset=
a.branchset.reverse();if(a.children)for(var e=0;e<a.children.length;e++)Aa(a.children[e],b,d)}function za(a,d){if(a.id==d)b.branchset=[a];else if(a.children)for(var e=0;e<a.children.length;e++)za(a.children[e],d)}function S(){O(!0);ba()}function da(b,d){var e=!0,f=b.y+1,h=b.y+5.5*Z(b,!0).length,m=b.x-a.nodeHeight,k=b.x+a.nodeHeight;if(!H)return H=b,!0;a.lineupNodes&&!b.children&&(f+=phyd3.phylogram.dx-b.y,h+=phyd3.phylogram.dx-b.y);var n=H,q=n.y+1,t=n.y+5.5*Z(n,!0).length,u=n.x-a.nodeHeight,v=n.x+
a.nodeHeight;a.lineupNodes&&!n.children&&(q+=phyd3.phylogram.dx-n.y,t+=phyd3.phylogram.dx-n.y);if(q<=f&&f<=t&&u<=m&&m<=v||q<=h&&h<=t&&u<=m&&m<=v||q<=f&&f<=t&&u<=k&&k<=v||q<=h&&h<=t&&u<=k&&k<=v)e=!1;if(e&&d)for(d=0;d<phyd3.phylogram.lineAreas.length;d++)if(u=phyd3.phylogram.lineAreas[d],n=u.start[0],q=u.end[0],t=u.start[1],u=u.end[1],t==u){if(m<=t&&u<=k&&!(n>h||q<f)){e=!1;break}}else if(n==q&&f<=n&&q<=h&&!(t>k||u<m)){e=!1;break}e&&(H=b);return e}function ba(){var b=ea.translate(),d=b[0]+a.marginX-
a.translateX,g=b[1]+a.marginY-a.translateY;n.attr("transform",function(a){return"translate("+parseInt(d)+","+parseInt(g)+") scale(1)"});0!=a.lastScale&&(ya(),a.dynamicHide&&L());ta();f()}a=a||{};a.scaleY=a.scaleY||1;a.scaleX=a.scaleX||1;a.translateX=a.translateX||0;a.translateY=a.translateY||0;a.height=a.height||800;a.margin=a.margin||20;a.scaleStep=a.scaleStep||.3;a.nodeHeight=a.nodeHeight||6;a.nodeHeightStep=a.nodeHeightStep||1;a.textLength=a.textLength||100;a.domainWidth=a.domainWidth||100;a.domainWidthStep=
a.domainWidthStep||100;a.graphWidth=a.graphWidth||20;a.graphWidthStep=a.graphWidthStep||10;a.domainLevel=a.domainLevel||1;a.domainLevelStep=a.domainLevelStep||10;a.outline=a.outline||.3;a.popupWidth=a.popupWidth||500;a.maxDecimalsSupportValues=a.maxDecimalsSupportValues||0;a.maxDecimalsLengthValues=a.maxDecimalsLengthValues||2;a.nanColor=a.nanColor||"#fff";a.foregroundColor=a.foregroundColor||"#000";a.backgroundColor=a.backgroundColor||"#fff";a.branchLengthColor=a.branchLengthColor||"red";a.supportValuesColor=
a.supportValuesColor||"blue";a.showNodesType=a.showNodesType||"only leaf";a.treeWidth=a.treeWidth||"auto";a.showFullTaxonomy="showFullTaxonomy"in a?a.showFullTaxonomy:!1;a.showLabels="showLabels"in a?a.showLabels:!0;a.showDomains="showDomains"in a?a.showDomains:!0;a.dynamicHide="dynamicHide"in a?a.dynamicHide:!1;a.invertColors="invertColors"in a?a.invertColors:!1;a.lineupNodes="lineupNodes"in a?a.lineupNodes:!0;a.showSupportValues="showSupportValues"in a?a.showSupportValues:!1;a.showLengthValues=
"showLengthValues"in a?a.showLengthValues:!1;a.showTaxonomy="showTaxonomy"in a?a.showTaxonomy:!0;a.showTaxonomyColors="showTaxonomyColors"in a?a.showTaxonomyColors:!0;a.showDomainNames="showDomainNames"in a?a.showDomainNames:!1;a.showDomainColors="showDomainColors"in a?a.showDomainColors:!0;a.showGraphs="showGraps"in a?a.showGraphs:!0;a.showGraphLegend="showGraphLegend"in a?a.showGraphLegend:!0;a.showNodeNames="showNodeNames"in a?a.showNodeNames:!0;a.showPhylogram="showPhylogram"in a?a.showPhylogram:
!1;a.pinnedNodes=a.pinnedNodes||[];b.groups=b.groups?b.groups:{};var n,T,C,I,K,P,H,E=[],D=[],R=100,Q=100,w=0,X=100,ma=0,Da=jQuery.extend(!0,{},b),J=!1;if(b.graphs)for(var oa=0;oa<b.graphs.length;oa++)if(0!=b.graphs[oa].legend.show){J=!0;break}X=J?X:0;a.marginX=a.margin;a.marginY=a.margin+(a.showGraphs?X:0);a.initialGraphWidth=a.graphWidth;var aa=jQuery(d).width(),N="auto"==a.treeWidth?aa-Ca():a.treeWidth,W=a.height-(2*a.margin+(a.showGraphs?X:0));N=parseInt(N);W=parseInt(W);0>N&&(N=aa-2*Q-(a.showGraphs?
2*X:0));var ea=d3.behavior.zoom().scaleExtent([1,1]).on("zoom",function(b){b=0;var d=d3.event.sourceEvent;if(d){if(0<d.deltaY||0>d.wheelDelta)b=-1*a.scaleStep;if(0>d.deltaY||0<d.wheelDelta)b=a.scaleStep;var e=.5,f=.5;d.altKey||d.ctrlKey||(e=d.layerX/aa,f=d.layerY/a.height);if(d.ctrlKey||!d.altKey)a.scaleY+=b,a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY+=W*b*f;if(d.altKey||!d.ctrlKey)a.graphWidth+=0==b?0:0>b?-10:10,10>a.graphWidth&&(a.graphWidth=10),a.domainWidth+=0==b?0:0>b?-1*a.domainWidthStep:
a.domainWidthStep,a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep),a.scaleX+=b,a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX+=N*b*e;a.lastScale=b;0!=a.lastScale?requestAnimFrame(S):requestAnimFrame(ba)}});d3.select("#dynamicHide").on("click",function(){a.dynamicHide=!a.dynamicHide;O(!0);L()});d3.select("#invertColors").on("click",function(){a.invertColors=!a.invertColors;ja()});jQuery("#foregroundColorButton").colorpicker().on("changeColor",function(){a.foregroundColor=
jQuery("#foregroundColor").val();ja()});jQuery("#backgroundColorButton").colorpicker().on("changeColor",function(){a.backgroundColor=jQuery("#backgroundColor").val();ja()});d3.select("#nodesType").on("change",function(){a.showNodesType=jQuery(this).val();S();U()});d3.select("#phylogram").on("click",function(){a.showPhylogram=!a.showPhylogram;O(!0);F();L()});d3.select("#lineupNodes").on("click",function(){a.lineupNodes=!a.lineupNodes;O(!0);ra();F();L()});d3.select("#lengthValues").on("click",function(){a.showLengthValues=
!a.showLengthValues;a.showLengthValues?d3.select("#maxDecimalsLengthValues").attr("disabled",null):d3.select("#maxDecimalsLengthValues").attr("disabled","disabled");ia()});d3.select("#maxDecimalsLengthValues").on("change",function(){a.maxDecimalsLengthValues=jQuery(this).val();ia()});d3.select("#supportValues").on("click",function(){a.showSupportValues=!a.showSupportValues;a.showSupportValues?d3.select("#maxDecimalsSupportValues").attr("disabled",null):d3.select("#maxDecimalsSupportValues").attr("disabled",
"disabled");ha()});d3.select("#maxDecimalsSupportValues").on("change",function(){a.maxDecimalsSupportValues=jQuery(this).val();ha()});d3.select("#nodeNames").on("click",function(){a.showNodeNames=!a.showNodeNames;U();F()});d3.select("#nodeLabels").on("click",function(){a.showLabels=!a.showLabels;sa()});d3.select("#taxonomy").on("click",function(){a.showTaxonomy=!a.showTaxonomy;U();F()});d3.select("#sequences").on("click",function(){a.showSequences=!a.showSequences;U();F()});d3.select("#nodesType").on("change",
function(){a.showNodesType=jQuery(this).val();S();U();F()});d3.select("#taxonomyColors").on("click",function(){a.showTaxonomyColors=!a.showTaxonomyColors;Y()});d3.select("#nodeHeightLower").on("click",function(){a.nodeHeight-=a.nodeHeightStep;a.nodeHeight<a.nodeHeightStep&&(a.nodeHeight=a.nodeHeightStep);O(!0);F();L();f()});d3.select("#nodeHeightHigher").on("click",function(){a.nodeHeight+=a.nodeHeightStep;O(!0);F();L();f()});d3.select("#resetZoom").on("click",function(){b=jQuery.extend(!0,{},Da);
z();B()});d3.select("#zoominY").on("click",function(){a.scaleY+=a.scaleStep;a.translateY+=W*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(S)});d3.select("#zoomoutY").on("click",function(){a.scaleY-=a.scaleStep;a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY-=W*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(S)});d3.select("#zoominX").on("click",function(){a.scaleX+=a.scaleStep;a.translateX+=N*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(S)});d3.select("#zoomoutX").on("click",
function(){a.scaleX-=a.scaleStep;a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX-=N*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(S)});d3.select("#domains").on("click",function(){a.showDomains=!a.showDomains;a.showDomains?(d3.select("#domainNames").attr("disabled",null),d3.select("#domainColors").attr("disabled",null),d3.select("#domainWidthLower").attr("disabled",null),d3.select("#domainWidthHigher").attr("disabled",null),d3.select("#domainLevelLower").attr("disabled",null),d3.select("#domainLevelHigher").attr("disabled",
null)):(d3.select("#domainNames").attr("disabled","disabled"),d3.select("#domainColors").attr("disabled","disabled"),d3.select("#domainWidthLower").attr("disabled","disabled"),d3.select("#domainWidthHigher").attr("disabled","disabled"),d3.select("#domainLevelLower").attr("disabled","disabled"),d3.select("#domainLevelHigher").attr("disabled","disabled"));pa();fa()});d3.select("#domainColors").on("click",function(){a.showDomainColors=!a.showDomainColors;ka()});d3.select("#domainNames").on("click",function(){a.showDomainNames=
!a.showDomainNames;fa()});d3.select("#domainWidthLower").on("click",function(){a.domainWidth-=a.domainWidthStep;a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep);P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);M()});d3.select("#domainWidthHigher").on("click",function(){a.domainWidth+=a.domainWidthStep;P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);M()});d3.select("#domainLevelLower").on("click",function(){a.domainLevel/=a.domainLevelStep;ca()});d3.select("#domainLevelHigher").on("click",
function(){a.domainLevel*=a.domainLevelStep;ca()});d3.select("#graphLegend").on("click",function(){a.showGraphLegend=!a.showGraphLegend;a.marginY=a.margin+(a.showGraphs?X:0);V();ba()});d3.select("#graphs").on("click",function(){a.showGraphs=!a.showGraphs;a.showGraphs?(d3.select("#graphLegend").attr("disabled",null),d3.select("#graphWidthLower").attr("disabled",null),d3.select("#graphWidthHigher").attr("disabled",null)):(d3.select("#graphLegend").attr("disabled","disabled"),d3.select("#graphWidthLower").attr("disabled",
"disabled"),d3.select("#graphWidthHigher").attr("disabled","disabled"));qa()});d3.select("#graphWidthLower").on("click",function(){a.graphWidth-=a.graphWidthStep;a.graphWidth<a.graphWidthStep&&(a.graphWidth=a.graphWidthStep);F()});d3.select("#graphWidthHigher").on("click",function(){a.graphWidth+=a.graphWidthStep;F()});d3.select("#linkSVG").on("click",function(){var a=va();saveAs(new Blob([unescape(encodeURIComponent(a))],{type:"application/svg+xml"}),"phylogram.svg")});d3.select("#linkPNG").on("click",
function(){var a=va(),b=document.createElement("canvas");d3.select("g#main").node().getBBox();canvg(b,a);b.toBlob(function(a){saveAs(a,"phylogram.png")})});d3.select("#searchQuery").on("change",function(){ta()});d3.select("#applyTaxonomyColors").on("click",function(){d3.selectAll("input.taxColor").each(function(a){b.taxcolors[a.taxonomy].color=jQuery(this).val()});jQuery("#taxonomyColorsModal").modal("hide");Y()});d3.select("#applyGroupLabel").on("click",function(){var a=jQuery("#groupID").val(),
d=jQuery("#groupDepth").val(),g=jQuery("#groupLabel").val(),n=jQuery("#groupLabelBackground").val(),p=jQuery("#groupLabelForeground").val();jQuery("#groupLabelModal").modal("hide");b.groups[a]={id:a,depth:d,label:g,foregroundColor:p,backgroundColor:n};h(b,a);B();f()});d3.select("#clearGroupLabel").on("click",function(){var a=jQuery("#groupID").val();jQuery("#groupLabelModal").modal("hide");delete b.groups[a];t(b,a);B()});J=function(){var a=[],d;for(d in b.taxcolors){var f=b.taxcolors[d];f.taxonomy=
d;a.push(f)}return a}();J.length?(J=d3.select("#taxonomyColorsList").selectAll("div.taxColor").data(J).enter().append("div").attr("class","form-group col-xs-4 taxColor"),J.append("label").attr("class","col-xs-4 control-label").text(function(a){return a.taxonomy}),J=J.append("div").attr("class","col-xs-8").append("div").attr("class","input-group colorpicker-component"),J.append("input").attr("class","form-control taxColor").attr("type","text").attr("value",function(a){return a.color.replace("0x","#")}),
J.append("span").attr("class","input-group-btn").append("span").attr("class","input-group-addon btn btn-fab btn-fab-mini").append("i"),jQuery(".colorpicker-component").colorpicker(),jQuery("#taxonomyColorsModal").on("hide.bs.modal",function(){jQuery(".colorpicker-component").colorpicker("hide")}),jQuery("#groupLabelModal").on("hide.bs.modal",function(){jQuery(".colorpicker-component").colorpicker("hide")})):d3.select("#taxonomyColorsList").append("div").attr("class","col-sm-12 text-center").text("No taxonomy information available");
B()}})();
...
{show:parseInt(d.getAttribute("show")),stacked:parseInt(d.getAttribute("stacked")),fields:[],gradient:{}},a=d.getElementsByTagName("field"),f=0;f<a.length;f++){var h=a[f],v=h.getElementsByTagName("name")[0];v=v?v.textContent:"";var t=h.getElementsByTagName("color")[0];t=t?t.textContent:"";var z=h.getElementsByTagName("background")[0];z=z?z.textContent:"";var B=h.getElementsByTagName("shape")[0];B=B?B.textContent:"";h=(h=h.getElementsByTagName("url")[0])?h.textContent:"";b.fields.push({name:v,color:t,
background:z,shape:B,url:h})}if(d=d.getElementsByTagName("gradient")[0])v=(v=d.getElementsByTagName("name")[0])?v.textContent:"",d=(d=d.getElementsByTagName("classes")[0])?parseInt(d.textContent):0,b.gradient={name:v,classes:d};return b};phyd3.phyloxml.parseData=function(d){var b={tag:d.getAttribute("tag"),ref:d.getAttribute("ref")},a=d.getElementsByTagName("min")[0];b.min=a?a.textContent:void 0;a=d.getElementsByTagName("max")[0];b.max=a?a.textContent:void 0;d=d.getElementsByTagName("values");for(a=
0;a<d.length;a++){var f=d[a].getAttribute("for"),h=d[a].getElementsByTagName("value");b[f]=[];for(var v=0;v<h.length;v++)b[f].push(parseFloat(h[v].textContent))}return b};phyd3.phyloxml.parseGraph=function(d){for(var b={name:"",legend:{},data:{},type:d.getAttribute("type")},a=0;a<d.childNodes.length;a++){var f=d.childNodes[a];switch(f.nodeName){case "name":b.name=f.textContent;break;case "legend":b.legend=phyd3.phyloxml.parseLegend(f);break;case "data":b.data=phyd3.phyloxml.parseData(f);break;case "#text":case "#comment":break;
default:console.log("Undefined tag: "+f.nodeName+" "+f.textContent+" - skipping...")}}return b};phyd3.phyloxml.parseLabel=function(d){for(var b={showLegend:!1,name:"",data:{},type:d.getAttribute("type")},a=0;a<d.childNodes.length;a++){var f=d.childNodes[a];switch(f.nodeName){case "name":b.name=f.textContent;b.showLegend=f.getAttribute("show");break;case "data":b.data=phyd3.phyloxml.parseData(f);break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+f.nodeName+" "+f.textContent+
" - skipping...")}}return b};phyd3.phyloxml.parse=function(d){var b={branchset:[],properties:[],confidence:[]},a=d.getElementsByTagName("phylogeny");a=a[0];b.rooted=a.getAttribute("rooted");b.rerootable=a.getAttribute("rerootable");b.branch_length_unit=a.getAttribute("branch_length_unit");b.type=a.getAttribute("type");if(a&&a.childNodes)for(var f=0;f<a.childNodes.length;f++){var h=a.childNodes[f];switch(h.nodeName){case "clade":b.branchset.push(phyd3.phyloxml.parseClade(h));break;case "confidence":b.confidence.push(phyd3.phyloxml.parseConfidence(h));
break;case "date":b.date=h.textContent;break;case "description":b.description=h.textContent;break;case "id":b.id=h.textContent;break;case "name":b.name=h.textContent;break;case "property":b.properties.push(phyd3.phyloxml.parseProperty(h));break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+h.nodeName+" "+h.textContent+" - skipping...")}}a=d.getElementsByTagName("taxonomies");a=a[0];var v={};if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++){var t=a.childNodes[f];if("color"==
t.nodeName){h=t.getElementsByTagName("code")[0].textContent;var z=t.getElementsByTagName("value")[0].textContent;v[h]={color:z}}if("taxonomy"==t.nodeName){h=t.getAttribute("code");z=(z=t.getElementsByTagName("color")[0])?z.textContent:z;var B=t.getElementsByTagName("name")[0];B=B?B.textContent:B;t=(t=t.getElementsByTagName("url")[0])?t.textContent:t;v[h]={color:z,name:B,url:t}}}b.taxcolors=v;a=d.getElementsByTagName("domains");a=a[0];v={};if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)t=a.childNodes[f],
"color"==t.nodeName&&(h=t.getElementsByTagName("code")[0].textContent,z=t.getElementsByTagName("value")[0].textContent,v[h]={color:z}),"domain"==t.nodeName&&(h=t.getAttribute("name"),z=(z=t.getElementsByTagName("color")[0])?z.textContent:z,B=(B=t.getElementsByTagName("description")[0])?B.textContent:B,t=(t=t.getElementsByTagName("url")[0])?t.textContent:t,v[h]={color:z,description:B,url:t});b.domcolors=v;a=d.getElementsByTagName("graphs");a=a[0];h=[];if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)z=
a.childNodes[f],"graph"==z.nodeName&&h.push(phyd3.phyloxml.parseGraph(z));b.graphs=h;a=d.getElementsByTagName("labels");a=a[0];d=[];if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)h=a.childNodes[f],"label"==h.nodeName&&d.push(phyd3.phyloxml.parseLabel(h));b.labels=d;return b}})();
/* Blob.js

  ...
  networkMenu: function(o, type, it) {
    ...
    if(type == 'cs') // right click menu only
    {
        ...
        if(o.nodes.length > 1)
        {
          ...
          items.push({
            ...
            handler: function () {
              ...
              var seen = {}, found, changed = [], f = function(node) { // called when node added successfully
                for(var i = 0; i < n.length; i++)
                  if(n[i].parentNode != node.id)
                  {
                    changed.push(n[i]);
                    n[i].parentNode = node.id;
                  }
              ...
              for(var i = 0; i < n.length; i++)
                if(n[i].parentNode)
                {
                  if(found && !seen[n[i].parentNode])
                  {
                    Ext.Msg.alert('Error', 'The nodes you selected belong to different groups - '+n[i].label+'\'s parent is '+this.canvas.nodes[n[i].parentNode].label+', different than '+found.label+'\'s parent '+this.canvas.nodes[found.parentNode].label+'. Grouping multiple nodes previously belonging to different groups is frequently a user mistake, and thus intentionally not supported');
                    return;
                  }
                  found = n[i];
                  seen[n[i].parentNode] = 1;
                }
              if(!found) // gotta make a new node, tricky
              {
                Ext.Msg.show({
                  title:'Important',
            ...
            handler: function () {
              ...
              Ext.Msg.confirm(multi? 'Warning!!':'Confirm the operation', multi?'The nodes you selected belong to different groups. Ungrouping multiple groups at once is frequently a user mistake, are you sure you still want to proceed? This operation cannot be reverted!':'This operation cannot be reverted. Are you sure you would like to ungroup the whole group (group parent: '+this.canvas.nodes[found.parentNode].label+', '+all.length+' child node'+(all.length>1?'s':'')+')?', function(btn) {
                if(btn == 'yes') {
                  var changed = [];
          ...
          if(o.nodes.length == 2)
          {
            var n1 = o.nodes[0], label1 = n1.label || n1.name || n1.id,
                n2 = o.nodes[1], label2 = n2.label || n2.name || n2.id;
            items.push({
        ...
        else
        {
          ...
          items.push({
            ...
            handler: function() {
              ...
              this.canvas.addRemoveToSelectedDataPoints('Network', [this.canvas.data.nodeIndices[n.id]]);
          ...
          if(n.parentNode)
          {
            var pn = this.canvas.nodes[n.parentNode], pl = pn.label || pn.name || pn.id;
            items.push({
            text: 'Grouping',
            menu: [
              {
      ...
      else if(o && o.edges && o.edges[0])
      {
        ...
        var f = function(node) {
          var label = node.label || node.name || node.id, item = {
            icon: this.imgDir + 'edit.png',
            text: 'Edit ' + label,
      ...
      else
      {
        ...
        items.push({
          text: (this.canvas.overrideEventlessNodes? 'Enable':'Disable') + ' Eventless',
          scope: this.canvas,
          handler: function() {
...
//         }, '-');
  ...
  editNode: function(b, e, o, callback) {
  ...
  constructor: function(config, extCanvas, callback) {
    ...
    var item1 = [
      // label is required, parent, name, color, size, shape can all be hidden if user wants
      {
        ...
        items: [
          ...
          {
            xtype: 'checkbox',
            checked: isMulti? false : config.hideLabel || config.hideName,
            boxLabel: 'Hide label?',
            name: 'hideLabel',
            flex: 1
          }
      ...
      },
      {
        ...
        fieldLabel: 'Tooltip',
        items: [
          { xtype: 'textfield', value: isMulti? null : config.tooltip || config.name || '',
              ...
              render: function(l) {
                l.el.on('click', function() {
                  extCanvas.selectNode(function(node) {
                    l.previousSibling().setValue(node.id);
                  });
                });
              }
            }
    ...
    var item2 = [
        ...
        hideLabel: h.color, hidden: h.color },
      {
        ...
        items: [
          ...
          { xtype:'numberfield', name:'outlineWidth', hideLabel: h.outlineWidth, hidden: h.outlineWidth,
            width: 30, value: isMulti? null : config.outlineWidth || '' }
        ]
      },
      {
        xtype: 'compositefield',
        ...
        allowBlank: true, name:'imagePath', disabled: config.shape != 'image' },
      {
        ...
        items: [{
          xtype: 'checkbox',
          checked: isMulti? false : config.eventless,
          boxLabel: 'Node is eventless?',
          name: 'eventless',
          flex: 1
        },
        {
  ...
  constructor: function(config, extCanvas, isSaveAs) { // config should be the network obj (with (optional) id,name,(optional) description), with a (optional) key extCanvas being the panel
    ...
    if(isSaveAs)
    {
      config = { category: config.category, description: config.description,
        options: BMS.AG.ML.Util.clone(config.options) };
    }
    var ps = [{
      title: 'Customize',
      defaults: { width: 220 },
      items: [
        ...
        },
        {
          ...
          triggerAction: 'all',
          typeahead: true,
          emptyText: 'Type UserIds or Select',
          mode: 'local',
          store: ['owner only', 'everyone'],
          value: config.shared || 'owner only'
        }
//         {
//           xtype: 'checkbox',
//           name: 'shared',
//           hideLabel: true,
//           boxLabel: 'Shared with everyone?',
//           checked: config.shared? true : false
//         }
      ]
    }];
    var bitems = ['->'];

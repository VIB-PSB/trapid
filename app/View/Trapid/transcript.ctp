    ...
    <div class="col-md-9 col-lg-10" id="transcript-data-col">
            ...
            <div class="row">
                ...
                <div class="col-md-10 col-xs-8">
                    ...
                    if ($exp_info['genefamily_type'] == "HOM" && $transcript_info['full_frame_info']) {
                        ...
                        echo $this->Html->link(
                            "<span class='glyphicon glyphicon-edit'></span> change gene family",
                            array("controller" => "trapid", "action" => "similarity_hits", $exp_id, $transcript_info['transcript_id']),
                            array("escape" => false, "class" => "btn btn-xs btn-default pull-right", "title" => "Browse similarity search results to change gene family")
                        );
                    } else {
                        // Should that be displayed at all? Maybe confusing for users...
                        echo $this->Html->link(
                            "<span class='glyphicon glyphicon-edit'></span> change gene family",
                            "#",
        ...
        <section class="page-section-sm">
            ...
            if ($transcript_info['putative_frameshift'] == 1) {
                ...
                echo "<li class='text-warning'>A putative frameshift was detected in this sequence</li>";
                echo "</ul>\n";
                echo "</div>\n";
                echo "</div>\n";
            }
            ...
            <div class="panel panel-default container-fluid">
                ...
                <div class="panel-body">
                    <div class="textarea-wrapper">
                        <textarea readonly class='fixed-width-text textarea-sqce' name="transcript_sequence" id="transcript_sequence"><?php echo $transcript_info['transcript_sequence']; ?></textarea>
                    </div>
                </div>
            ...
            <div class="panel panel-default container-fluid">
                <div class="panel-heading row">
                    Protein sequence (translated ORF)
                </div>
                <?php if ($transcript_info['orf_sequence'] != "") : ?>
                    <div class="panel-body">
                        <div class="textarea-wrapper">
        ...
        <section class="page-section-sm">
            ...
            <div class="row">
                ...
                <div class="col-md-10 col-xs-8">
                    <?php
                    $possible_meta = array(
                        "No Information" => array("color" => "#bbbbbb", "css-class" => "meta-ni"),
            ...
            <?php if (in_array("go", $exp_info['function_types'])) : ?>
                <section class="page-section-xs">
                    ...
                    if ($associated_go) {
                        ...
                        echo "<ul class='tabbed-header list-unstyled list-inline'>\n";
                        echo "<strong>Show: </strong>";
                        echo "<li id='tab_collapsed' class='selected_tab'>";
                        echo "<a>Collapsed GO data</a> ";
                        echo "</li>\n";
                        echo "<li id='tab_all'>";
                        echo "<a>All GO data</a> &nbsp;";
                        echo "</li>\n";
                        ...
                        foreach ($all_gos as $go_cat_id => $go_category) {
                            if (!empty($go_category["go_terms"])) {
                                ...
                                echo "</h5>";
                                echo "<table class='table table-striped table-condensed table-bordered table-hover table-trs-fct'>\n";
                                echo "<thead><tr><th>GO term</th><th>Description</th></tr></thead>\n";
                                echo "<tbody>\n";
                        ...
                        foreach ($all_gos as $go_cat_id => $go_category) {
                            if (!empty($go_category["go_terms"])) {
                                ...
                                echo "<tbody>\n";
                                foreach ($go_category["go_terms"] as $go_term) {
                                    $class = $go_term["is_hidden"] == 0 ? "class='success'" : null;
                                    $web_go = str_replace(":", "-", $go_term["go_id"]);
                                    echo "<tr " . $class . ">";
                                    echo "<td>" . $this->Html->link($go_term["go_id"], array("controller" => "functional_annotation", "action" => "go", $exp_id, $web_go)) . "</td>";
                                    echo "<td>" . $go_info[$go_term["go_id"]]['desc'] . "</td>";
                        ...
                        echo "<table class='table table-striped table-condensed table-bordered table-hover table-trs-fct'>\n";
                        echo "<thead>\n";
                        echo "<tr><th>GO term</th><th>Description</th></tr>\n";
                        echo "</thead>\n";
                        echo "<tbody>\n";
                        foreach ($associated_go as $ag) {
                ...
                <section class="page-section-sm">
                    ...
                    <dd>
                        ...
                        if ($associated_interpro) {
                            echo "<table class='table table-striped table-condensed table-bordered table-hover table-trs-fct'>\n";
                            echo "<thead>\n";
                            echo "<tr><th>InterPro domain</th><th>Description</th></tr>\n";
                            echo "</thead>\n";
                            echo "<tbody>\n";
                ...
                <section class="page-section-sm">
                    ...
                    <dd>
                        ...
                        if ($associated_ko) {
                            echo "<table class='table table-striped table-condensed table-bordered table-hover table-trs-fct'>\n";
                            echo "<thead>\n";
                            echo "<tr><th>KO term</th><th>Description</th></tr>\n";
                            echo "</thead>\n";
                            echo "<tbody>\n";
            ...
            <div class="row">
                ...
                <div class="col-md-10 col-xs-8">
                    ...
                    if ($transcript_info['full_frame_info']) { // Link is displayed only if the transcript had any similarity search hit
                        echo $this->Html->link(
                            "Browse transcript's similarity search output (DIAMOND)",
...
<script type="text/javascript">
    /* Toggle collpased/all GO annotations */
    ...
    document.getElementById('tab_collapsed').addEventListener('click', () => switch_go_display('tab_collapsed','div_collapsed'));
    document.getElementById('tab_all').addEventListener('click', () => switch_go_display('tab_all','div_all'));
    ...
    document.getElementById("copy_transcript_sequence").addEventListener("click", function() {
        copy_to_clipboard("transcript_sequence");
    }, false);
    <?php if ($transcript_info['orf_sequence'] != "") : ?>
        document.getElementById("copy_orf_nt_sequence").addEventListener("click", function() {
            copy_to_clipboard("orf_sequence");
        }, false);
        document.getElementById("copy_orf_aa_sequence").addEventListener("click", function() {
            copy_to_clipboard("aa_sequence");
        }, false);
        const orf_textarea = document.getElementById("orf_sequence");
        orf_textarea.addEventListener("input", function() {
            const orf_changed = orf_textarea.value !== orf_textarea.defaultValue;
    ...
    <?php if ($exp_info['perform_tax_binning'] == 1) : ?>
        ...
        $(".toggle-clades").on("click", function() {
            const $hide_clades_elmt = $("#hide-clades");
            const clades_shown = $hide_clades_elmt.hasClass("hidden");
    ...
    const meta_select = document.getElementById("meta_annotation");
    meta_annotation.addEventListener("change", function() {
        let meta_changed = false;
        for (let i = 0; i < meta_select.length; i++) {
    ...
    let new_subset_name_input = document.getElementById("new_subset_name");
    let new_subset_checkbox = document.getElementById("new_subset");
    ...
    $('#all-subsets .subset-checkbox').on("change", function() {
        let subsets_changed = false;
        $('#all-subsets .subset-checkbox').each(function() {
            const checkbox_elmt = $(this)[0];
...
<?php echo $this->element("help_tooltips/enable_tooltips",  ["container" => "#transcript-data-col"]); ?>

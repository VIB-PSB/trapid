...
<section class="page-section">
    ...
    <dl class="dl-horizontal">
        <dt>Transcript id</dt>
        <dd>
            <?php echo $this->Html->link($transcript_info['transcript_id'], [
                'controller' => 'trapid',
                'action' => 'transcript',
                $exp_id,
        ...
        <dt>Gene family</dt>
        <?php if ($transcript_info['gf_id'] == '') {
            echo "<dd><span class='disabled'>Unavailable</span></dd>\n";
        } else {
            echo '<dd>' .
                $this->Html->link($transcript_info['gf_id'], [
                    'controller' => 'gene_family',
                    'action' => 'gene_family',
                    $exp_id,
                    $transcript_info['gf_id']
                ]) .
                '</dd>';
...
<?php
if (isset($error)) {
    echo "<br/><br/><span class='text-danger'>" . $error . '</span><br/><br/>';
}
if (isset($message)) {
    echo "<br/><br/><span class='text-info'>" . $message . '</span><br/><br/>';
}
?>
                    ...
                    if ($index == 0) {
                        if (!$exp_info['allow_linkout'] || $db_type == 'eggnog') {
                            ...
                            echo '<td>' .
                                $this->Html->link(
                                    $gene_id,
                                    $exp_info['datasource_URL'] . 'genes/view/' . urlencode($gene_id)
                                ) .
                                '</td>';
                        }
                    } else {

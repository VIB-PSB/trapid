...
$data_title = ['go' => 'GO term', 'ipr' => 'InterPro domain', 'ko' => 'KO term', 'subset' => 'subset'];
...
$base_link = [
    'go' => $this->Html->url(['controller' => 'functional_annotation', 'action' => 'go', $exp_id]),
    'ipr' => $this->Html->url(['controller' => 'functional_annotation', 'action' => 'interpro', $exp_id]),
    'ko' => $this->Html->url(['controller' => 'functional_annotation', 'action' => 'ko', $exp_id]),
    'subset' => $this->Html->url(['controller' => 'labels', 'action' => 'view', $exp_id])
];
...
switch ($data_type) {
    case 'go':
        for ($i = $data_offset; $i < $data_count; $i++) {
            ...
            $popover_data[] = '<li>';
            $popover_data[] =
                "<a href='" . $base_link['go'] . '/' . $go_web . "'>" . $data_desc[$trs_data[$i]]['desc'] . '</a>';
            $popover_data[] =
                ' ' .
                $this->element('go_category_badge', [
                    'go_category' => $data_desc[$trs_data[$i]]['type'],
                    'small_badge' => true,
                    'no_color' => false
                ]);
            $popover_data[] = '</li>';

...
// Ideally, it should be updated to make it work with jQuery dataTables!
...
$gf_header_class = null;
$gf_cell_class = null;
if (isset($gf_info)) {
    $gf_header_class = "class='warning'";
    $gf_cell_class = 'warning-faint';
}
$go_header_class = null;
$go_cell_class = null;
if (isset($go_info)) {
    $go_header_class = "class='warning'";
    $go_cell_class = 'warning-faint';
}
$ipr_header_class = null;
$ipr_cell_class = null;
if (isset($interpro_info)) {
    $ipr_header_class = "class='warning'";
    $ipr_cell_class = 'warning-faint';
}
$ko_header_class = null;
$ko_cell_class = null;
if (isset($ko_info)) {
    $ko_header_class = "class='warning'";
    $ko_cell_class = 'warning-faint';
}
$label_header_class = null;
$label_cell_class = null;
if (isset($label)) {
    $label_header_class = "class='warning'";
    $label_cell_class = 'warning-faint';
}
...
$function_headers = [
    'go' => ['highlight_class' => $go_header_class, 'label' => 'GO annotation'],
    'interpro' => ['highlight_class' => $ipr_header_class, 'label' => 'Protein domain annotation'],
    'ko' => ['highlight_class' => $ko_header_class, 'label' => 'KO annotation']
];
?>
...
<table id="table_func" class="table table-striped table-bordered table-hover table-condensed small">
    ...
    <tbody>
        ...
        foreach ($transcript_data as $transcript_dat) {
            ...
            // Transcript ID
            echo '<td>' .
                $this->Html->link($td['transcript_id'], [
                    'controller' => 'trapid',
                    'action' => 'transcript',
                    $exp_id,
                    urlencode($td['transcript_id'])
                ]) .
                '</td>';
            // GF ID
            ...
            if ($td['gf_id']) {
                ...
                echo "<span class='text-muted'>" . $unassigned_str . '</span>';
            }
            echo "</td>\n";
            // GO annotation
                    ...
                    for ($i = 0; $i < $n_trs_go && $i < $max_items; $i++) {
                        ...
                        echo ' ' .
                            $this->element('go_category_badge', [
                                'go_category' => $go_info_transcripts[$go]['type'],
                                'small_badge' => true,
                                'no_color' => false
                            ]);
                        if ($i == $max_items - 1 && $n_trs_go > $max_items) {
                            echo $this->element('table_more_label', [
                                'trs_data' => $transcripts_go[$td['transcript_id']],
                                'data_desc' => $go_info_transcripts,
                                'data_type' => 'go',
                                'data_offset' => $max_items,
                                'data_title' => 'GO term'
                            ]);
                        }
                        echo '</li>';
                ...
                if (!array_key_exists($td['transcript_id'], $transcripts_ipr)) {
                    ...
                    for ($i = 0; $i < $n_trs_ipr && $i < $max_items; $i++) {
                        ...
                        if ($i == $max_items - 1 && $n_trs_ipr > $max_items) {
                            echo $this->element('table_more_label', [
                                'trs_data' => $transcripts_ipr[$td['transcript_id']],
                                'data_desc' => $ipr_info_transcripts,
                                'data_type' => 'ipr',
                                'data_offset' => $max_items,
                                'data_title' => 'InterPro domain'
                            ]);
                        }
                        echo '</li>';
                    }
                ...
                if (!array_key_exists($td['transcript_id'], $transcripts_ko)) {
                    ...
                    for ($i = 0; $i < $n_trs_ko && $i < $max_items; $i++) {
                        ...
                        if ($i == $max_items - 1 && $n_trs_ko > $max_items) {
                            echo $this->element('table_more_label', [
                                'trs_data' => $transcripts_ko[$td['transcript_id']],
                                'data_desc' => $ko_info_transcripts,
                                'data_type' => 'ko',
                                'data_offset' => $max_items,
                                'data_title' => 'KO term'
                            ]);
            ...
            if (!array_key_exists($td['transcript_id'], $transcripts_labels)) {
                ...
                for ($i = 0; $i < $n_trs_sub && $i < $max_items; $i++) {
                    ...
                    if ($i == $max_items - 1 && $n_trs_sub > $max_items) {
                        echo $this->element('table_more_label', [
                            'trs_data' => $transcripts_labels[$td['transcript_id']],
                            'data_type' => 'subset',
                            'data_offset' => $max_items,
                            'data_title' => 'subset'
                        ]);
            ...
            echo '<td>' .
                $this->Html->link($td['meta_annotation'], [
                    'controller' => 'trapid',
                    'action' => 'transcript_selection',
                    $exp_id,
                    'meta_annotation',
                    urlencode($td['meta_annotation'])
                ]) .
                '</td>';
            echo "</tr>\n";
        }

...
if(isset($ref_gf_id)) {
    ...
    if(isset($func_data)) {
        if(!empty($func_data)) {
            echo " <span class=\"label label-default\">" . $func_data['func_cat_id']  ."</span> <strong>" . $func_data['func_cat_label'] . "</strong> ; ";
    ...
    if(isset($top_iprs)) {
        ...
        if(!empty($top_iprs)) {
            ...
            foreach ($top_iprs as $ipr) {
                echo "<a class='linkout' target='_blank' href='https://www.ebi.ac.uk/interpro/entry/InterPro/" . $ipr['name'] . "' >" . $ipr['desc'] . "</a> ";
            }
        }
        else {

 ...
 *
 * @author Joaqu�n Windm�ller <http://aikon.com.ve/>
 * @copyright Joaqu�n Windm�ller, 14 May, 2008
...
class FlashChartHelper extends AppHelper {
	 ...
	 * @return void
	 * @author Joaqu�n Windm�ller
	 **/
	function begin($width=null, $height=null, $x_legend = 'X', $y_legend = 'Y', $autoRange = true,$new_id=null) {
		$this->graph = new graph();
		if($new_id){		  
		  $this->graph->set_unique_id2($new_id);
		}
		$this->types = array();
	...
	/* CREATED BY MIBEL : ACCESS TO RIGHT HAND Y AXIS
	 * 
	 */
	function createRightYAxis($max_value,$title,$color){
	  $this->graph->set_y_right_label_style('10',$color);
	  $this->graph->set_y_right_legend($title,'10',"#666666");
	  $this->graph->set_y_right_max($max_value);	 
	  $this->graph->set_y_right_min(0);  	  
	  $this->graph->y_right_axis_colour($color);
	}


	function addToRightYAxis($data_number){
	    $this->graph->attach_to_y_right_axis($data_number);
	}
	...
	function __scatter($info) {
		...
		if ($this->autoRange) {		 
		  	...
		  	$x_values = Set::extract($info['data'],'{n}.x');		       
			$y_values = Set::extract($info['data'],'{n}.y');
	 ...
	 * @author Joaqu�n Windm�ller
	 **/
	function pie($data, $alpha=60, $line_color = '#505050', 
	   $style = array('font-size' => '12px', 'color' => '#333333')) {
	...
	/*
	 * CREATED BY MIBEL : USED FOR CREATING GENE FAMILY  PIES
	 */
	function pie_genefamilies($data, $links=null,$alpha=60, $line_color = '#505050', $style = array('font-size' => '12px', 'color' => '#333333')) {
		$style = $this->__prepareOptions($style);
		$this->graph->pie($alpha, $line_color, $style);
		$r = rand(100,200);
		$g = rand(0,100);
		...
		$values = array_values(Set::extract($data, '{}.value'));
		if($links)
		  $this->graph->pie_values($values, $labels,$links);
		else
		  $this->graph->pie_values($values,$labels);
		$colors = array_values(Set::extract($data, '{}.color'));
		$this->graph->pie_slice_colours($colors);
		$this->setToolTip('#x_label#<br>#val# genes');	
		$this->data = null;	
	}


	function setNewToolTip($new_tool_tip){
	  $this->setToolTip($new_tool_tip);
	}
	...
	function setGridColor($color1,$color2){
	  $this->graph->x_axis_colour($color1,$color2);
	  $this->graph->y_axis_colour($color1,$color2);
	}
	...
	/*
	 * Sets the inner background color for the chart
	 */
	function setInnerBackground($color1,$color2=null,$angle=null){
	  $this->graph->set_inner_background($color1,$color2,$angle);
	}

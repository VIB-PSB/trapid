...
<section class="page-section">
    ...
    echo $this->Form->create(false, [
        'url' => ['controller' => 'tools', 'action' => 'compare_ratios_chart', $exp_id, $type],
        'type' => 'post'
    ]);
    ...
    if ($type == 'go') {
        ...
        echo '<dd>';
        $min_cov_values = ['none' => 'No minimum coverage'];
        for ($i = 1; $i < 10; $i++) {
            $min_cov_values[$i] = $i . '%';
        }
        for ($i = 10; $i < 100; $i += 10) {
        ...
        echo '<dd>';
        $both_present_check = null;
        if (isset($both_present)) {
            $both_present_check = " checked='checked' ";
        }
...
<?php if (isset($result)): ?>
    ...
    <?php else: ?>
        <?php
        echo $this->Html->script(['canvasXpress/canvasXpress.min.js', 'canvasXpress/ext-canvasXpress.js']);
        ...
        <script type="text/javascript">
            ...
            function redirect(o) {
                var basic_url = <?php echo "'" .
                    $this->Html->url(
                        ['controller' => 'trapid', 'action' => 'transcript_selection', $exp_id],
                        true
                    ) .
                    "'"; ?>;

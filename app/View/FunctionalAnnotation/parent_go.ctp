...
<div class="page-header">
    <h1 class="text-primary"><?php echo $go; ?> <small>parental GO terms</small></h1>
</div>
...
<?php if (isset($max_parent_gos_reached)) {
    echo "<section class='page-section-xs'>\n";
    echo "<span class='text-danger'>\n";
    echo 'Too many parental GOs present (limit is ' . $max_parent_gos_reached . ')<br/>';
    echo 'Only top ' . $max_parent_gos_reached . ' parental GO terms are displayed';
    echo '</span>';
    echo "</section>\n";
} ?>
<section class="page-section-sm">
    ...
    <?php if ($num_parent_gos == 0): ?>
        <p class="text-justify text-muted">No parental GO terms with associated transcripts available.</p>
    <?php else: ?>
        ...
        <table class='table table-striped table-condensed table-bordered table-hover sortable'>
            ...
            <tbody>
                <?php foreach ($parent_go_counts as $parent_go => $parent_go_info) {
                    echo '<tr>';
                    echo '<td>' .
                        $this->Html->link($parent_go, [
                            'controller' => 'functional_annotation',
                            'action' => 'go',
                            $exp_id,
                            str_replace(':', '-', $parent_go)
                        ]) .
                        ' ' .
                        $this->element('go_category_badge', [
                            'go_category' => $parent_go_info['info'],
                            'small_badge' => false,
                            'no_color' => false
                        ]) .
                        '</td>';

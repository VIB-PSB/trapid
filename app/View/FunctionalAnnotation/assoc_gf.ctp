    ...
    <dl class="standard dl-horizontal">
        <?php if ($type == 'go') {
            ...
            echo $exp_info['allow_linkout']
                ? $this->Html->link($go, $exp_info['datasource_URL'] . 'go/view/' . $go_web)
                : $go;
            echo '&nbsp;';
            echo $this->element('go_category_badge', ['go_category' => $go_category, 'small_badge' => true]);
            echo '&nbsp; &nbsp;';
            echo $this->element('linkout_func', ['linkout_type' => 'amigo', 'query_term' => $go]);
            echo '&nbsp;';
            echo $this->element('linkout_func', ['linkout_type' => 'quickgo', 'query_term' => $go]);
            echo "</dd>\n";
                ...
                foreach ($gene_families as $gf_id => $transcript_count) {
                    ...
                    if (in_array('go', $exp_info['function_types'])) {
                        ...
                        for ($i = 0; $i < count($extra_annot_go[$gf_id]) && $i < $max_items; $i++) {
                            ...
                            echo '&nbsp;';
                            echo $this->element('go_category_badge', ['go_category' => $go_cat, 'small_badge' => true]);
                            echo '</li>';
                        }
                        echo '</ul>';
                        echo '</td>';

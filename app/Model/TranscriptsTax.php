...
class TranscriptsTax extends AppModel {
    ...
    function getSummaryAndLineages($exp_id) {
        $tax_array = []; // Array that will store result sand be returned
        // $set_options = 'SET GLOBAL group_concat_max_len=20000000';
        // mysql_query($set_options);
        $query =
            "SELECT tt.txid, tt.transcripts, full_taxonomy.tax from (select distinct txid, group_concat(transcript_id) as transcripts 
                  from transcripts_tax where experiment_id = '" .
            $exp_id .
            "' group by txid) as tt 
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $tax_id = $r['tt']['txid'];
            $tax_array[$tax_id] = ['transcripts' => $transcripts, 'lineage' => $lineage];
        }
        return $tax_array;
    }
}

...
class TranscriptsGo extends AppModel {
    ...
    function getExportData($exp_id) {
        $result = [];
        $query =
            "SELECT COUNT(`transcript_id`),`name`,`is_hidden` FROM `transcripts_annotation` WHERE `experiment_id`='" .
            $exp_id .
            "' AND `type`='go'";
    ...
    function findGoCountsFromTranscripts($exp_id, $transcript_ids) {
        ...
        $query =
            ...
            $transcripts_string .
            " AND `type`='go' GROUP BY `name` ORDER BY `count` DESC";
        $res = $this->query($query);
        foreach ($res as $r) {
            $go = $r['transcripts_annotation']['name'];
            $count = $r[0]['count'];
    ...
    function getStats($exp_id) {
        $query =
            ...
            "' AND `type`='go' ";
        $res = $this->query($query);
        $result = ['num_go' => 0, 'num_transcript_go' => 0];
        if ($res) {

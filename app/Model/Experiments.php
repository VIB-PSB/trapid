...
class Experiments extends AppModel {
    ...
    function getSharedExperiments($exp_ids) {
        ...
        $query1 =
            "SELECT Experiments.*, DataSources.`name`,DataSources.`URL` FROM `data_sources` DataSources, `experiments` Experiments
			 WHERE Experiments.experiment_id IN " .
            $exp_id_string .
            " AND DataSources.`db_name`=Experiments.`used_plaza_database`
    ...
    function getDefaultInformation($exp_id) {
        ...
        $query =
            'SELECT exp.*, exp_jobs.job_count AS job_count, tr.`transcript_count` AS transcript_count, tl.`label_count` AS label_count, ds.`name`, ds.`URL`, ds.`plaza_linkout`, ds.`gf_prefix`, ds.`go`, ds.`interpro`, ds.`ko` FROM `experiments` exp JOIN `data_sources` ds ON exp.`used_plaza_database`=ds.`db_name`,' .
            "(SELECT COUNT(transcript_id) AS transcript_count FROM transcripts WHERE experiment_id='" .
            $exp_id .
            "') tr, " .
            "(SELECT COUNT(DISTINCT `label`) AS label_count FROM `transcripts_labels` WHERE `experiment_id`='" .
            $exp_id .
            "') tl, " .
            "(SELECT COUNT(id) AS job_count FROM `experiment_jobs` WHERE `experiment_id` = '" .
            $exp_id .
            "') exp_jobs " .
        ...
        if ($res) {
            ...
            foreach (['go', 'interpro', 'ko'] as $fa_type) {
                if ($res[0]['ds'][$fa_type] == 1) {

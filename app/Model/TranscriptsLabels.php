...
/*
 * This model represents transcript label ('subsets' on the UI) information.
 */
...
class TranscriptsLabels extends AppModel {
    ...
    function getDataTranscript2Labels($exp_id) {
        ...
        foreach ($res as $r) {
            ...
            if (!array_key_exists($transcript_id, $tmp)) {
                $tmp[$transcript_id] = [];
            }
    ...
    function enterTranscripts($exp_id, $transcripts, $label) {
        ...
        foreach ($transcripts as $transcript_id) {
            if (array_key_exists($transcript_id, $all_transcripts)) {
                $counter++;
                $statement =
                    "INSERT INTO `transcripts_labels` (`experiment_id`,`transcript_id`,`label`) VALUES
				('" .
                    $exp_id .
                    "','" .
                    $transcript_id .
    ...
    function getLabels($exp_id) {
        $query =
            "SELECT `label`,count(`transcript_id`) as count FROM `transcripts_labels` WHERE `experiment_id`='" .
            $exp_id .
            "' GROUP BY `label` ";
        $res = $this->query($query);
        $result = [];
    ...
    function enterTranscriptsInsertMulti($exp_id, $transcripts, $label) {
        ...
        $fields = ['transcript_id', 'experiment_id', 'label'];
        $to_save = [];
        $counter = 0;
        foreach ($transcripts as $transcript) {
            $counter += 1;
            array_push($to_save, [$transcript, $exp_id, $label]);
        }

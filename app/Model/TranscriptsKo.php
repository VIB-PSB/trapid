...
class TranscriptsKo extends AppModel {
    ...
    function findTranscriptsFromKo($exp_id,$ko_terms){
        ...
        $query	= "SELECT `name`,count(`transcript_id`) as count FROM `transcripts_annotation` WHERE `experiment_id`='".$exp_id."' AND `type`='ko' AND `name` IN ".$ko_terms_string." GROUP BY `name` ";
        $res	= $this->query($query);
        foreach($res as $r){
            $result[$r['transcripts_annotation']['name']] = array("count"=>$r[0]['count'],"desc"=>$ko_terms[$r['transcripts_annotation']['name']]);
        }
        return $result;
    }

<?php
/*
 * Model for a reference database's annotation data.
 */
class Annotation extends AppModel {
    ...
    function getSequencesInterpro($interpro_id) {
        ...
        $gene_ids = [];
        foreach ($res as $r) {
            $gene_ids[] = $r['protein_motifs_data']['gene_id'];
        }
        return $this->getSequences($gene_ids);
    }
    ...
    function getLengths($species) {
        $query =
            "SELECT CHAR_LENGTH(`seq`) as `length` FROM `annotation` WHERE `species`='" .
            $species .
            "' AND `type`='coding'";
        $res = $this->query($query);
        $result = [];
    ...
    function getSpeciesGeneCounts() {
        $query =
            "SELECT `species`,count(`gene_id`) as count FROM `annotation` WHERE `type`='coding' GROUP BY `species`";
        $res = $this->query($query);
        $result = [];
        foreach ($res as $r) {
    ...
    function getSpeciesForGenes($gene_ids) {
        ...
        foreach ($res as $r) {
            ...
            if (!array_key_exists($spec, $result)) {
                $result[$spec] = [];
            }
            $result[$spec][] = $gene;
        }
    ...
    function getSpeciesProfile($gene_ids) {
        ...
        $gene_string = "('" . implode("','", $gene_ids) . "')";
        $query =
            'SELECT `species`,COUNT(`gene_id`) as count FROM `annotation` WHERE `gene_id` IN ' .
            $gene_string .
            ' GROUP BY `species` ';
        $res = $this->query($query);
        foreach ($res as $r) {

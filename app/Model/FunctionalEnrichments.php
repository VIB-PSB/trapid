...
class FunctionalEnrichments extends AppModel {
    ...
    function getEnrichedIdentifier($exp_id, $type) {
        ...
        foreach ($res as $r) {
            ...
            if (!isset($result[$label][$p_val])) {
                $result[$label][$p_val] = [];
            }
            $result[$label][$p_val][$ident] = [$hidden, $sign];
        }
        return $result;
    }
    ...
    function getSankeyEnrichmentResults($exp_id, $type) {
        ...
        return ['enrichment' => $enr_results, 'n_hits_gf' => $enr_hits_gf];
    }

<?php
/*
 * A model representing GF data  information (where each record corresponding to a GF member) from a reference database.
 */
class GfData extends AppModel {
    ...
    function getProfile($gene_ids, $gf_prefix) {
        ...
        if ($gf_prefix) {
            $query = $query . " AND `gf_id` LIKE '" . $gf_prefix . "%' ";
        }
    ...
    function getGeneCount($gf_ids) {
        ...
        $query =
            'SELECT `gf_id`,count(`gene_id`) as count FROM `gf_data` WHERE `gf_id` IN ' .
            $string_gf_ids .
            ' GROUP BY `gf_id` ';
        $res = $this->query($query);
        foreach ($res as $r) {
    ...
    function getGenes($gf_ids) {
        ...
        if (is_array($gf_ids)) {
            ...
            foreach ($res as $r) {
                ...
                if (!array_key_exists($gf_id, $result)) {
                    $result[$gf_id] = [];
                }
    ...
    function getSpeciesCount($gf_ids) {
        ...
        $string_gf_ids = "('" . implode("','", $gf_ids) . "')";
        $query =
            'SELECT `gf_id`, count(distinct `species`) as count FROM `gf_data` WHERE `gf_id` IN ' .
            $string_gf_ids .
            ' GROUP BY `gf_id` ';
        $res = $this->query($query);
        foreach ($res as $r) {
    ...
    function gfExists($ref_gf_id) {
        $gf_data = $this->find('first', ['conditions' => ['gf_id' => $ref_gf_id]]);
        if ($gf_data) {
            return true;
        } else {
            return false;
    ...
    function getTopGoTerms($ref_gf_id, $n_max) {
        $top_gos = ['BP' => [], 'MF' => [], 'CC' => []];
        $query =
            "SELECT gf_fd.`name`, fd.`desc`, fd.`info` FROM  `gf_functional_data` gf_fd, `functional_data` fd WHERE gf_fd.`gf_id`='" .
            $ref_gf_id .
            "' AND gf_fd.`type`='go' AND gf_fd.`is_hidden`='0' AND fd.`name`=gf_fd.`name` ORDER BY gf_fd.`f_score` DESC";
        $res = $this->query($query);
        foreach ($res as $r) {
            $go_aspect = $r['fd']['info'];
            ...
            if (count($top_gos[$go_aspect]) < $n_max) {
                $top_gos[$go_aspect][] = ['name' => $go_name, 'desc' => $go_desc];
            }
        }
    ...
    function getTopIprTerms($ref_gf_id, $n_max) {
        $top_iprs = [];
        $query =
            "SELECT gf_fd.`name`, fd.`desc` FROM  `gf_functional_data` gf_fd, `functional_data` fd WHERE gf_fd.`gf_id`='" .
            $ref_gf_id .
            "' AND gf_fd.`type`='interpro' AND gf_fd.`is_hidden`='0' AND fd.`name`=gf_fd.`name` ORDER BY gf_fd.`f_score` DESC";
        $res = $this->query($query);
        foreach ($res as $r) {
            $ipr_name = $r['gf_fd']['name'];
            $ipr_desc = $r['fd']['desc'];
            if (count($top_iprs) < $n_max) {
                $top_iprs[] = ['name' => $ipr_name, 'desc' => $ipr_desc];
            }
        }
        return $top_iprs;
    }
    ...
    function getEggnogTaxScope($nog_id) {
        ...
        if ($res) {
            $tax_scope_data = [
                'name' => $res[0]['taxonomic_levels']['name'],
                'scope' => $res[0]['taxonomic_levels']['scope']
            ];
        }
    ...
    function getEggnogFuncData($nog_id) {
        $func_data = [];
        $db = $this->getDataSource();
        $res = $db->fetchAll('SELECT `func_cats`, `description` FROM `gene_families` WHERE `gf_id` = ?', [$nog_id]);
        if ($res) {
            ...
            $func_cat_query =
                "SELECT group_concat(`description`) AS `func_cat_label` FROM `functional_categories` WHERE `func_cat_id` IN ('" .
                implode("','", $func_cats) .
                "')";

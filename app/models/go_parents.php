...
class GoParents extends AppModel{
  ...
  var $useDbConfig 	= "db_trapid_ref_plaza_monocots_03_test";
	...
	function getParentsDepth($go_id){
      ...
      // Also add a `type` constraint?  (a.`type`='go' AND b.`type`='go' [...])
      $query   = "SELECT a.`parent`, b.`num_sptr_steps`,b.`desc` FROM `functional_parents` a, `functional_data` b
 				WHERE a.`child`='".$go_id."'
 	      		AND a.`parent` = b.`name` ; ";
	   $rs	    = $this->query($query);
	   $result1 = array();
    ...
    $query 	   = "SELECT `child`,`parent` FROM `functional_parents`
    			WHERE `child` IN ('".$children_string."')
    			AND `parent` IN ('".$parents_string."') ;";

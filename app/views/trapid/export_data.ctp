...
<div class="subdiv">
	...
	<?php
		$unfinished = null;	
		if($exp_info['process_state']!="finished"){$unfinished = " disabled='disabled' ";}
	?>

	<h3>Export structural data</h3>
	<div class="subdiv bottom">
		<span style="font-weight:bold;">Select columns for output data</span><br/><br/>
		<?php echo $form->create("",array("action"=>"export_data/".$exp_id,"type"=>"post"));?>
		<input type="hidden" name="export_type" value="structural" />
		<?php
		foreach($structural_export as $k=>$v){
		if($k!="transcript_id"){echo "<input type='checkbox' name='".$k."' class='standard_checkbox' $unfinished />";}
		else{echo "<input type='checkbox' name='".$k."' checked='checked' class='standard_checkbox' />";}
	...
	<div class="subdiv">
		...
		<input type="submit" value="Transcripts with protein domain" class="bigbutton" <?php echo $unfinished;?>/>
		</form>
		</div>
	...
	<div class="subdiv">
		<?php 					
			echo $form->create("",array("action"=>"export_data/".$exp_id,"type"=>"post"));
			echo "<input type='hidden' name='export_type' value='subsets' />\n";
			
			echo "<div style='float:left;width:220px;'>\n";		
			$disabled = null; if(count($available_subsets)==0){$disabled=" disabled='disabled' ";}			

...
def parse_arguments():
    """Parse command-line arguments and return them (Namespace)"""
    ...
    cmd_parser.add_argument('--naive', dest='naive_scoring',
                            help='Score GFs without taking their weight into account (scoring exlcusively based on log10 e-values of top hits). '
                                 'Not providing this flag may give more importance to GFs having a large weight. ',
...
def main(output_dir, trapid_db_name, trapid_db_user, trapid_db_host, experiment_id, transcript_label, top_hits, species_perc, clade, naive_scoring):
    ...
    core_gf_analysis.process_blast_output(blast_df=trapid_df, n_hits=top_hits, gene_gf_map=gene_gf_map, output_dir=output_dir,
                         all_gfs_dict=all_gfs, naive_scoring=naive_scoring, gf_len=False, sqce_lengths=dict(), min_len=0.0)

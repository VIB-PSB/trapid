"""
A wrapper script to perform the entirety of a core GF completeness analysis (core GF retrieval + completeness analysis)
for a given TRAPID experiment, used within TRAPID 2.
"""
...
# Note: is it really needed to export GF weight to TRAPID datatabase? It can be computed easily (n_species/n_genes)
...
def parse_arguments():
    ...
    cmd_parser.add_argument('clade', help='Retrieve core GFs for which clade?')
    cmd_parser.add_argument('experiment_id', help='TRAPID experiment identifier. ')
    ...
    data_retrieval_args.add_argument(
        ...
        default=0.9)
    data_retrieval_args.add_argument('-db', '--trapid_db', type=str, help='Name of TRAPID database. ',
                                     default='db_trapid_dev')
    data_retrieval_args.add_argument(
        '-u', '--username', type=str,
    ...
    cmd_parser.add_argument(
        ...
        help=('Score GFs without taking their weight into account (scoring exclusively based on log10 e-values of '
              'top hits). Providing this flag may give more importance to GFs having a low weight. '),
